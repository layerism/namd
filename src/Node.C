...
void Node::startup() {
	...
	case 9:
		...
		if (!CkMyPe())
			LdbCoordinator::Object()->createLoadBalancer();
...
void Node::reloadCharges(const char *filename) {
	...
	if (!file)
		NAMD_die("node::reloadCharges():Error opening charge file.");
...
//lyk
void Node::loadAdaptCgParam(const char *filename) {

	FILE *file = fopen(filename, "r");
	if (!file)
		NAMD_die("node::loadAdaptCgParam():Error opening cg param file.");

	int natom;
	int ntype;
	char c[256];

	if (!fscanf(file, "%s", c)) //natom
		NAMD_die("Node::loadAdaptCgParam(): #natom");
	if (!fscanf(file, "%d", &natom))
		NAMD_die("Node::loadAdaptCgParam(): natom");

	//-----------------cg info---------------------
	float *charge = new float[natom];
	int *type = new int[natom];
	float *mass = new float[natom];
	int *atomIndex = new int[natom];

	if (!fscanf(file, "%s", c)) //index
		NAMD_die("Node::loadAdaptCgParam():#index");
	for (int i = 0; i < natom; ++i) {
		if (!fscanf(file, "%d", &atomIndex[i]))
			NAMD_die("Node::loadAdaptCgParam():index");
	}
	int maxId = atomIndex[natom - 1] + 1;
	int *aid2MapId = new int[maxId];
	...
	if (!fscanf(file, "%s", c)) //mass
		NAMD_die("Node::loadAdaptCgParam():#mass");
	for (int i = 0; i < natom; ++i) {
		if (!fscanf(file, "%f", &mass[i]))
			NAMD_die("Node::loadAdaptCgParam():mass");
	}

	if (!fscanf(file, "%s", c)) //charge
		NAMD_die("Node::loadAdaptCgParam():#charge");
	for (int i = 0; i < natom; ++i) {
		if (!fscanf(file, "%f", &charge[i]))
			NAMD_die("Node::loadAdaptCgParam():charge");
	}

	if (!fscanf(file, "%s", c)) //type
		NAMD_die("Node::loadAdaptCgParam():#type");
	for (int i = 0; i < natom; ++i) {
		if (!fscanf(file, "%d", &type[i]))
			NAMD_die("Node::loadAdaptCgParam():type");
	}

	//----------------vdw------------------
	if (!fscanf(file, "%s", c)) //ntype
		NAMD_die("Node::loadAdaptCgParam():#ntype");
	if (!fscanf(file, "%d", &ntype))
		NAMD_die("Node::loadAdaptCgParam():ntype");

	float *vdwEps = new float[ntype * ntype];
	float *vdwSgm = new float[ntype * ntype];
	if (!fscanf(file, "%s", c)) //vdwEps
		NAMD_die("Node::loadAdaptCgParam():#vdwEps");
	for (int i = 0; i < ntype * ntype; ++i) {
		if (!fscanf(file, "%f", &vdwEps[i]))
			NAMD_die("Node::loadAdaptCgParam():vdwEps");
	}

	if (!fscanf(file, "%s", c)) //vdwSgm
		NAMD_die("Node::loadAdaptCgParam():#vdwSgm");
	for (int i = 0; i < ntype * ntype; ++i) {
		if (!fscanf(file, "%f", &vdwSgm[i]))
			NAMD_die("Node::loadAdaptCgParam():vdwSgm");
	}

	//---------------exclude-----------------
	int nExcludePair = 0;
	if (!fscanf(file, "%s", c)) //excludePair
		NAMD_die("Node::loadAdaptCgParam():#nExcludePair");
	if (!fscanf(file, "%d", &nExcludePair)) //excludePair
		NAMD_die("Node::loadAdaptCgParam():#nExcludePair");

	unsigned long maxExcludePair = natom * natom;
	bool *exclude = new bool[maxExcludePair];
	for (unsigned long pair_i = 0; pair_i < maxExcludePair; ++pair_i) {
		exclude[pair_i] = false;
	}
	//
	//index for ij pair, i<j 0-base
	//i*(maxId-1)-(i-1)*i/2+j-i-1
	//
	if (!fscanf(file, "%s", c)) //exclude
		NAMD_die("Node::loadAdaptCgParam():#exclude");
	for (int i = 0; i < nExcludePair; ++i) {
		int aid1 = 0, aid2 = 0;
		if (!fscanf(file, "%d", &aid1))
			NAMD_die("Node::loadAdaptCgParam():exclude_id1");
		if (!fscanf(file, "%d", &aid2))
			NAMD_die("Node::loadAdaptCgParam():exclude_id2");
		//exclude[aid1 * (maxId - 1) - aid1 * (aid1 - 1) / 2 + aid2 - aid1 - 1] = true;
		exclude[aid2MapId[aid1] * natom + aid2MapId[aid2]] = true;
		exclude[aid2MapId[aid1] + aid2MapId[aid2] * natom] = true;
	}

	iout<<"ACG EXCLUDE PAIRS: \n"<<endi;
	for (int i = 0; i < natom - 1; ++i) {
		for (int j = i + 1; j < natom; ++j) {
			int idi = atomIndex[i];
			int idj = atomIndex[j];
			int excPair_ij = aid2MapId[idi] * natom + aid2MapId[idj];
			if (exclude[excPair_ij] == true)
				iout<<idi<<" "<<idj<<" ";
			}
		}
	iout<<"\n\n\n"<<endi;

	CProxy_Node(thisgroup).loadAdaptCgParam(atomIndex, charge, mass, type, vdwEps, vdwSgm, aid2MapId, exclude, natom,
			ntype, maxId);
...
void Node::loadAdaptCgParam(int atomindex[], float charge[], float mass[], int type[], float vdwEps[], float vdwSgm[],
		int aid2MapId[], bool exclude[], int n, int ntype, int maxId) {
	molecule->loadAdaptCgParam(atomindex, charge, mass, type, vdwEps, vdwSgm, aid2MapId, exclude, n, ntype, maxId);
}
...
void Node::outputPatchComputeMaps(const char *filename, int tag) {
	if (!simParameters->outputMaps && !simParameters->simulateInitialMapping)
		return;
	...
	fprintf(fp, "%d %d %d %d %d %d %d\n", numpes, nodesize, numPatches, numComputes, pMap->gridsize_a(),
			pMap->gridsize_b(), pMap->gridsize_c());
