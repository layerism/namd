...
void ComputeGlobal::configure(AtomIDList &newaid, AtomIDList &newgdef) {
	DebugM(4,
			"Receiving configuration (" << newaid.size() << " atoms and " << newgdef.size()
					<< " atoms/groups) on client\n");
...
void ComputeGlobal::recvResults(ComputeGlobalResultsMsg *msg) {
	DebugM(3,
			"Receiving results (" << msg->aid.size() << " forces, " << msg->newgdef.size()
	...
	if (setForces) { // we are requested to 
		...
		for (; a != a_e; ++a, ++f2) {
			...
			if (localID.pid == notUsed || !f[localID.pid])
				continue;
			Force f_atom = (*f2);
	...
	if (msg->reconfig)
		configure(msg->newaid, msg->newgdef);
...
void ComputeGlobal::sendData() {
	...
	for (; a != a_e; ++a) {
		LocalID localID = atomMap->localID(*a);
		if (localID.pid == notUsed || !t[localID.pid])
			continue;
		msg->aid.add(*a);
		msg->count++;
		FullAtom &atom = t[localID.pid][localID.index];
...
void ComputeGlobal::saveTotalForces(HomePatch *homePatch) {
	if (!forceSendEnabled)
		NAMD_bug("ComputeGlobal::saveTotalForces called unexpectedly");

	if (Node::Object()->simParameters->accelMDOn && Node::Object()->simParameters->accelMDDebugOn
			&& Node::Object()->simParameters->accelMDdihe) {
	...
	int num = homePatch->numAtoms;
	FullAtomList &atoms = homePatch->atom;
	ForceList &f1 = homePatch->f[Results::normal], &f2 = homePatch->f_saved[Results::nbond], &f3 =
			homePatch->f_saved[Results::slow];
	Force f_sum;
