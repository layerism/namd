...
inline void pairlistFromAll(nonbonded *params, GBISParamStruct *gbisParams, int minIg, int strideIg, int maxI) {
	...
	for (int ngi = minIg; ngi < maxI; /*ngi updated at loop bottom*/) {
		...
		for (int ngj = unique * (ngi + params->p[0][ngi].nonbondedGroupSize); ngj < params->numAtoms[1];
				ngj += params->p[1][ngj].nonbondedGroupSize) {
		...
		for (int i = ngi; i < ngi + iGroupSize; i++) {
			...
			for (int k = 0; k < numGBISPairlists; k++) {
				size[k] = 0;
				pairs[k] = gbisParams->gbisStepPairlists[k]->newlist(maxPairs);
			}
			...
			if (unique)
				for (int j = i + 1; j < ngi + iGroupSize; j++) {
					...
					if (r2 < ami2 && r2 < amj2 && r2 > rhois2_16 && r2 > 16.0 * rhojs2) {
						//belongs to 22
						pairs[0][size[0]++] = j;
					} else if (r2 < api2 && r2 < apj2 && r2 > ami2 && r2 > amj2) {
						//belongs to 11
						pairs[1][size[1]++] = j;
...
#endif
	if (params->numAtoms[0] == 0 || params->numAtoms[1] == 0)
		return;
		...
		for (int ngi = minIg; ngi < maxI; /**/) {
			...
			for (int i = ngi; i < ngi + iGroupSize; i++) {
				...
				for (register int jj = 0; jj < numPairs; jj++) {
					...
					rs2 = rhojs * rhojs;
					hij = /*0.125*r_i*/tmp1
							* (1 + rr * rmrsi + a_cut_i2 * (/*r2 - 4.0*a_cut*r*/tmp2 - rs2) + logrj + logrj);

					rmrsi = 1.0 / rmris;
					//rmrs2 = rmris*rmris;
					rs2 = rhois * rhois;
					hji = /*0.125*r_i*/tmp1
							* (1 + rr * rmrsi + a_cut_i2 * (/*r2 - 4.0*a_cut*r*/tmp2 - rs2) + 2.0 * logri);
//#if 1
#ifdef PRINT_COMP
					int id1 = params->pExt[0][i].id;
					int id2 = params->pExt[1][j].id;
		...
		for (int ngi = minIg; ngi < maxI; /**/) {
			...
			for (int i = ngi; i < ngi + iGroupSize; i++) {
				...
				for (register int jj = 0; jj < numPairs; jj++) {
					...
					CalcHPair(r, r2, r_i, a_cut, rhoi0, rhojs, rhoj0, rhois, dij, dji, hij, hji);
//#if 1
#ifdef PRINT_COMP
					int id1 = params->pExt[0][i].id;
					int id2 = params->pExt[1][j].id;
				...
				for (int i = ngi; i < ngi + iGroupSize; i++) {
					...
					for (int jj = 0; jj < numPairs; jj++) {
						...
						if (gbisParams->doFullElectrostatics) {
							...
							tmp_dEda = 0.5 * qiqj * f_i * f_i * (kappa * epsilon_s_i * expkappa - Dij * f_i)
		...
		for (int ngi = minIg; ngi < maxI; /**/) {
			...
			for (int i = ngi; i < ngi + iGroupSize; i++) {
				...
				for (jj = 0; jj < numPairs; jj++) {
					...
					k = rhojs * r_i;
					k *= k;  //k=(rs/r)^2
					dhij = -rhojs * r_i3 * k * (da + k * (db + k * (dc + k * (dd + k * de))));

					k = rhois * r_i;
					k *= k;  //k=(rs/r)^2
					dhji = -rhois * r_i3 * k * (da + k * (db + k * (dc + k * (dd + k * de))));
		...
		for (int ngi = minIg; ngi < maxI; /**/) {
			...
			for (int i = ngi; i < ngi + iGroupSize; i++) {
				...
				for (jj = 0; jj < numPairs; jj++) {
					...
					CalcDHPair(r, r2, r_i, a_cut, rhoi0, rhojs, rhoj0, rhois, dij, dji, dhij, dhji);
