...
struct dihedral_params {
	...
	dihedral_params() {
		memset(this , 0 , sizeof(dihedral_params));
	}
};
...

{
	...
	while (NAMD_read_line(pfile , buffer) != -1) {
		...
		/*  which is parsed.                                    */
		if (( buffer[0] != '!' ) && !NAMD_blank_string(buffer)
				&& ( strncasecmp(first_word , "REMARK" , 6) != 0 ) && ( strcasecmp(first_word , "set") != 0 )
...

{
	...
	while (NAMD_read_line(pfile , buffer) != -1) {
		...
		/*  start with "END".                                          */
		if (!NAMD_blank_string(buffer) && ( strncmp(first_word , "!" , 1) != 0 )
				&& ( strncmp(first_word , "*" , 1) != 0 ) && ( strncasecmp(first_word , "END" , 3) != 0 )) {
			if (skipall) {
				iout<< iWARN
...

{
	...
	if (paramType == paraXplor) {
		/* read XPLOR format */
		read_count = sscanf(buf , "%*s %s %s %f %f\n" , atom1name , atom2name , &forceconstant , &distance);
	} else if (paramType == paraCharmm) {
		/* read CHARMM format */
...
struct bond_params *Parameters::add_to_bond_tree(struct bond_params *new_node , struct bond_params *tree)
	...
	if (compare_code == 0) {
		...
		if (compare_code == 0) {
			...
			if (( tree->forceconstant != new_node->forceconstant )
				...
				<< new_node->atom1name << "-" << new_node->atom2name
				<< "\nPREVIOUS VALUES  k=" << tree->forceconstant
				<< "  x0=" << tree->distance << "\n   USING VALUES  k="
				<< new_node->forceconstant << "  x0="
				<< new_node->distance << "\n" << endi;
...
struct angle_params *Parameters::add_to_angle_tree(struct angle_params *new_node , struct angle_params *tree)

{
	...
	if (compare_code == 0) {
		...
		if (compare_code == 0) {
			...
			compare_code = strcasecmp(new_node->atom3name , tree->atom3name);
			...
			if (compare_code == 0) {
				...
				//****** BEGIN CHARMM/XPLOR type changes
				/* we do not care about identical replacement */
				if (( tree->forceconstant != new_node->forceconstant ) || ( tree->angle != new_node->angle )
						|| ( tree->k_ub != new_node->k_ub ) || ( tree->r_ub != new_node->r_ub )
						|| ( tree->normal != new_node->normal )) {
					iout<< "\n" << iWARN << "DUPLICATE ANGLE ENTRY FOR "
					<< new_node->atom1name << "-" << new_node->atom2name
					<< "-" << new_node->atom3name
					<< "\nPREVIOUS VALUES  k=" << tree->forceconstant
					<< "  theta0=" << tree->angle << " k_ub="
					<< tree->k_ub << " r_ub=" << tree->r_ub
					<< "\n   USING VALUES  k="
					<< new_node->forceconstant << "  theta0="
					<< new_node->angle << " k_ub=" << new_node->k_ub
					<< " r_ub=" << new_node->r_ub << "\n" << endi;
...

{
	...
	if (paramType == paraXplor) {
		...
		read_count = sscanf(buf , "%*s %s %s %s %s MULTIPLE= %d %f %d %f\n" , atom1name , atom2name ,
				atom3name , atom4name , &multiplicity , &forceconstant , &periodicity , &phase_shift);
	} else if (paramType == paraCharmm) {
		/* read CHARMM format */
	...
	if (multiplicity > MAX_MULTIPLICITY) {
		...
		sprintf(err_msg , "Multiple dihedral with multiplicity of %d greater than max of %d" , multiplicity ,
		MAX_MULTIPLICITY);
		NAMD_die(err_msg);
	}
	...
	if (paramType == paraXplor && periodicity != 0)
		phase_shift *= -1;
...
void Parameters::add_to_dihedral_list(struct dihedral_params *new_node)

{
	...
	while (ptr != NULL) {
		if (( ( strcasecmp(new_node->atom1name , ptr->atom1name) == 0 )
			...
			if (ptr->multiplicity != new_node->multiplicity) {
				echoWarn = 1;
			}

			if (!echoWarn) {
				for (i = 0; i < ptr->multiplicity; i++) {
					if (ptr->values[i].k != new_node->values[i].k) {
						echoWarn = 1;
						break;
					}
					if (ptr->values[i].n != new_node->values[i].n) {
						echoWarn = 1;
						break;
					}
					if (ptr->values[i].delta != new_node->values[i].delta) {
						echoWarn = 1;
						break;
					}
				}
			...
			if (echoWarn) {
				...
				<< ptr->atom1name << "-" << ptr->atom2name << "-"
				<< ptr->atom3name << "-" << ptr->atom4name
				<< "\nPREVIOUS VALUES MULTIPLICITY "
				<< ptr->multiplicity << "\n";

				for (i = 0; i < ptr->multiplicity; i++) {
					iout << "  k=" << ptr->values[i].k << "  n="
					<< ptr->values[i].n << "  delta="
					<< ptr->values[i].delta;
				}
				...
				for (i = 0; i < new_node->multiplicity; i++) {
					iout << "  k=" << new_node->values[i].k << "  n="
					<< new_node->values[i].n << "  delta="
					<< new_node->values[i].delta;
				}
	...
	if (new_node->atom1wild || new_node->atom2wild || new_node->atom3wild || new_node->atom4wild) {
...
void Parameters::add_to_charmm_dihedral_list(struct dihedral_params *new_node)
		...
		if (( ( strcasecmp(new_node->atom1name , ptr->atom1name) == 0 )
			...
			if (echoWarn) {
				...
				if (!replace) {
					...
					if (!same_as_last)
					iout << "INCREASING MULTIPLICITY TO: "
					<< ptr->multiplicity << "\n";
...

{
	...
	if (multiplicity > MAX_MULTIPLICITY) {
		...
		sprintf(err_msg , "Multiple improper with multiplicity of %d greater than max of %d" , multiplicity ,
		MAX_MULTIPLICITY);
		NAMD_die(err_msg);
	}
...

{
	...
	while (ptr != NULL) {
		if (( ( strcasecmp(new_node->atom1name , ptr->atom1name) == 0 )
			...
			if (echoWarn) {
				...
				iout << "\n" << "USING VALUES MULTIPLICITY "
				<< new_node->multiplicity << "\n";
	...
	/*  linear search for a bond and be guaranteed to have specific */
	/*  entries take precendence over over wildcards          */
	if (( strcasecmp(new_node->atom1name , "X") == 0 ) || ( strcasecmp(new_node->atom2name , "X") == 0 )
...

{
	...
	read_count = sscanf(buf , "%s %s %s %s %s %s %s %s %d\n" , atom1name , atom2name , atom3name , atom4name ,
			atom5name , atom6name , atom7name , atom8name , &dimension);
	...
	while (nread < nterms) {
		...
		read_count = sscanf(buffer , "%lf %lf %lf %lf %lf %lf %lf %lf\n" , new_node->values + nread ,
				new_node->values + nread + 1 , new_node->values + nread + 2 , new_node->values + nread + 3 ,
				new_node->values + nread + 4 , new_node->values + nread + 5 , new_node->values + nread + 6 ,
...

{
	...
	while (ptr != NULL) {
		if (( ( strcasecmp(new_node->atom1name , ptr->atom1name) == 0 )
			...
			if (ptr->dimension != new_node->dimension) {
				echoWarn = 1;
			}
			...
			if (!echoWarn) {
				...
				for (i = 0; i < nvals; i++) {
					if (ptr->values[i] != new_node->values[i]) {
						echoWarn = 1;
						break;
					}
				}
			...
			if (echoWarn) {
				...
				<< ptr->atom1name << "-" << ptr->atom2name << "-"
				<< ptr->atom3name << "-" << ptr->atom4name << " "
				<< ptr->atom5name << "-" << ptr->atom6name << "-"
				<< ptr->atom7name << "-" << ptr->atom8name
				<< ", USING NEW VALUES\n";
...
struct vdw_params *Parameters::add_to_vdw_tree(struct vdw_params *new_node , struct vdw_params *tree)

{
	...
	if (compare_code == 0) {
		...
		if (( tree->sigma != new_node->sigma ) || ( tree->epsilon != new_node->epsilon )
				|| ( tree->sigma14 != new_node->sigma14 ) || ( tree->epsilon14 != new_node->epsilon14 )) {
			iout<< iWARN << "DUPLICATE vdW ENTRY FOR " << tree->atomname
			<< "\nPREVIOUS VALUES  sigma=" << tree->sigma << " epsilon="
			<< tree->epsilon << " sigma14=" << tree->sigma14
			<< " epsilon14=" << tree->epsilon14
			<< "\n   USING VALUES  sigma=" << new_node->sigma
			<< " epsilon=" << new_node->epsilon << " sigma14="
			<< new_node->sigma14 << " epsilon14=" << new_node->epsilon14
			<< "\n" << endi;
...

{
	...
	if (paramType == paraXplor) {
		/* read XPLOR format */
		read_count = sscanf(buf , "%*s %s %s %f %f %f %f\n" , atom1name , atom2name , &A , &B , &A14 , &B14);
	} else if (paramType == paraCharmm) {
		// XXX CHARMM CAN HAVE 1-4 PARAMETERS TOO!!!
		/* read CHARMM format */
		read_count = sscanf(buf , "%s %s %f %f\n" , atom1name , atom2name , &A , &B);
	...
	while (ptr != NULL) {
		...
		if (compare_code == 0) {
			...
			if (compare_code == 0) {
				...
				/*  message, assign the values to the current   */
				/*  node in the tree, and then free the new_node*/
				if (( ptr->A != new_node->A ) || ( ptr->B != new_node->B ) || ( ptr->A14 != new_node->A14 )
						|| ( ptr->B14 != new_node->B14 )) {
					iout<< iWARN << "DUPLICATE vdW PAIR ENTRY FOR "
					<< new_node->atom1name << "-" << new_node->atom2name
					<< "\nPREVIOUS VALUES  A=" << ptr->A << " B="
					<< ptr->B << " A14=" << ptr->A14 << " B14"
					<< ptr->B14 << "\n   USING VALUES  A="
					<< new_node->A << " B=" << new_node->B << " A14="
					<< new_node->A14 << " B14" << new_node->B14 << "\n"
					<< endi;
...

{
	...
	while (ptr != NULL) {
		...
		for (i = 0; i < N; i++) {
			crossterm_array[index].c[i][N].d00 = crossterm_array[index].c[i][0].d00;
			crossterm_array[index].c[N][i].d00 = crossterm_array[index].c[0][i].d00;
		}
		crossterm_array[index].c[N][N].d00 = crossterm_array[index].c[0][0].d00;
...
void Parameters::assign_vdw_index(char *atomtype , Atom *atom_ptr)
	...
	if (!found) {
		...
		while (!found && ( ptr != NULL )) {
			...
			if (comp_code == 0) {
				...
				found = 1;
				char errbuf[100];
				sprintf(errbuf , "VDW TYPE NAME %s MATCHES PARAMETER TYPE NAME %s" , atomtype ,
						ptr->atomname);
				int i;
				for (i = 0; i < error_msgs.size(); i++) {
					if (strcmp(errbuf , error_msgs[i]) == 0)
						break;
				}
	...
	if (!found) {
		...
		sprintf(err_msg , "DIDN'T FIND vdW PARAMETER FOR ATOM TYPE %s" , atomtype);
		NAMD_die(err_msg);
	}
...
//    printf("Comparing %i with %i and %i with %i\n", ind1, ptr->ind1, ind2, ptr->ind2);
		if (( ind1 == ptr->ind1 ) && ( ind2 == ptr->ind2 )) {
			found = TRUE;
		} else if (( ind1 < ptr->ind1 ) || ( ( ind1 == ptr->ind1 ) && ( ind2 < ptr->ind2 ) )) {
			/*  Go left          */
			ptr = ptr->left;
		} else {
			/*  Go right          */
...
int Parameters::get_vdw_pair_params(Index ind1 , Index ind2 , Real *A , Real *B , Real *A14 , Real *B14)
...

{
	...
	if (!found) {
		if (( strcmp(atom1 , "DRUD") == 0 || strcmp(atom2 , "DRUD") == 0 )
			...
			sprintf(err_msg , "UNABLE TO FIND BOND PARAMETERS FOR %s %s (ATOMS %i %i)" , atom1 , atom2 ,
					bond_ptr->atom1 + 1 , bond_ptr->atom2 + 1);
			NAMD_die(err_msg);
		}
	}
...
void Parameters::assign_angle_index(char *atom1 , char *atom2 , char*atom3 , Angle *angle_ptr ,
		int notFoundIndex)
	...
	if (!found) {
		...
		sprintf(err_msg , "UNABLE TO FIND ANGLE PARAMETERS FOR %s %s %s (ATOMS %i %i %i)" , atom1 , atom2 ,
				atom3 , angle_ptr->atom1 + 1 , angle_ptr->atom2 + 1 , angle_ptr->atom3 + 1);
...
void Parameters::assign_dihedral_index(char *atom1 , char *atom2 , char *atom3 , char *atom4 ,
		Dihedral *dihedral_ptr , int multiplicity , int notFoundIndex)
	...
	if (!found) {
		...
		sprintf(err_msg , "UNABLE TO FIND DIHEDRAL PARAMETERS FOR %s %s %s %s (ATOMS %i %i %i %i)" , atom1 ,
				atom2 , atom3 , atom4 , dihedral_ptr->atom1 + 1 , dihedral_ptr->atom2 + 1 ,
	...
	if (multiplicity > maxDihedralMults[ptr->index]) {
		...
		sprintf(err_msg , "Multiplicity of Paramters for diehedral bond %s %s %s %s of %d exceeded" , atom1 ,
				atom2 , atom3 , atom4 , maxDihedralMults[ptr->index]);
		NAMD_die(err_msg);
	}
...
void Parameters::assign_improper_index(char *atom1 , char *atom2 , char *atom3 , char *atom4 ,
		Improper *improper_ptr , int multiplicity)
	...
	while (!found && ( ptr != NULL )) {
		...
		/*  in reverse, since they are really the same          */
		/*  physically.            */
		if (( ( strcasecmp(ptr->atom1name , atom1) == 0 ) || ( strcasecmp(ptr->atom1name , "X") == 0 ) )
				&& ( ( strcasecmp(ptr->atom2name , atom2) == 0 ) || ( strcasecmp(ptr->atom2name , "X") == 0 ) )
				&& ( ( strcasecmp(ptr->atom3name , atom3) == 0 ) || ( strcasecmp(ptr->atom3name , "X") == 0 ) )
				&& ( ( strcasecmp(ptr->atom4name , atom4) == 0 ) || ( strcasecmp(ptr->atom4name , "X") == 0 ) )) {
			/*  Found an exact match      */
			found = 1;
		} else if (( ( strcasecmp(ptr->atom4name , atom1) == 0 ) || ( strcasecmp(ptr->atom4name , "X") == 0 ) )
				&& ( ( strcasecmp(ptr->atom3name , atom2) == 0 ) || ( strcasecmp(ptr->atom3name , "X") == 0 ) )
				&& ( ( strcasecmp(ptr->atom2name , atom3) == 0 ) || ( strcasecmp(ptr->atom2name , "X") == 0 ) )
				&& ( ( strcasecmp(ptr->atom1name , atom4) == 0 ) || ( strcasecmp(ptr->atom1name , "X") == 0 ) )) {
			/*  Found a reverse match      */
			found = 1;
		} else {
			/*  Didn't find a match, go to the next node  */
	...
	if (!found) {
		...
		sprintf(err_msg , "UNABLE TO FIND IMPROPER PARAMETERS FOR %s %s %s %s (ATOMS %i %i %i %i)" , atom1 ,
				atom2 , atom3 , atom4 , improper_ptr->atom1 + 1 , improper_ptr->atom2 + 1 ,
	...
	if (multiplicity > maxImproperMults[ptr->index]) {
		...
		sprintf(err_msg , "Multiplicity of Paramters for improper bond %s %s %s %s of %d exceeded" , atom1 ,
				atom2 , atom3 , atom4 , maxImproperMults[ptr->index]);
		NAMD_die(err_msg);
	}
...
void Parameters::assign_crossterm_index(char *atom1 , char *atom2 , char *atom3 , char *atom4 , char *atom5 ,
		char *atom6 , char *atom7 , char *atom8 , Crossterm *crossterm_ptr) {
	...
	while (!found && ( ptr != NULL )) {
		...
		found = 0;
		if (( ( strcasecmp(ptr->atom5name , atom5) == 0 ) || ( strcasecmp(ptr->atom5name , "X") == 0 ) )
				&& ( ( strcasecmp(ptr->atom6name , atom6) == 0 ) || ( strcasecmp(ptr->atom6name , "X") == 0 ) )
				&& ( ( strcasecmp(ptr->atom7name , atom7) == 0 ) || ( strcasecmp(ptr->atom7name , "X") == 0 ) )
				&& ( ( strcasecmp(ptr->atom8name , atom8) == 0 ) || ( strcasecmp(ptr->atom8name , "X") == 0 ) )) {
			/*  Found an exact match      */
			found = 1;
		} else if (( ( strcasecmp(ptr->atom8name , atom5) == 0 ) || ( strcasecmp(ptr->atom8name , "X") == 0 ) )
				&& ( ( strcasecmp(ptr->atom7name , atom6) == 0 ) || ( strcasecmp(ptr->atom7name , "X") == 0 ) )
				&& ( ( strcasecmp(ptr->atom6name , atom7) == 0 ) || ( strcasecmp(ptr->atom6name , "X") == 0 ) )
				&& ( ( strcasecmp(ptr->atom5name , atom8) == 0 ) || ( strcasecmp(ptr->atom5name , "X") == 0 ) )) {
			/*  Found a reverse match      */
			found = 1;
		}
		if (!found) {
	...
	if (!found) {
		...
		sprintf(err_msg , "UNABLE TO FIND CROSSTERM PARAMETERS FOR %s  %s  %s  %s  %s  %s  %s  %s\n"
				...
				atom8 , crossterm_ptr->atom1 + 1 , crossterm_ptr->atom1 + 2 , crossterm_ptr->atom1 + 3 ,
				crossterm_ptr->atom4 + 1 , crossterm_ptr->atom1 + 5 , crossterm_ptr->atom1 + 6 ,
				crossterm_ptr->atom1 + 7 , crossterm_ptr->atom8 + 1);
...

{
	...
	DebugM(3 ,
			"BOND " << tree->atom1name << "  " << tree->atom2name << " index=" << tree->index << " k=" << tree->forceconstant << " x0=" << tree->distance);
...

{
	...
	if (tree->left != NULL) {
		traverse_angle_params(tree->left);
			...
			"ANGLE " << tree->atom1name << "  " << tree->atom2name << "  " << tree->atom3name << " index=" << tree->index << " k=" << tree->forceconstant << " theta0=" << tree->angle);
...

{
	...
	while (list != NULL) {
		DebugM(3 ,
				"DIHEDRAL  " << list->atom1name << "  " << list->atom2name << "  " << list->atom3name << "  " << list->atom4name << " index=" << list->index << " multiplicity=" << list->multiplicity << "\n");

		for (i = 0; i < list->multiplicity; i++) {
			DebugM(3 ,
					"k=" << list->values[i].k << " n=" << list->values[i].n << " delta=" << list->values[i].delta);
		}
...

{
	...
	while (list != NULL) {
		DebugM(3 ,
				"Improper  " << list->atom1name << "  " << list->atom2name << "  " << list->atom3name << "  " << list->atom4name << " index=" << list->index << " multiplicity=" << list->multiplicity << "\n");
...

{
	...
	DebugM(3 ,
			"vdW " << tree->atomname << " index=" << tree->index << " sigma=" << tree->sigma << " epsilon=" << tree->epsilon << " sigma 1-4=" << tree->sigma14 << " epsilon 1-4=" << tree->epsilon14 << std::endl);
...

{
	...
	DebugM(3 ,
			"vdW PAIR  " << list->atom1name << "  " << list->atom2name << " A=" << list->A << " B=" << list->B << " A 1-4=" << list->A14 << " B 1-4=" << list->B14);
...

{
	...
	DebugM(3 ,
			"NBTHOLE PAIR  " << list->atom1name << "  " << list->atom2name << " tholeij =" << list->tholeij);
...
void Parameters::print_nbthole_pair_params() {
	DebugM(3 ,
			NumNbtholePairParams << " NBTHOLE PAIR PARAMETERS\n" << "*****************************************");
...
void Parameters::send_Parameters(MOStream *msg) {
	...
	if (NumAngleParams) {
		...
		if (( a1 == NULL ) || ( a2 == NULL ) || ( a3 == NULL ) || ( a4 == NULL ) || ( i1 == NULL )) {
			NAMD_die("memory allocation failed in Parameters::send_Parameters");
		}
	...
	if (NumDihedralParams) {
		...
		if (( i1 == NULL ) || ( kvals == NULL ) || ( nvals == NULL ) || ( deltavals == NULL )) {
			NAMD_die("memory allocation failed in Parameters::send_Parameters");
		}
...

{
	...
	if (NumAngleParams) {
		...
		if (( angle_array == NULL ) || ( a1 == NULL ) || ( a2 == NULL ) || ( a3 == NULL ) || ( a4 == NULL )
				|| ( i1 == NULL )) {
			NAMD_die("memory allocation failed in Parameters::receive_Parameters");
		}
	...
	if (NumVdwParams) {
		...
		if (( vdw_array == NULL ) || ( a1 == NULL ) || ( a2 == NULL ) || ( a3 == NULL ) || ( a4 == NULL )
				|| ( atomTypeNames == NULL )) {
			NAMD_die("memory allocation failed in Parameters::receive_Parameters");
		}
	...
	if (NumNbtholePairParams) {
		...
		if (( nbthole_array == NULL ) || ( a1 == NULL ) || ( a2 == NULL ) || ( a3 == NULL ) || ( i1 == NULL )
				|| ( i2 == NULL )) {
			NAMD_die("memory allocation failed in Parameters::receive_Parameters");
		}
...
IndexedTablePair *Parameters::add_to_indexed_table_pairs(IndexedTablePair *new_node , IndexedTablePair *tree)
...
IndexedVdwPair *Parameters::add_to_indexed_vdw_pairs(IndexedVdwPair *new_node , IndexedVdwPair *tree)
...
int Parameters::vdw_pair_to_arrays(int *ind1_array , int *ind2_array , Real *A , Real *A14 , Real *B ,
		Real *B14 , int arr_index , IndexedVdwPair *tree)
	...
	arr_index = vdw_pair_to_arrays(ind1_array , ind2_array , A , A14 , B , B14 , arr_index , tree->left);
	arr_index = vdw_pair_to_arrays(ind1_array , ind2_array , A , A14 , B , B14 , arr_index , tree->right);
...
int Parameters::nbthole_pair_to_arrays(int *ind1_array , int *ind2_array , Real *alphai , Real *alphaj ,
		Real *tholeij , int arr_index , IndexedNbtholePair *tree)
	...
	arr_index = nbthole_pair_to_arrays(ind1_array , ind2_array , alphai , alphaj , tholeij , arr_index ,
			tree->left);
	arr_index = nbthole_pair_to_arrays(ind1_array , ind2_array , alphai , alphaj , tholeij , arr_index ,
			tree->right);
...
int Parameters::table_pair_to_arrays(int *ind1_array , int *ind2_array , int *K , int arr_index ,
	...
	arr_index = table_pair_to_arrays(ind1_array , ind2_array , K , arr_index , tree->left);
	arr_index = table_pair_to_arrays(ind1_array , ind2_array , K , arr_index , tree->right);
...
void Parameters::read_parm(Ambertoppar *amber_data , BigReal vdw14) {
	...
	for (i = 0; i < NumDihedralParams; ++i) {
		...
		if (amber_data->Pn[i] > 0) {
			...
			char err_msg[181];
			sprintf(err_msg , "Multiple dihedral with multiplicity of %d greater than max of %d" , mul + 1 ,
			MAX_MULTIPLICITY);
			NAMD_die(err_msg);
		}
	}
...
void Parameters::read_parm(const GromacsTopFile *gf , Bool min) {
	...
	for (i = 0; i < NumBondParams; i++) {
		Real x0 , k;
		gf->getBondParams(i , &x0 , // the bond length
	...
	for (i = 0; i < NumAngleParams; i++) {
		Real theta0 , k;
		gf->getAngleParams(i , &theta0 , // the angle size
				&k ,      // the spring constant
				&funct);                // discarded
	...
	for (i = 0; i < NumDihedralParams; ++i) { // iterate over all dihedral angles
		...
		case 3: ////////// it is a Ryckaert-Bellemans dihedral
			...
			for (j = 0; j < 5; j++)
				dihedral_array[i].values[j].delta = 0;
			for (j = 0; j < 5; j++)
				dihedral_array[i].values[j].n = j + 1;
	...
	for (i = 0; i < numtype; i++) {
		for (j = i; j < numtype; j++) {
			...
			gf->getVDWParams(i , j , &( new_node->B ) , &( new_node->A ) , &( new_node->B14 ) ,
					&( new_node->A14 ));
...
//      printf("Setting table_types[%i] to %s\n", numtypesread, newtypename);
			...
			if (!strncasecmp(interp_type , "linear" , 6)) {
				...
				if (read_energy_type_bothcubspline(enertable , numtypesread , table_ener , table_spacing ,
						maxdist) != 0) {
					char err_msg[512];
...
int Parameters::read_energy_type_bothcubspline(FILE* enertable , const int typeindex , BigReal* table_ener ,
		const float table_spacing , const float maxdist) {
...
//    printf("Interpolating on interval %i\n", i);
		...
		for (j = 0; j < entriesperseg; j++) {
			...
			if (distbin >= (int) mynearbyint(maxdist / table_spacing))
				break;
			BigReal energy;
			BigReal force;

			energy = Ae + ( Be * mydistfrac ) + ( Ce * mydistfrac * mydistfrac )
					+ ( De * mydistfrac * mydistfrac * mydistfrac );
			force = Af + ( Bf * mydistfrac ) + ( Cf * mydistfrac * mydistfrac )
					+ ( Df * mydistfrac * mydistfrac * mydistfrac );
...
int Parameters::read_energy_type_cubspline(FILE* enertable , const int typeindex , BigReal* table_ener ,
		const float table_spacing , const float maxdist) {
...
// Perform cubic spline interpolation to get the energies and forces
	...
	for (i = 0; i < numentries - 1; i++) {
		...
		for (j = 0; j < entriesperseg; j++) {
			...
			energy = A + ( B * mydistfrac ) + ( C * mydistfrac * mydistfrac )
					+ ( D * mydistfrac * mydistfrac * mydistfrac );
			force = B + ( 2 * C * mydistfrac ) + ( 3 * D * mydistfrac * mydistfrac );
			// Multiply force by 1 / (interval length)
			force *= ( 1.0 / spacing );

			printf("Adding energy/force entry %f / %f in bins %i / %i for distance %f (%f)\n" , energy ,
					force , ( table_prefix + 2 * distbin ) , ( table_prefix + 2 * distbin + 1 ) , mydist ,
					mydistfrac);
			table_ener[table_prefix + 2 * distbin] = energy;
			table_ener[table_prefix + 2 * distbin + 1] = force;
			distbin++;
	...
	//The procedure above leaves out the last entry -- add it explicitly
	distbin = (int) mynearbyint(maxdist / table_spacing);
	printf("Adding energy/force entry %f / %f in bins %i / %i\n" , enervalues[numentries - 1] , 0.0 ,
			( table_prefix + 2 * distbin ) , ( table_prefix + 2 * distbin + 1 ));
	table_ener[table_prefix + 2 * distbin] = enervalues[numentries - 1];
	table_ener[table_prefix + 2 * distbin + 1] = 0.0;
	distbin++;
...
int Parameters::read_energy_type(FILE* enertable , const int typeindex , BigReal* table_ener ,
		const float table_spacing , const float maxdist) {
	...
	while (fgets(tableline , 120 , enertable) && distbin <= (int) ( mynearbyint(maxdist / table_spacing) + 1 )) {
		...
		while (currdist <= readdist && distbin <= (int) ( mynearbyint(maxdist / table_spacing) )) {
			...
			int table_loc = 2
					* ( distbin + ( typeindex * ( 1 + (int) mynearbyint(maxdist / table_spacing) ) ) );
			printf("Doing interpolation for energy between %f %f and %f %f: Dist %f\n" , readener , readdist ,
					lastener , lastdist , currdist);
