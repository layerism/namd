...
public:
	...
	int operator<(const Exclusion &o) const {
		return ((atom1 < o.atom1) || (atom1 == o.atom1 && atom2 < o.atom2));
	}
};
...
enum TupleSigType {
	BOND = 0, ANGLE, DIHEDRAL, IMPROPER, DONOR, ACCEPTOR, CROSSTERM, EXCLUSION
};
...
public:
	...
	int operator==(const AtomSignature& sig) const {
		if (bondCnt != sig.bondCnt)
			return 0;
		if (angleCnt != sig.angleCnt)
			return 0;
		if (dihedralCnt != sig.dihedralCnt)
			return 0;
		if (improperCnt != sig.improperCnt)
			return 0;
		if (crosstermCnt != sig.crosstermCnt)
			return 0;
...
struct ExclusionSignature {
	...
	int operator==(const ExclusionSignature& sig) const {
		if (fullExclCnt != sig.fullExclCnt)
			return 0;
		if (modExclCnt != sig.modExclCnt)
			return 0;

		for (int i = 0; i < fullExclCnt; i++) {
			if (fullOffset[i] != sig.fullOffset[i])
				return 0;
		}
		for (int i = 0; i < modExclCnt; i++) {
			if (modOffset[i] != sig.modOffset[i])
				return 0;
		}
		return 1;
	}
