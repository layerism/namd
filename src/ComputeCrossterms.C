...
enum {
	CMAP_TABLE_DIM = 25, CMAP_SETUP_DIM = 24, CMAP_SPACING = 15, CMAP_PHI_0 = -180, CMAP_PSI_0 = -180
};
...
void CrosstermElem::computeForce(BigReal *reduction, BigReal *pressureProfileData) {
	DebugM(3,
			"::computeForce() localIndex = " << localIndex[0] << " " << localIndex[1] << " " << localIndex[2] << " " << localIndex[3] << std::endl);
	...
	//  Calculate the energy
	{
		...
		if (ilo < 0)
			ilo = 0;
		if (ilo >= CMAP_SETUP_DIM)
			ilo = CMAP_SETUP_DIM - 1;
		jlo = (int) floor(dy_h);
		if (jlo < 0)
			jlo = 0;
		if (jlo >= CMAP_SETUP_DIM)
			jlo = CMAP_SETUP_DIM - 1;
	...
	if (fabs(sin_phi) > 0.1) {
		...
		f2.x += U_phi * (r12.z * dcosdA.y - r12.y * dcosdA.z + r34.y * dcosdB.z - r34.z * dcosdB.y);
		f2.y += U_phi * (r12.x * dcosdA.z - r12.z * dcosdA.x + r34.z * dcosdB.x - r34.x * dcosdB.z);
		f2.z += U_phi * (r12.y * dcosdA.x - r12.x * dcosdA.y + r34.x * dcosdB.y - r34.y * dcosdB.x);
	} else {
		...
		f2.x += U_phi
				* (-(r23.y * r12.y + r23.z * r12.z) * dsindC.x + (2.0 * r23.x * r12.y - r12.x * r23.y) * dsindC.y
						+ (2.0 * r23.x * r12.z - r12.x * r23.z) * dsindC.z + dsindB.z * r34.y - dsindB.y * r34.z);
		f2.y += U_phi
				* (-(r23.z * r12.z + r23.x * r12.x) * dsindC.y + (2.0 * r23.y * r12.z - r12.y * r23.z) * dsindC.z
						+ (2.0 * r23.y * r12.x - r12.y * r23.x) * dsindC.x + dsindB.x * r34.z - dsindB.z * r34.x);
		f2.z += U_phi
				* (-(r23.x * r12.x + r23.y * r12.y) * dsindC.z + (2.0 * r23.z * r12.x - r12.z * r23.x) * dsindC.x
	...
	if (fabs(sin_psi) > 0.1) {
		...
		f5.x += U_psi * (r56.z * dcosdD.y - r56.y * dcosdD.z + r78.y * dcosdE.z - r78.z * dcosdE.y);
		f5.y += U_psi * (r56.x * dcosdD.z - r56.z * dcosdD.x + r78.z * dcosdE.x - r78.x * dcosdE.z);
		f5.z += U_psi * (r56.y * dcosdD.x - r56.x * dcosdD.y + r78.x * dcosdE.y - r78.y * dcosdE.x);
	} else {
		//  This angle is closer to 0 or 180 than it is to
		//  90, so use the cos version to avoid 1/sin terms
		U_psi = -U_psi / cos_psi;
		...
		f4.x += U_psi
				* ((r67.y * r67.y + r67.z * r67.z) * dsindF.x - r67.x * r67.y * dsindF.y - r67.x * r67.z * dsindF.z);
		f4.y += U_psi
				* ((r67.z * r67.z + r67.x * r67.x) * dsindF.y - r67.y * r67.z * dsindF.z - r67.y * r67.x * dsindF.x);
		f4.z += U_psi
				* ((r67.x * r67.x + r67.y * r67.y) * dsindF.z - r67.z * r67.x * dsindF.x - r67.z * r67.y * dsindF.y);

		f6 += cross(U_psi, dsindE, r67);

		f5.x += U_psi
				* (-(r67.y * r56.y + r67.z * r56.z) * dsindF.x + (2.0 * r67.x * r56.y - r56.x * r67.y) * dsindF.y
						+ (2.0 * r67.x * r56.z - r56.x * r67.z) * dsindF.z + dsindE.z * r78.y - dsindE.y * r78.z);
		f5.y += U_psi
				* (-(r67.z * r56.z + r67.x * r56.x) * dsindF.y + (2.0 * r67.y * r56.z - r56.y * r67.z) * dsindF.z
						+ (2.0 * r67.y * r56.x - r56.y * r67.x) * dsindF.x + dsindE.x * r78.z - dsindE.z * r78.x);
		f5.z += U_psi
				* (-(r67.x * r56.x + r67.y * r56.y) * dsindF.z + (2.0 * r67.z * r56.x - r56.z * r67.x) * dsindF.x
						+ (2.0 * r67.z * r56.y - r56.z * r67.y) * dsindF.y + dsindE.y * r78.x - dsindE.x * r78.y);
	}
	...
	if (pressureProfileData) {
		...
		pp_reduction(pressureProfileSlabs, n1, n2, p1, p2, pn, f1.x * r12.x, f1.y * r12.y, f1.z * r12.z,
				pressureProfileData);
		pp_reduction(pressureProfileSlabs, n2, n3, p2, p3, pn, f2.x * r23.x, f2.y * r23.y, f2.z * r23.z,
				pressureProfileData);
		pp_reduction(pressureProfileSlabs, n3, n4, p3, p4, pn, f3.x * r34.x, f3.y * r34.y, f3.z * r34.z,
				pressureProfileData);
		pp_reduction(pressureProfileSlabs, n5, n6, p5, p6, pn, f4.x * r56.x, f4.y * r56.y, f4.z * r56.z,
				pressureProfileData);
		pp_reduction(pressureProfileSlabs, n6, n7, p6, p7, pn, f5.x * r67.x, f5.y * r67.y, f5.z * r67.z,
				pressureProfileData);
		pp_reduction(pressureProfileSlabs, n7, n8, p7, p8, pn, f6.x * r78.x, f6.y * r78.y, f6.z * r78.z,
				pressureProfileData);
	}
...
void crossterm_setup(CrosstermData *table) {
	...
	for (i = 0; i < N; i++) {
		for (j = 0; j < N; j++) {
			...
			table[ij].d11 = h_2 * (1. / 3)
					* (table[ijp1p1].d00 + table[ijm1m1].d00 - table[ijp1m1].d00 - table[ijm1p1].d00
							- (1. / 16)
									* (table[ijp2p2].d00 + table[ijm2m2].d00 - table[ijp2m2].d00 - table[ijm2p2].d00));
		}
	}
