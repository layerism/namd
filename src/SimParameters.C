...
int atobool(const char *s) {
	return ((!strncasecmp(s, "yes", 8)) || (!strncasecmp(s, "on", 8)) || (!strncasecmp(s, "true", 8)));
}
;
...
#define SCRIPT_PARSE_STRING(NAME,VAR) { if ( ! strncasecmp(param,(NAME),MAX_SCRIPT_PARAM_SIZE) ) { strcpy(VAR,value); return; } }
	SCRIPT_PARSE_FLOAT("timestep", dt)
	...
	SCRIPT_PARSE_BOOL("langevinBAOAB", langevin_useBAOAB)
	// [!!] Use the BAOAB integrator or not
	SCRIPT_PARSE_FLOAT("loweAndersenTemp", loweAndersenTemp)
	// BEGIN LA, END LA
	...
	SCRIPT_PARSE_MOD_FLOAT("LangevinPistonTarget", langevinPistonTarget, /PRESSUREFACTOR)
	...
	SCRIPT_PARSE_MOD_FLOAT("SurfaceTensionTarget", surfaceTensionTarget, *(100.0/PRESSUREFACTOR))
	SCRIPT_PARSE_BOOL("BerendsenPressure", berendsenPressureOn)
	SCRIPT_PARSE_MOD_FLOAT("BerendsenPressureTarget", berendsenPressureTarget, /PRESSUREFACTOR)
	SCRIPT_PARSE_MOD_FLOAT("BerendsenPressureCompressibility", berendsenPressureCompressibility, *PRESSUREFACTOR)
	SCRIPT_PARSE_FLOAT("BerendsenPressureRelaxationTime", berendsenPressureRelaxationTime)
...
void SimParameters::config_parser_basic(ParseOptions &opts) {
	...
	////// basic options
	opts.require("main", "timestep", "size of the timestep, in fs", &dt, 1.0);
	opts.range("timestep", NOT_NEGATIVE);
	opts.units("timestep", N_FSEC);

	opts.optional("main", "numsteps", "number of timesteps to perform", &N, 0);
	opts.range("numsteps", NOT_NEGATIVE);

	opts.optional("main", "stepspercycle", "Number of steps between atom migrations", &stepsPerCycle, 20);
	opts.range("stepspercycle", POSITIVE);

	opts.require("main", "cutoff", "local electrostatic and Vdw distance", &cutoff);
	opts.range("cutoff", POSITIVE);
	opts.units("cutoff", N_ANGSTROM);

	opts.optional("main", "nonbondedScaling", "nonbonded scaling factor", &nonbondedScaling, 1.0);
	opts.range("nonbondedScaling", NOT_NEGATIVE);

	opts.optional("main", "limitDist", "limit nonbonded below this distance", &limitDist, 0.0);
	opts.range("limitDist", NOT_NEGATIVE);

	opts.require("main", "exclude", "Electrostatic and VDW exclusion policy",
	PARSE_STRING);

	opts.optional("exclude", "1-4scaling", "1-4 electrostatic scaling factor", &scale14, 1.0);
	opts.range("1-4scaling", POSITIVE);

	opts.optionalB("main", "switching", "Should a smoothing function be used?", &switchingActive, TRUE);

	opts.optionalB("switching", "vdwForceSwitching", "Use force switching for vdw?", &vdwForceSwitching,
	FALSE);

	opts.optional("switching", "switchdist", "Distance for switching function activation", &switchingDist);
	opts.range("switchdist", POSITIVE);
	opts.units("switchdist", N_ANGSTROM);

	opts.optionalB("main", "martiniSwitching", "Use Martini residue-based coarse-grain switching?", &martiniSwitching,
	FALSE);
	opts.optionalB("main", "martiniDielAllow", "Allow use of dielectric != 15.0 when using Martini", &martiniDielAllow,
	FALSE);

	opts.optional("main", "pairlistdist", "Pairlist inclusion distance", &pairlistDist);
	opts.range("pairlistdist", POSITIVE);
	opts.units("pairlistdist", N_ANGSTROM);

	opts.optional("main", "pairlistMinProcs", "Min procs for pairlists", &pairlistMinProcs, 1);
	opts.range("pairlistMinProcs", POSITIVE);

	opts.optional("main", "pairlistsPerCycle", "regenerate x times per cycle", &pairlistsPerCycle, 2);
	opts.range("pairlistsPerCycle", POSITIVE);

	opts.optional("main", "outputPairlists", "how often to print warnings", &outputPairlists, 0);
	opts.range("outputPairlists", NOT_NEGATIVE);

	opts.optional("main", "pairlistShrink", "tol *= (1 - x) on regeneration", &pairlistShrink, 0.01);
	opts.range("pairlistShrink", NOT_NEGATIVE);

	opts.optional("main", "pairlistGrow", "tol *= (1 + x) on trigger", &pairlistGrow, 0.01);
	opts.range("pairlistGrow", NOT_NEGATIVE);

	opts.optional("main", "pairlistTrigger", "trigger is atom > (1 - x) * tol", &pairlistTrigger, 0.3);
	opts.range("pairlistTrigger", NOT_NEGATIVE);

	opts.optional("main", "temperature", "initial temperature", &initialTemp);
	opts.range("temperature", NOT_NEGATIVE);
	opts.units("temperature", N_KELVIN);

	opts.optionalB("main", "COMmotion", "allow initial center of mass movement", &comMove, FALSE);

	opts.optionalB("main", "zeroMomentum", "constrain center of mass", &zeroMomentum, FALSE);
	opts.optionalB("zeroMomentum", "zeroMomentumAlt", "constrain center of mass", &zeroMomentumAlt,
	FALSE);

	opts.optionalB("main", "wrapWater", "wrap waters around periodic boundaries on output", &wrapWater,
	FALSE);
	opts.optionalB("main", "wrapAll", "wrap all clusters around periodic boundaries on output", &wrapAll,
	FALSE);
	opts.optionalB("main", "wrapNearest", "wrap to nearest image to cell origin", &wrapNearest, FALSE);

	opts.optional("main", "dielectric", "dielectric constant", &dielectric, 1.0);
	...
	opts.optional("main", "outputEnergies", "How often to print energies in timesteps", &outputEnergies, 1);
	opts.range("outputEnergies", POSITIVE);

	opts.optional("main", "outputMomenta", "How often to print linear and angular momenta in timesteps", &outputMomenta,
			0);
	opts.range("outputMomenta", NOT_NEGATIVE);

	opts.optional("main", "outputTiming", "How often to print timing data in timesteps", &outputTiming);
	opts.range("outputTiming", NOT_NEGATIVE);

	opts.optional("main", "outputCudaTiming", "How often to print CUDA timing data in timesteps", &outputCudaTiming, 0);
	opts.range("outputCudaTiming", NOT_NEGATIVE);

	opts.optional("main", "outputPressure", "How often to print pressure data in timesteps", &outputPressure, 0);
	opts.range("outputPressure", NOT_NEGATIVE);

	opts.optionalB("main", "mergeCrossterms", "merge crossterm energy with dihedral when printing?", &mergeCrossterms,
	TRUE);
	...
	opts.optionalB("main", "ignoreMass", "Do not use masses to find hydrogen atoms", &ignoreMass, FALSE);
	...
	opts.optional("main", "extendedSystem", "Initial configuration of extended system variables and periodic cell",
	PARSE_STRING);

	opts.optional("main", "cellBasisVector1", "Basis vector for periodic cell", &cellBasisVector1);
	opts.optional("main", "cellBasisVector2", "Basis vector for periodic cell", &cellBasisVector2);
	opts.optional("main", "cellBasisVector3", "Basis vector for periodic cell", &cellBasisVector3);
	opts.optional("main", "cellOrigin", "Fixed center of periodic cell", &cellOrigin);

	opts.optionalB("main", "molly", "Rigid bonds to hydrogen", &mollyOn, FALSE);
	opts.optional("main", "mollyTolerance", "Error tolerance for MOLLY", &mollyTol, 0.00001);
	opts.optional("main", "mollyIterations", "Max number of iterations for MOLLY", &mollyIter, 100);

	opts.optional("main", "rigidBonds", "Rigid bonds to hydrogen", PARSE_STRING);
	opts.optional("main", "rigidTolerance", "Error tolerance for rigid bonds to hydrogen", &rigidTol, 1.0e-8);
	opts.optional("main", "rigidIterations", "Max number of SHAKE iterations for rigid bonds to hydrogen", &rigidIter,
			100);
	opts.optionalB("main", "rigidDieOnError", "Die if rigidTolerance is not achieved after rigidIterations", &rigidDie,
	TRUE);
	opts.optionalB("main", "useSettle", "Use the SETTLE algorithm for rigid waters", &useSettle, TRUE);

	opts.optional("main", "nonbondedFreq", "Nonbonded evaluation frequency", &nonbondedFrequency, 1);
	opts.range("nonbondedFreq", POSITIVE);

	opts.optionalB("main", "outputPatchDetails", "print number of atoms in each patch", &outputPatchDetails,
	FALSE);
	opts.optionalB("main", "staticAtomAssignment", "never migrate atoms", &staticAtomAssignment, FALSE);
	...
	opts.optionalB("main", "LJcorrection", "Apply analytical tail corrections for energy and virial", &LJcorrection,
	FALSE);
}
...
//    PARSE_STRING);
	...
	opts.optional("main", "velocities", "initial velocities, given as a PDB file", PARSE_STRING);
	opts.optional("main", "binvelocities", "initial velocities, given as a binary restart", PARSE_STRING);
	opts.optional("main", "bincoordinates", "initial coordinates in a binary restart file", PARSE_STRING);
...
//"inputs allowed)", PARSE_MULTIPLES);
	opts.optional("main", "parameters", "CHARMm 19 or CHARMm 22 compatable force field file (multiple "
			"inputs allowed)", PARSE_MULTIPLES);

	//****** BEGIN CHARMM/XPLOR type changes
	//// enable XPLOR as well as CHARMM input files for parameters
	opts.optionalB("parameters", "paraTypeXplor", "Parameter file in Xplor format?", &paraTypeXplorOn,
	FALSE);
	opts.optionalB("parameters", "paraTypeCharmm", "Parameter file in Charmm format?", &paraTypeCharmmOn,
	FALSE);
	...
	opts.require("GoForcesOn", "GoMethod", "Which type of matrix should be used to store Go contacts?",
	PARSE_STRING);
	// End of Port -- JL

	opts.require("main", "outputname", "prefix for the final PDB position and velocity filenames", outputFilename);

	opts.optional("main", "auxFile", "Filename for data stream output", auxFilename);

	opts.optional("main", "numinputprocs", "Number of pes to use for parallel input", &numinputprocs, 0);
	opts.range("numinputprocs", NOT_NEGATIVE);

	opts.optional("main", "numoutputprocs", "Number of pes to use for parallel output", &numoutputprocs, 0);
	...
	opts.optional("DCDfreq", "DCDfile", "DCD trajectory output file name", dcdFilename);
	opts.optionalB("DCDfreq", "DCDunitcell", "Store unit cell in dcd timesteps?", &dcdUnitCell);
	...
	opts.optional("velDCDfreq", "velDCDfile", "velocity DCD output file name", velDcdFilename);
	...
	opts.optional("forceDCDfreq", "forceDCDfile", "force DCD output file name", forceDcdFilename);
	...
	opts.optionalB("restartfreq", "binaryrestart", "Specify use of binary restart files ", &binaryRestart,
	TRUE);

	opts.optionalB("outputname", "binaryoutput", "Specify use of binary output files ", &binaryOutput,
	TRUE);

	opts.optionalB("main", "amber", "Is it AMBER force field?", &amberOn, FALSE);
	opts.optionalB("amber", "readexclusions", "Read exclusions from parm file?", &readExclusions, TRUE);
	opts.require("amber", "scnb", "1-4 VDW interactions are divided by scnb", &vdwscale14, 2.0);
...
// REDUNDANT TI ENDS
	...
	/* GROMACS options */
	opts.optionalB("main", "gromacs", "Use GROMACS-like force field?", &gromacsOn, FALSE);
	...
	// OPLS options
	opts.optionalB("main", "vdwGeometricSigma", "Use geometric mean to combine L-J sigmas, as for OPLS",
			&vdwGeometricSigma, FALSE);

	// load/store computeMap
	opts.optional("main", "computeMapFile", "Filename for computeMap", computeMapFilename);
	opts.optionalB("main", "storeComputeMap", "store computeMap?", &storeComputeMap, FALSE);
	opts.optionalB("main", "loadComputeMap", "load computeMap?", &loadComputeMap, FALSE);
}
...
void SimParameters::config_parser_fullelect(ParseOptions &opts) {
	...
	opts.optional("main", "fullElectFrequency", "Number of steps between full electrostatic executions",
			&fullElectFrequency);
	opts.range("fullElectFrequency", POSITIVE);

	//  USE OF THIS PARAMETER DISCOURAGED
	opts.optional("main", "fmaFrequency", "Number of steps between full electrostatic executions", &fmaFrequency);
	opts.range("fmaFrequency", POSITIVE);

	opts.optional("main", "fmaTheta", "FMA theta parameter value", &fmaTheta, 0.715);
	...
	opts.optionalB("main", "MSM", "Use multilevel summation method for electrostatics?", &MSMOn, FALSE);
	opts.optional("MSM", "MSMQuality", "MSM quality", &MSMQuality, 0);
	opts.optional("MSM", "MSMApprox", "MSM approximation", &MSMApprox, 0);
	opts.optional("MSM", "MSMSplit", "MSM splitting", &MSMSplit, 0);
	opts.optional("MSM", "MSMLevels", "MSM maximum number of levels", &MSMLevels, 0); // set to 0 adapts to as many as needed
	opts.optional("MSM", "MSMGridSpacing", "MSM grid spacing (Angstroms)", &MSMGridSpacing, 2.5);
	opts.optional("MSM", "MSMPadding", "MSM padding (Angstroms)", &MSMPadding, 2.5);
	opts.optional("MSM", "MSMBlockSizeX", "MSM grid block size along X direction (for decomposing parallel work)",
			&MSMBlockSizeX, 8);
	opts.optional("MSM", "MSMBlockSizeY", "MSM grid block size along Y direction (for decomposing parallel work)",
			&MSMBlockSizeY, 8);
	opts.optional("MSM", "MSMBlockSizeZ", "MSM grid block size along Z direction (for decomposing parallel work)",
			&MSMBlockSizeZ, 8);

	opts.optionalB("MSM", "MsmSerial", "Use MSM serial version for long-range calculation?", &MsmSerialOn,
	FALSE);

	///////////  Fast Multipole Method

	opts.optionalB("main", "FMM", "Use fast multipole method for electrostatics?", &FMMOn, FALSE);
	opts.optional("FMM", "FMMLevels", "FMM number of levels", &FMMLevels, 0);
	opts.optional("FMM", "FMMPadding", "FMM padding margin (Angstroms)", &FMMPadding, 0);

	///////////  Particle Mesh Ewald

	opts.optionalB("main", "PME", "Use particle mesh Ewald for electrostatics?", &PMEOn, FALSE);
	opts.optional("PME", "PMETolerance", "PME direct space tolerance", &PMETolerance, 1.e-6);
	opts.optional("PME", "PMEInterpOrder", "PME interpolation order", &PMEInterpOrder, 4); // cubic interpolation is default
	opts.optional("PME", "PMEGridSizeX", "PME grid in x dimension", &PMEGridSizeX, 0);
	opts.optional("PME", "PMEGridSizeY", "PME grid in y dimension", &PMEGridSizeY, 0);
	opts.optional("PME", "PMEGridSizeZ", "PME grid in z dimension", &PMEGridSizeZ, 0);
	opts.optional("PME", "PMEGridSpacing", "Maximum PME grid spacing (Angstroms)", &PMEGridSpacing, 0.);
	opts.range("PMEGridSpacing", NOT_NEGATIVE);
	opts.optional("PME", "PMEProcessors", "PME FFT and reciprocal sum processor count", &PMEProcessors, 0);
	opts.optional("PME", "PMEMinSlices", "minimum thickness of PME reciprocal sum slab", &PMEMinSlices, 2);
	opts.range("PMEMinSlices", NOT_NEGATIVE);
	opts.optional("PME", "PMEPencils", "PME FFT and reciprocal sum pencil grid size", &PMEPencils, -1);
	opts.optional("PME", "PMEPencilsX", "PME FFT and reciprocal sum pencil grid size X", &PMEPencilsX, 0);
	opts.optional("PME", "PMEPencilsY", "PME FFT and reciprocal sum pencil grid size Y", &PMEPencilsY, 0);
	opts.optional("PME", "PMEPencilsZ", "PME FFT and reciprocal sum pencil grid size Z", &PMEPencilsZ, 0);
	opts.range("PMEPencilsX", NOT_NEGATIVE);
	opts.range("PMEPencilsY", NOT_NEGATIVE);
	opts.range("PMEPencilsZ", NOT_NEGATIVE);
	opts.optional("PME", "PMEPencilsYLayout", "PME FFT and reciprocal sum Y pencil layout strategy", &PMEPencilsYLayout,
			0);
	opts.optional("PME", "PMEPencilsXLayout", "PME FFT and reciprocal sum X pencil layout strategy", &PMEPencilsXLayout,
			0);
	opts.range("PMEPencilsYLayout", NOT_NEGATIVE);
	opts.range("PMEPencilsXLayout", NOT_NEGATIVE);
	opts.optional("PME", "PMESendOrder", "PME message ordering control", &PMESendOrder, 0);
	opts.range("PMESendOrder", NOT_NEGATIVE);
	opts.optional("PME", "PMEMinPoints", "minimum points per PME reciprocal sum pencil", &PMEMinPoints, 10000);
	opts.range("PMEMinPoints", NOT_NEGATIVE);
	opts.optionalB("main", "PMEBarrier", "Use barrier in PME?", &PMEBarrier, FALSE);

	opts.optionalB("PME", "useOptPME", "Use the new scalable PME optimization", &useOptPME, FALSE);
	opts.optionalB("PME", "useManyToMany", "Use the many-to-many PME optimization", &useManyToMany,
	FALSE);
	if (PMEOn && !useOptPME)
		useManyToMany = false;
	...
	opts.optionalB("main", "FFTWUseWisdom", "Read/save wisdom file for FFTW?",
			...
			&FFTWUseWisdom, TRUE);
#endif
	opts.optional("FFTWUseWisdom", "FFTWWisdomFile", "File for FFTW wisdom", FFTWWisdomFile);
...
void SimParameters::config_parser_methods(ParseOptions &opts) {
	...
	opts.optionalB("main", "minimization", "Should minimization be performed?", &minimizeCGOn, FALSE);
	opts.optionalB("main", "minVerbose", "Print extra minimization diagnostics?", &minVerbose, FALSE);
	opts.optional("main", "minTinyStep", "very first minimization steps", &minTinyStep, 1.0e-6);
	opts.range("minTinyStep", POSITIVE);
	opts.optional("main", "minBabyStep", "initial minimization steps", &minBabyStep, 1.0e-2);
	opts.range("minBabyStep", POSITIVE);
	opts.optional("main", "minLineGoal", "line minimization gradient reduction", &minLineGoal, 1.0e-3);
	opts.range("minLineGoal", POSITIVE);

	opts.optionalB("main", "velocityQuenching", "Should old-style minimization be performed?", &minimizeOn,
	FALSE);
	...
	opts.optionalB("main", "Langevin", "Should Langevin dynamics be performed?", &langevinOn, FALSE);
	...
	opts.optional("Langevin", "langevinDamping", "Damping coefficient (1/ps)", &langevinDamping);
	...
	opts.optionalB("Langevin", "langevinBAOAB", "Should Langevin dynamics be performed using BAOAB integration?",
			&langevin_useBAOAB, FALSE);

// BEGIN LA
	opts.optionalB("main", "LoweAndersen", "Should Lowe-Andersen dynamics be performed?", &loweAndersenOn,
	FALSE);
	...
	opts.optional("LoweAndersen", "loweAndersenRate", "Collision rate (1/ps)", &loweAndersenRate, 50);
	opts.range("loweAndersenRate", POSITIVE);
	opts.optional("LoweAndersen", "loweAndersenCutoff", "Cutoff radius", &loweAndersenCutoff, 2.7);
...
//  alchemical fep options
	opts.optionalB("main", "alch", "Is achemical simulation being performed?", &alchOn, FALSE);
	opts.optional("alch", "alchType", "Which alchemical method to use?",
	PARSE_STRING);
	opts.optionalB("alch", "alchFepWCARepuOn", "WCA decomposition repu interaction in use?", &alchFepWCARepuOn, FALSE);
	opts.optionalB("alch", "alchFepWCADispOn", "WCA decomposition disp interaction in use?", &alchFepWCADispOn, FALSE);
	opts.optionalB("alch", "alchEnsembleAvg", "Ensemble Average in use?", &alchEnsembleAvg, TRUE);
	opts.optionalB("alch", "alchFepWhamOn", "Energy output for Wham postprocessing in use?", &alchFepWhamOn,
	FALSE);
...
// REDUNDANT TI ENDS
	...
	opts.optionalB("main", "alchDecouple", "Enable alchemical decoupling?", &alchDecouple, FALSE);

	opts.optionalB("main", "les", "Is locally enhanced sampling enabled?", &lesOn, FALSE);
	...
	opts.optionalB("les", "lesReduceTemp", "Reduce enhanced atom temperature?", &lesReduceTemp, FALSE);
	opts.optionalB("les", "lesReduceMass", "Reduce enhanced atom mass?", &lesReduceMass, FALSE);

	// Drude oscillators
	opts.optionalB("main", "drude", "Perform integration of Drude oscillators?", &drudeOn, FALSE);
	...
	opts.optionalB("main", "pairInteraction", "Are pair interactions calculated?", &pairInteractionOn,
	FALSE);
	...
	opts.optional("pairInteraction", "pairInteractionCol",
			...
			PARSE_STRING);
	opts.require("pairInteraction", "pairInteractionGroup1", "Flag for interaction group 1", &pairInteractionGroup1);
	opts.optional("pairInteraction", "pairInteractionGroup2", "Flag for interaction group 2", &pairInteractionGroup2,
			-1);
	opts.optionalB("pairInteraction", "pairInteractionSelf", "Compute only within-group interactions?",
			&pairInteractionSelf,
			FALSE);
	...
	opts.optionalB("main", "globalTest", "Should global integration (for development) be used?", &globalOn,
	FALSE);
	opts.optionalB("main", "dihedral", "Should dihedral angle dynamics be performed?", &dihedralOn,
	FALSE);
	COLDOn = FALSE;
	opts.optionalB("dihedral", "COLD", "Should overdamped Langevin dynamics be performed?", &COLDOn,
	FALSE);
	opts.require("COLD", "COLDTemp", "Temperature for heat bath in COLD", &COLDTemp);
	opts.range("COLDTemp", NOT_NEGATIVE);
	opts.units("COLDTemp", N_KELVIN);
	opts.require("COLD", "COLDRate", "Damping rate for COLD", &COLDRate, 3000.0);
	opts.range("COLDRate", NOT_NEGATIVE);

	//  Get the parameters for temperature coupling
	opts.optionalB("main", "tcouple", "Should temperature coupling be performed?", &tCoupleOn, FALSE);
	opts.require("tcouple", "tCoupleTemp", "Temperature for temperature coupling", &tCoupleTemp);
	...
	opts.optional("main", "rescaleTemp", "Target temperature for velocity rescaling", &rescaleTemp);
	...
	opts.optional("main", "reassignTemp", "Target temperature for velocity reassignment", &reassignTemp);
	opts.range("reassignTemp", NOT_NEGATIVE);
	opts.units("reassignTemp", N_KELVIN);
	opts.optional("main", "reassignIncr", "Temperature increment for velocity reassignment", &reassignIncr);
	opts.units("reassignIncr", N_KELVIN);
	opts.optional("main", "reassignHold", "Final holding temperature for velocity reassignment", &reassignHold);
	opts.range("reassignHold", NOT_NEGATIVE);
	opts.units("reassignHold", N_KELVIN);

	////  Group rather than atomic pressure
	opts.optionalB("main", "useGroupPressure", "Use group rather than atomic quantities for pressure control?",
			&useGroupPressure, FALSE);

	////  Anisotropic cell fluctuations
	opts.optionalB("main", "useFlexibleCell", "Use anisotropic cell fluctuation for pressure control?",
			&useFlexibleCell, FALSE);

	// Fix specific cell dimensions
	opts.optionalB("main", "fixCellDims", "Fix some cell dimensions?", &fixCellDims, FALSE);

	opts.optionalB("fixCellDims", "fixCellDimX", "Fix the X dimension?", &fixCellDimX, FALSE);
	opts.optionalB("fixCellDims", "fixCellDimY", "Fix the Y dimension?", &fixCellDimY, FALSE);
	opts.optionalB("fixCellDims", "fixCellDimZ", "Fix the Z dimension?", &fixCellDimZ, FALSE);

	////  Constant dimension ratio in X-Y plane
	opts.optionalB("main", "useConstantRatio", "Use constant X-Y ratio for pressure control?", &useConstantRatio,
	FALSE);

	////  Constant area and normal pressure conditions
	opts.optionalB("main", "useConstantArea", "Use constant area for pressure control?", &useConstantArea,
	FALSE);

	//// Exclude atoms from pressure
	opts.optionalB("main", "excludeFromPressure", "Should some atoms be excluded from pressure rescaling?",
			&excludeFromPressure, FALSE);
	opts.optional("excludeFromPressure", "excludeFromPressureFile", "PDB file for atoms to be excluded from pressure",
	PARSE_STRING);
	opts.optional("excludeFromPressure", "excludeFromPressureCol", "Column in the excludeFromPressureFile"
			"containing the flags (nonzero means excluded);\n"
			"default is 'O'", PARSE_STRING);

	////  Berendsen pressure bath coupling
	opts.optionalB("main", "BerendsenPressure", "Should Berendsen pressure bath coupling be performed?",
			&berendsenPressureOn, FALSE);
	opts.require("BerendsenPressure", "BerendsenPressureTarget", "Target pressure for pressure coupling",
			&berendsenPressureTarget);
	// opts.units("BerendsenPressureTarget",);
	opts.require("BerendsenPressure", "BerendsenPressureCompressibility",
			"Isothermal compressibility for pressure coupling", &berendsenPressureCompressibility);
	// opts.units("BerendsenPressureCompressibility",);
	opts.require("BerendsenPressure", "BerendsenPressureRelaxationTime", "Relaxation time for pressure coupling",
	...
	opts.optional("BerendsenPressure", "BerendsenPressureFreq", "Number of steps between volume rescaling",
			&berendsenPressureFreq, 1);
	opts.range("BerendsenPressureFreq", POSITIVE);

	////  Langevin Piston pressure control
	opts.optionalB("main", "LangevinPiston", "Should Langevin piston pressure control be used?", &langevinPistonOn,
	FALSE);
	opts.optionalB("LangevinPiston", "LangevinPistonBarrier", "Should Langevin piston barrier be used?",
			&langevinPistonBarrier, TRUE);
	opts.require("LangevinPiston", "LangevinPistonTarget", "Target pressure for pressure control",
			&langevinPistonTarget);
	opts.require("LangevinPiston", "LangevinPistonPeriod", "Oscillation period for pressure control",
			&langevinPistonPeriod);
	opts.range("LangevinPistonPeriod", POSITIVE);
	opts.units("LangevinPistonPeriod", N_FSEC);
	opts.require("LangevinPiston", "LangevinPistonDecay", "Decay time for pressure control", &langevinPistonDecay);
	opts.range("LangevinPistonDecay", POSITIVE);
	opts.units("LangevinPistonDecay", N_FSEC);
	opts.require("LangevinPiston", "LangevinPistonTemp", "Temperature for pressure control piston",
			&langevinPistonTemp);
	opts.range("LangevinPistonTemp", POSITIVE);
	opts.units("LangevinPistonTemp", N_KELVIN);
	opts.optional("LangevinPiston", "StrainRate", "Initial strain rate for pressure control (x y z)", &strainRate);

	//// Surface tension
	opts.optional("main", "SurfaceTensionTarget", "Surface tension in the x-y plane", &surfaceTensionTarget, 0);

	//// Pressure Profile calculations
	opts.optionalB("main", "pressureprofile", "Compute pressure profile?", &pressureProfileOn, FALSE);
	opts.require("pressureprofile", "pressureprofileslabs", "Number of pressure profile slabs", &pressureProfileSlabs,
			10);
	opts.optional("pressureprofile", "pressureprofilefreq", "How often to store profile data", &pressureProfileFreq, 1);
	opts.optional("pressureprofile", "pressureProfileAtomTypes", "Number of pressure profile atom types",
	...
	opts.optional("pressureProfile", "pressureProfileAtomTypesCol",
			...
			PARSE_STRING);
	opts.optionalB("pressureProfile", "pressureProfileEwald", "Compute Ewald contribution to pressure profile",
			&pressureProfileEwaldOn, FALSE);
	opts.optional("pressureProfile", "pressureProfileEwaldX", "Ewald grid size X", &pressureProfileEwaldX, 10);
	opts.range("pressureProfileEwaldX", POSITIVE);
	opts.optional("pressureProfile", "pressureProfileEwaldY", "Ewald grid size Y", &pressureProfileEwaldY, 10);
	opts.range("pressureProfileEwaldY", POSITIVE);
	opts.optional("pressureProfile", "pressureProfileEwaldZ", "Ewald grid size Z", &pressureProfileEwaldZ, 10);
	...
	//lyk
	//ITS
	opts.optionalB("main", "its", "Perform ITS sampling", &itsOn, FALSE);
	opts.require("its", "itsFile", "PDB files with interaction flags", PARSE_STRING);
	opts.optional("its", "itsCol", "Column with the interaction flags", PARSE_STRING);
	opts.optional("its", "itsFirstStep", "First ITS step", &itsFirstStep, 0);
	opts.range("itsFirstStep", NOT_NEGATIVE);
	opts.optional("its", "itsLastStep", "Last ITS step", &itsLastStep, 9999999);
	opts.range("itsLastStep", NOT_NEGATIVE);
	opts.optional("its", "itsOutFreq", "Frequency of ITS output", &itsOutFreq, 1);
	opts.range("itsOutFreq", POSITIVE);
	opts.optional("its", "itsWhamTimes", "WHAM times", &itsWhamTimes, 100);
	opts.range("itsWhamTimes", NOT_NEGATIVE);
	opts.require("its", "itsReplicaNum", "itsReplicaNum", &itsReplicaNum, 10);
	opts.range("itsReplicaNum", POSITIVE);

	opts.require("its", "groupIdx1", "Flag for 1 group", &group1, 1);
	opts.require("its", "groupIdx2", "Flag for 2 group", &group2, 2);

	opts.optional("its", "itsLbdMin", "ITS LbdMin", &itsLbdMin, 0.0);
	opts.optional("its", "itsLbdMax", "ITS LbdMax", &itsLbdMax, 1.0);
	opts.require("its", "itsTemp", "ITS TEMP", &itsTemp, 300.0);
	opts.range("itsTemp", NOT_NEGATIVE);

	opts.optional("its", "itsInFile", "FILE for read", itsInFile);
	opts.optional("its", "itsUpdateLog", "itsUpdateLog", itsUpdateLog);
	opts.optionalB("its", "itsUpdate", " readfile?", &itsUpdateOn, FALSE);
	opts.optional("its", "itsUpdateScaling", "itsUpdateScaling", &itsUpdateScaling, 0.5);
	opts.range("itsUpdateScaling", NOT_NEGATIVE);

	opts.optional("its", "itsUpdateFreq", "ITS update freq", &itsUpdateFreq, 5000);
	opts.range("itsUpdateFreq", NOT_NEGATIVE);
	opts.optional("its", "itsSamplingFreq", "ITS Sampling Freq", &itsSamplingFreq, 10);
	opts.range("itsSamplingFreq", NOT_NEGATIVE);

	opts.optionalB("its", "onlyInt", " onlyIntaction?", &onlyIntOn, FALSE);
	opts.require("its", "itsIntType", "itsIntType", itsIntType);

	// ACG
	opts.optionalB("main", "adaptCg", " adaptCg?", &adaptCgOn, FALSE);
	opts.require("adaptCg", "aaGroup", "Flag for aa group", &aaGroup, 1);
	opts.require("adaptCg", "cgGroup", "Flag for cg group", &cgGroup, 2);
	opts.require("adaptCg", "adaptCgFile", "PDB files with interaction flags", PARSE_STRING);
	opts.optional("adaptCg", "adaptCgCol", "Column with the interaction flags", PARSE_STRING);
	opts.optional("adaptCg", "adaptCgFirstStep", "First ACG step", &adaptCgFirstStep, 0);
	opts.range("adaptCgFirstStep", NOT_NEGATIVE);
	opts.optional("adaptCg", "adaptCgLastStep", "Last ACG step", &adaptCgLastStep, 0);
	opts.range("adaptCgLastStep", NOT_NEGATIVE);
	opts.optional("adaptCg", "adaptCgWhamTimes", "WHAM times", &adaptCgWhamTimes, 1000);
	opts.range("adaptCgWhamTimes", NOT_NEGATIVE);

	opts.optional("adaptCg", "adaptCgUpdateFreq", "Frequency of ACG update", &adaptCgUpdateFreq, 1000);
	opts.optional("adaptCg", "adaptCgSamplingFreq", "Frequency of ACG sampling", &adaptCgSamplingFreq, 10);

	opts.optional("adaptCg", "adaptCgLbdMin", "ACG adaptCgLbdMin", &adaptCgLbdMin, 0.0);
	opts.optional("adaptCg", "adaptCgLbdMax", "ACG adaptCgLbdMax", &adaptCgLbdMax, 1.0);
	opts.require("adaptCg", "adaptCgTemp", "adaptCg TEMP", &adaptCgTemp, 300.0);
	opts.range("adaptCgTemp", NOT_NEGATIVE);

	opts.optional("adaptCg", "adaptCgUpdateScaling", "adaptCgUpdateScaling", &adaptCgUpdateScaling, 0.5);
	opts.range("adaptCgUpdateScaling", NOT_NEGATIVE);

	opts.optional("adaptCg", "adaptCgReplicaNum", "adaptCgReplicaNum", &adaptCgReplicaNum, 10);
	opts.range("adaptCgReplicaNum", POSITIVE);

	opts.optional("adaptCg", "adaptCgCvMin", "adaptCgCvMin", &adaptCgCvMin, 0.0);
	opts.optional("adaptCg", "adaptCgCvMax", "adaptCgCvMax", &adaptCgCvMax, 0.0);

	opts.optional("adaptCg", "adaptCgInFile", "FILE for read", adaptCgInFile);
	opts.optional("adaptCg", "adaptCgUpdateLog", "adaptCgUpdateLog", adaptCgUpdateLog);
	opts.require("adaptCg", "adaptCgIntType", "FILE for adaptCgIntType", adaptCgIntType);

	opts.optionalB("adaptCg", "adaptCgUpdate", " readfile?", &adaptCgUpdateOn, FALSE);
	opts.optional("adaptCg", "adaptCgOutFreq", "Frequency of ACG output", &adaptCgOutFreq, 1);
	opts.range("adaptCgOutFreq", POSITIVE);

	opts.optional("adaptCg", "cgDielectric", "cgDielectric", &cgDielectric, 15.0);
	opts.range("cgDielectric", NOT_NEGATIVE);
	opts.optional("adaptCg", "cgVdwCappedN", "cgVdwCappedN", &cgVdwCappedN, 6.0);
	opts.range("cgVdwCappedN", POSITIVE);
	opts.optional("adaptCg", "cgVdwCappedSwitchF", "cgVdwCappedSwitchF", &cgVdwCappedSwitchF, 1.0);
	opts.range("cgVdwCappedSwitchF", POSITIVE);
	opts.optional("adaptCg", "cgScaling", "cgScaling", &cgScaling, 1.0);
	opts.range("cgScaling", NOT_NEGATIVE);

	//common for its and acg
	opts.optionalB("main", "useDihed", "useDihed", &useDihedOn, FALSE);
	opts.optional("main", "DENSITYALAPHA", "DENSITYALAPHA", &DENSITYALAPHA, 2.0);
	opts.range("DENSITYALAPHA", POSITIVE);
	opts.optional("main", "WCAP", "WCAP", &WCAP, 2.0);
	opts.range("WCAP", NOT_NEGATIVE);
	opts.optional("main", "WHAMSKIP", "WHAMSKIP", &WHAMSKIP, 1);
	opts.range("WHAMSKIP", NOT_NEGATIVE);
	opts.optional("main", "WHAMLOGZERROR", "WHAMLOGZERROR", &WHAMLOGZERROR, 1.0);
	opts.range("WHAMLOGZERROR", NOT_NEGATIVE);
	...
	opts.optional("adaptTempMD", "adaptTempFreq", "Frequency of writing average energies to adaptTempOutFile",
			&adaptTempFreq, 10);
	opts.range("adaptTempFreq", POSITIVE);
	opts.optionalB("adaptTempMD", "adaptTempDebug", "Print debug output for adaptTemp", &adaptTempDebug,
	FALSE);
	...
	opts.optional("adaptTempMD", "adaptTempAutoDt", "Average temperature update in percent of temperature range",
			&adaptTempAutoDt, 0.0);
	...
	opts.optionalB("adaptTempMD", "adaptTempLangevin", "Send adaptTemp temperature to langevin thermostat",
			&adaptTempLangevin, TRUE);
	opts.optionalB("adaptTempMD", "adaptTempRescaling", "Send adaptTemp temperature to velocity rescaling thermostat",
			&adaptTempRescale, TRUE);
	opts.optional("adaptTempMD", "adaptTempInFile", "File containing restart information for adaptTemp",
			adaptTempInFile);
	opts.optional("adaptTempMD", "adaptTempRestartFile", "File for writing adaptTemp restart information",
			adaptTempRestartFile);
	opts.require("adaptTempRestartFile", "adaptTempRestartFreq", "Frequency of writing restart file",
			&adaptTempRestartFreq, 0);
	opts.range("adaptTempRestartFreq", NOT_NEGATIVE);
	opts.optionalB("adaptTempMD", "adaptTempRandom", "Randomly assign a temperature if we step out of range",
			&adaptTempRandom, FALSE);
}

void SimParameters::config_parser_constraints(ParseOptions &opts) {

	////  Fixed Atoms
	opts.optionalB("main", "fixedatoms", "Are there fixed atoms?", &fixedAtomsOn, FALSE);
	opts.optionalB("fixedatoms", "fixedAtomsForces",
			"Calculate forces between fixed atoms?  (Required to unfix during run.)", &fixedAtomsForces,
			FALSE);
	...
	opts.optionalB("main", "constraints", "Are harmonic constraints active?", &constraintsOn, FALSE);
	opts.require("constraints", "consexp", "Exponent for harmonic potential", &constraintExp, 2);
	...
	opts.require("constraints", "consAtomListFile", "the text input file for constrained atoms "
			"used for parallel input IO", PARSE_STRING);
#endif
	opts.require("constraints", "constraintScaling", "constraint scaling factor", &constraintScaling, 1.0);
	...
	opts.optionalB("constraints", "selectConstraints",
			"Restrain only selected Cartesian components of the coordinates?", &selectConstraintsOn,
			FALSE);
	opts.optionalB("selectConstraints", "selectConstrX", "Restrain X components of coordinates ", &constrXOn,
	FALSE);
	opts.optionalB("selectConstraints", "selectConstrY", "Restrain Y components of coordinates ", &constrYOn,
	FALSE);
	opts.optionalB("selectConstraints", "selectConstrZ", "Restrain Z components of coordinates ", &constrZOn,
	FALSE);
	//****** END selective restraints (X,Y,Z) changes

	// spherical constraints
	opts.optionalB("constraints", "sphericalConstraints",
			"Restrain only radial spherical component of the coordinates?", &sphericalConstraintsOn,
			FALSE);
	opts.optional("sphericalConstraints", "sphericalConstrCenter", "Center of spherical constraints",
			&sphericalConstrCenter);

	//****** BEGIN moving constraints changes

	//// Moving Harmonic Constraints
	opts.optionalB("constraints", "movingConstraints", "Are some of the constraints moving?", &movingConstraintsOn,
	FALSE);
	opts.require("movingConstraints", "movingConsVel", "Velocity of the movement, A/timestep", &movingConsVel);
	//****** END moving constraints changes

	// BEGIN rotating constraints changes
	opts.optionalB("constraints", "rotConstraints", "Are the constraints rotating?", &rotConstraintsOn,
	FALSE);
	opts.require("rotConstraints", "rotConsAxis", "Axis of rotation", &rotConsAxis);
	opts.require("rotConstraints", "rotConsPivot", "Pivot point of rotation", &rotConsPivot);
	opts.require("rotConstraints", "rotConsVel", "Velocity of rotation, deg/timestep", &rotConsVel);

	// END rotating constraints changes

	// external command forces
	opts.optionalB("main", "extForces", "External command forces?", &extForcesOn, FALSE);
	opts.require("extForces", "extForcesCommand", "External forces command", extForcesCommand);
	opts.require("extForces", "extCoordFilename", "External forces coordinate filename", extCoordFilename);
	opts.require("extForces", "extForceFilename", "External forces force filename", extForceFilename);

	//print which bad contacts are being moved downhill
	opts.optionalB("main", "printBadContacts", "Print atoms with huge forces?", &printBadContacts, FALSE);

	/* GBIS generalized born implicit solvent*/

	opts.optionalB("main", "GBIS", "Use GB implicit solvent?", &GBISOn, FALSE);
	opts.optionalB("main", "GBISSer", "Use GB implicit solvent?", &GBISserOn, FALSE);

	opts.optional("GBIS", "solventDielectric", "Solvent Dielectric", &solvent_dielectric, 78.5);
	opts.optional("GBIS", "intrinsicRadiusOffset", "Coulomb Radius Offset", &coulomb_radius_offset, 0.09);
	opts.optional("GBIS", "ionConcentration", "Ion Concentration", &ion_concentration, 0.2); //0.2 mol/L
	opts.optional("GBIS", "GBISDelta", "delta from GBOBC", &gbis_delta, 1.0); //0.8 or 1.0
	opts.optional("GBIS", "GBISBeta", "beta from GBOBC", &gbis_beta, 0.8);   //0.0 or 0.8
	opts.optional("GBIS", "GBISGamma", "gamma from GBOBC", &gbis_gamma, 4.85);   //2.290912 or 4.85
	opts.optional("GBIS", "alphaCutoff", "cutoff for calculating effective born radius", &alpha_cutoff, 15);
	opts.optional("GBIS", "alphaMax", "maximum allowable born radius", &alpha_max, 30);
	opts.optional("GBIS", "fsMax", "maximum screened intrinsic radius", &fsMax, 1.728);

	opts.optionalB("main", "SASA", "Use Linear Combination of Pairwise Overlaps (LCPO) for calculating SASA", &LCPOOn,
	FALSE);
	opts.optional("SASA", "surfaceTension", "Surfce Tension for SASA (kcal/mol/Ang^2)", &surface_tension, 0.005);

	//****** BEGIN SMD constraints changes

	// SMD constraints
	opts.optionalB("main", "SMD", "Do we use SMD option?", &SMDOn, FALSE);
	opts.require("SMD", "SMDVel", "Velocity of the movement, A/timestep", &SMDVel);
	opts.range("SMDVel", NOT_NEGATIVE);
	opts.require("SMD", "SMDDir", "Direction of movement", &SMDDir);
	opts.require("SMD", "SMDk", "Elastic constant for SMD", &SMDk);
	opts.optional("SMD", "SMDk2", "Transverse elastic constant for SMD", &SMDk2, 0);
	opts.range("SMDk", NOT_NEGATIVE);
	opts.range("SMDk2", NOT_NEGATIVE);
	opts.require("SMD", "SMDFile", "File for SMD information", SMDFile);
	opts.optional("SMD", "SMDOutputFreq", "Frequency of output", &SMDOutputFreq, 1);
	opts.range("SMDOutputFreq", POSITIVE);

	//****** END SMD constraints changes

	//****** BEGIN tabulated energies section
	opts.optionalB("main", "tabulatedEnergies", "Do we get energies from a table?", &tabulatedEnergies,
	FALSE);
//   opts.require("tabulatedEnergies", "tableNumTypes","Number of types for energy tabulation", &tableNumTypes);
	...
	opts.optionalB("TMD", "TMDDiffRMSD", "Restrain Difference between the RMSD from two structures", &TMDDiffRMSD,
	FALSE);
	opts.require("TMDDiffRMSD", "TMDFile2", "Second file for TMD information", TMDFile2);

	opts.optional("TMD", "TMDOutputFreq", "Frequency of TMD output", &TMDOutputFreq, 1);
	...
	opts.optional("TMD", "TMDInitialRMSD", "Target RMSD at first TMD step (default -1 to use initial coordinates)",
			&TMDInitialRMSD);
	TMDInitialRMSD = -1;
	...
	opts.optional("symmetryRestraints", "symmetrykfile", "PDB file specifying force contants on a per-atom basis",
	PARSE_MULTIPLES);
	opts.optionalB("symmetryRestraints", "symmetryScaleForces", "Scale applied forces over time?", &symmetryScaleForces,
	FALSE);
	opts.require("symmetryRestraints", "symmetryFile", "File for symmetry information", PARSE_MULTIPLES);
	opts.optional("symmetryRestraints", "symmetryMatrixFile", "File(s) for transfromation matrices",
	PARSE_MULTIPLES);
	...
	opts.optional("symmetryRestraints", "symmetryLastFullStep",
			"Last full force symmetry timestep (default symmetryLastStep)", &symmetryLastFullStep, symmetryLastStep);
	opts.optional("symmetryRestraints", "symmetryFirstFullStep",
	...
	////  Global Forces / Tcl
	opts.optionalB("main", "tclForces", "Are Tcl global forces active?", &tclForcesOn, FALSE);
	opts.require("tclForces", "tclForcesScript", "Tcl script for global forces", PARSE_MULTIPLES);

	////  Boundary Forces / Tcl
	opts.optionalB("main", "tclBC", "Are Tcl boundary forces active?", &tclBCOn, FALSE);
	opts.require("tclBC", "tclBCScript", "Tcl script defining calcforces for boundary forces",
	PARSE_STRING);
	tclBCScript = 0;
	opts.optional("tclBC", "tclBCArgs", "Extra args for calcforces command", tclBCArgs);
	tclBCArgs[0] = 0;

	////  Global Forces / Misc
	opts.optionalB("main", "miscForces", "Are misc global forces active?", &miscForcesOn, FALSE);
	opts.optional("miscForces", "miscForcesScript", "script for misc forces", PARSE_MULTIPLES);

	////  Free Energy Perturbation
	opts.optionalB("main", "freeEnergy", "Perform free energy perturbation?", &freeEnergyOn, FALSE);
	opts.require("freeEnergy", "freeEnergyConfig", "Configuration file for free energy perturbation",
	PARSE_MULTIPLES);

	////  Constant Force
	opts.optionalB("main", "constantforce", "Apply constant force?", &consForceOn, FALSE);
	opts.optional("constantforce", "consForceFile", "Configuration file for constant forces",
	PARSE_STRING);
	opts.require("constantforce", "consForceScaling", "Scaling factor for constant forces", &consForceScaling, 1.0);

	//// Collective variables
	opts.optionalB("main", "colvars", "Is the colvars module enabled?", &colvarsOn, FALSE);
	opts.require("colvars", "colvarsConfig", "configuration for the collective variables", PARSE_STRING);
	opts.optional("colvars", "colvarsInput", "input restart file for the collective variables",
	PARSE_STRING);
...
void SimParameters::config_parser_mgridforce(ParseOptions &opts) {
	//// Gridforce
	opts.optionalB("main", "mgridforce", "Is Multiple gridforce active?", &mgridforceOn, FALSE);
...
void SimParameters::config_parser_gridforce(ParseOptions &opts) {
	//// Gridforce
	opts.optionalB("main", "gridforce", "Is Gridforce active?", &gridforceOn, FALSE);
	opts.optionalB("gridforce", "gridforcevolts", "Is Gridforce using Volts/eV as units?", &gridforceVolts,
	FALSE);
	...
	opts.optional("gridforce", "gridforcevoff", "Gridforce potential offsets", &gridforceVOffset);
	opts.optionalB("gridforce", "gridforcelite", "Use Gridforce Lite?", &gridforceLite, FALSE);
}
/* END gf */

void SimParameters::config_parser_movdrag(ParseOptions &opts) {
	//// moving drag
	opts.optionalB("main", "movDragOn", "Do we apply moving drag?", &movDragOn, FALSE);
	opts.require("movDragOn", "movDragFile", "Main moving drag PDB file", movDragFile);
	opts.require("movDragOn", "movDragCol", "Main moving drag PDB column", PARSE_STRING);
	opts.require("movDragOn", "movDragGlobVel", "Global moving drag velocity (A/step)", &movDragGlobVel);
	opts.require("movDragOn", "movDragVelFile", "Moving drag linear velocity file", movDragVelFile);
}

void SimParameters::config_parser_rotdrag(ParseOptions &opts) {
	//// rotating drag
	opts.optionalB("main", "rotDragOn", "Do we apply rotating drag?", &rotDragOn, FALSE);
	opts.require("rotDragOn", "rotDragFile", "Main rotating drag PDB file", rotDragFile);
	opts.require("rotDragOn", "rotDragCol", "Main rotating drag PDB column", PARSE_STRING);
	opts.require("rotDragOn", "rotDragAxisFile", "Rotating drag axis file", rotDragAxisFile);
	opts.require("rotDragOn", "rotDragPivotFile", "Rotating drag pivot point file", rotDragPivotFile);
	opts.require("rotDragOn", "rotDragGlobVel", "Global rotating drag angular velocity (deg/step)", &rotDragGlobVel);
	opts.require("rotDragOn", "rotDragVelFile", "Rotating drag angular velocity file", rotDragVelFile);
	opts.require("rotDragOn", "rotDragVelCol", "Rotating drag angular velocity column", PARSE_STRING);
}

void SimParameters::config_parser_constorque(ParseOptions &opts) {
	//// "constant" torque
	opts.optionalB("main", "consTorqueOn", "Do we apply \"constant\" torque?", &consTorqueOn, FALSE);
	opts.require("consTorqueOn", "consTorqueFile", "Main \"constant\" torque PDB file", consTorqueFile);
	opts.require("consTorqueOn", "consTorqueCol", "Main \"constant\" torque PDB column", PARSE_STRING);
	opts.require("consTorqueOn", "consTorqueAxisFile", "\"Constant\" torque axis file", consTorqueAxisFile);
	opts.require("consTorqueOn", "consTorquePivotFile", "\"Constant\" torque pivot point file", consTorquePivotFile);
	opts.require("consTorqueOn", "consTorqueGlobVal", "Global \"constant\" torque value (Kcal/(mol*A^2))",
			&consTorqueGlobVal);
	opts.require("consTorqueOn", "consTorqueValFile", "\"constant\" torque factors file", consTorqueValFile);
	opts.require("consTorqueOn", "consTorqueValCol", "\"constant\" torque factors column", PARSE_STRING);
}
...
void SimParameters::config_parser_boundary(ParseOptions &opts) {
	...
	opts.require("sphericalBC", "sphericalBCCenter", "Center of spherical boundaries", &sphericalCenter);
	...
	opts.require("sphericalBC", "sphericalBCk1", "Force constant for first "
			"sphere potential (+ is an inward force, - outward)", &sphericalBCk1);
	...
	opts.require("sphericalBCr2", "sphericalBCk2", "Force constant for second "
			"sphere potential (+ is an inward force, - outward)", &sphericalBCk2);
	...
	opts.require("cylindricalBC", "cylindricalBCk1", "Force constant for first "
			"cylinder potential (+ is an inward force, - outward)", &cylindricalBCk1);
...
// additions beyond those already found in spherical parameters    JJU
	...
	PARSE_STRING);
	opts.require("cylindricalBC", "cylindricalBCCenter", "Center of cylindrical boundaries", &cylindricalCenter);
	opts.require("cylindricalBC", "cylindricalBCl1", "Length of first cylinder", &cylindricalBCl1);
	opts.range("cylindricalBCl1", POSITIVE);
	opts.units("cylindricalBCl1", N_ANGSTROM);
	opts.optional("cylindricalBCl1", "cylindricalBCl2", "Length of second cylinder", &cylindricalBCl2);
...
// end  additions
	...
	opts.require("cylindricalBCr2", "cylindricalBCk2", "Force constant for second "
			"cylinder potential (+ is an inward force, - outward)", &cylindricalBCk2);
	...
	opts.optionalB("main", "eFieldOn", "Should an electric field be applied", &eFieldOn, FALSE);
	...
	///////////////  Stir options
	opts.optionalB("main", "stirOn", "Should stirring torque be applied", &stirOn, FALSE);
	...
	//////////  Extra bonds options
	opts.optionalB("main", "extraBonds", "Should extra bonded forces be applied", &extraBondsOn, FALSE);
	opts.optional("extraBonds", "extraBondsFile", "file with list of extra bonds",
	PARSE_MULTIPLES);
...
void SimParameters::config_parser_misc(ParseOptions &opts) {
	...
	opts.optional("main", "ldBalancer", "Load balancer", loadBalancer);
	opts.optional("main", "ldbStrategy", "Load balancing strategy", loadStrategy);
	opts.optional("main", "ldbPeriod", "steps between load balancing", &ldbPeriod);
	opts.range("ldbPeriod", POSITIVE);
	opts.optional("main", "firstLdbStep", "when to start load balancing", &firstLdbStep);
	opts.range("firstLdbStep", POSITIVE);
	opts.optional("main", "lastLdbStep", "when to stop load balancing", &lastLdbStep);
	opts.range("lastLdbStep", POSITIVE);
	opts.optional("main", "hybridGroupSize", "Hybrid load balancing group size", &hybridGroupSize);
	opts.optional("main", "ldbBackgroundScaling", "background load scaling", &ldbBackgroundScaling);
	opts.range("ldbBackgroundScaling", NOT_NEGATIVE);
	opts.optional("main", "ldbPMEBackgroundScaling", "PME node background load scaling", &ldbPMEBackgroundScaling);
	opts.range("ldbPMEBackgroundScaling", NOT_NEGATIVE);
	opts.optional("main", "ldbHomeBackgroundScaling", "home node background load scaling", &ldbHomeBackgroundScaling);
	opts.range("ldbHomeBackgroundScaling", NOT_NEGATIVE);
	opts.optional("main", "ldbRelativeGrainsize", "fraction of average load per compute", &ldbRelativeGrainsize, 0.);
	...
	opts.optionalB("main", "outputMaps",
			"whether to dump compute map and patch map for analysis just before load balancing", &outputMaps,
			FALSE);
	opts.optionalB("main", "benchTimestep",
			"whether to do benchmarking timestep in which case final file output is disabled", &benchTimestep,
			FALSE);
	opts.optional("main", "useCkLoop", "whether to use CkLoop library to parallelize a loop in a function like OpenMP",
			&useCkLoop, 0);
	opts.range("useCkLoop", NOT_NEGATIVE);

	opts.optionalB("main", "simulateInitialMapping", "whether to study the initial mapping scheme",
			&simulateInitialMapping, FALSE);
	...
	opts.optional("main", "simulatedNodeSize", "the node size to be used for studying initial mapping",
			&simulatedNodeSize);
	opts.range("simulatedNodeSize", POSITIVE);

	opts.optionalB("main", "ldbUnloadPME", "no load on PME nodes", &ldbUnloadPME, FALSE);
	opts.optionalB("main", "ldbUnloadZero", "no load on pe zero", &ldbUnloadZero, FALSE);
	opts.optionalB("main", "ldbUnloadOne", "no load on pe one", &ldbUnloadOne, FALSE);
	opts.optionalB("main", "ldbUnloadOutputPEs", "no load on output PEs", &ldbUnloadOutputPEs, FALSE);
	opts.optionalB("main", "noPatchesOnZero", "no patches on pe zero", &noPatchesOnZero, FALSE);
	opts.optionalB("main", "noPatchesOnOutputPEs", "no patches on Output PEs", &noPatchesOnOutputPEs,
	FALSE);
	opts.optionalB("main", "noPatchesOnOne", "no patches on pe one", &noPatchesOnOne, FALSE);
	opts.optionalB("main", "useCompressedPsf", "The structure file psf is in the compressed format", &useCompressedPsf,
	FALSE);
	opts.optionalB("main", "genCompressedPsf", "Generate the compressed version of the psf file", &genCompressedPsf,
	FALSE);
	opts.optionalB("main", "usePluginIO", "Use the plugin I/O to load the molecule system", &usePluginIO,
	FALSE);
	opts.optionalB("main", "mallocTest", "test how much memory all PEs can allocate", &mallocTest, FALSE);
	opts.optionalB("main", "printExclusions", "print exclusion lists to stdout", &printExclusions, FALSE);
	opts.optional("main", "proxySendSpanningTree", "using spanning tree to send proxies", &proxySendSpanningTree, -1);
	opts.optional("main", "proxyRecvSpanningTree", "using spanning tree to receive proxies", &proxyRecvSpanningTree, 0); // default off due to memory leak -1);
	opts.optional("main", "proxyTreeBranchFactor", "the branch factor when building a spanning tree",
			&proxyTreeBranchFactor, 4);
	opts.optionalB("main", "twoAwayX", "half-size patches in 1st dimension", &twoAwayX, -1);
	opts.optionalB("main", "twoAwayY", "half-size patches in 2nd dimension", &twoAwayY, -1);
	opts.optionalB("main", "twoAwayZ", "half-size patches in 3rd dimension", &twoAwayZ, -1);
	opts.optional("main", "maxPatches", "maximum patch count", &maxPatches, -1);

	/////  Restart timestep option
	opts.optional("main", "firsttimestep", "Timestep to start simulation at", &firstTimestep, 0);
	opts.range("firsttimestep", NOT_NEGATIVE);

	/////  Test mode options
	opts.optionalB("main", "test", "Perform self-tests rather than simulation", &testOn, FALSE);
	opts.optionalB("main", "commOnly", "Do not evaluate forces or integrate", &commOnly, FALSE);

	opts.optionalB("main", "statsOn", "counters in machine layer", &statsOn, FALSE);
	///////////////  hydrogen bond computation options
	opts.optionalB("main", "hbonds", "Use explicit hydrogen bond term", &HydrogenBonds, FALSE);
	opts.optionalB("hbonds", "hbAntecedents", "Include Antecedent in hbond term", &useAntecedent, TRUE);
	opts.optional("hbonds", "hbAAexp", "Hbond AA-A-H angle cos exponential", &aaAngleExp, 2);
	opts.optional("hbonds", "hbHAexp", "Hbond D-H-A angle cos exponential", &haAngleExp, 4);
	opts.optional("hbonds", "hbDistAexp", "Hbond A-D dist attractive exponential", &distAttExp, 4);
	opts.optional("hbonds", "hbDistRexp", "Hbond A-D dist repulstive exponential", &distRepExp, 6);
	opts.optional("hbonds", "hbCutoffAngle", "Hbond D-H-A cutoff angle", &dhaCutoffAngle, 100.0);
	opts.range("hbCutoffAngle", NOT_NEGATIVE);
	opts.optional("hbonds", "hbOnAngle", "Hbond D-H-A switch function on angle", &dhaOnAngle, 60.0);
	opts.range("hbOnAngle", NOT_NEGATIVE);
	opts.optional("hbonds", "hbOffAngle", "Hbond D-H-A switch function off angle", &dhaOffAngle, 80.0);
	opts.range("hbOffAngle", NOT_NEGATIVE);
	opts.optional("hbonds", "hbCutoffDist", "Hbond A-D cutoff distance", &daCutoffDist, 7.5);
	opts.range("hbCutoffDist", POSITIVE);
	opts.units("hbCutoffDist", N_ANGSTROM);
	opts.optional("hbonds", "hbOnDist", "Hbond A-D switch function on distance", &daOnDist, 5.5);
	opts.range("hbOnDist", POSITIVE);
	opts.units("hbOnDist", N_ANGSTROM);
	opts.optional("hbonds", "hbOffDist", "Hbond A-D switch function off distance", &daOffDist, 6.5);
	...
	opts.optional("ldBalancer", "maxSelfPart", "maximum number of self partitions in one patch", &maxSelfPart, 20);
	opts.range("maxSelfPart", POSITIVE);
	opts.optional("ldBalancer", "maxPairPart", "maximum number of pair partitions in one patch", &maxPairPart, 8);
	opts.range("maxPairPart", POSITIVE);
	opts.optional("ldBalancer", "numAtomsSelf", "maximum number of atoms in one self compute distribution",
			&numAtomsSelf, 154);
	opts.range("numAtomsSelf", NOT_NEGATIVE);

	opts.optional("ldBalancer", "numAtomsSelf2", "maximum number of atoms in one self compute distribution",
			&numAtomsSelf2, 154);
	opts.range("numAtomsSelf2", NOT_NEGATIVE);

	opts.optional("ldBalancer", "numAtomsPair", "maximum number of atoms in one pair compute distribution",
			&numAtomsPair, 318);
	opts.range("numAtomsPair", NOT_NEGATIVE);
	opts.optional("ldBalancer", "numAtomsPair2", "maximum number of atoms in one pair compute distribution",
			&numAtomsPair2, 637);
	opts.range("numAtomsPair2", NOT_NEGATIVE);
	opts.optional("main", "minAtomsPerPatch", "minimum average atoms per patch", &minAtomsPerPatch, 40);
	opts.range("minAtomsPerPatch", NOT_NEGATIVE);

	// Maximum exclusion flags per atom
	opts.optional("main", "maxExclusionFlags", "maximum number of exclusion flags per atom", &maxExclusionFlags, 256);
	...
	opts.optional("main", "mic_unloadMICPEs",
			"Indicates whether or not the load balancer should unload PEs driving Xeon Phi cards", &mic_unloadMICPEs,
			1);
	opts.range("mic_unloadMICPEs", NOT_NEGATIVE);
	opts.optional("main", "mic_deviceThreshold", "Threshold to use for directing computes to Xeon Phi devices",
			&mic_deviceThreshold, -1);
...
void SimParameters::check_config(ParseOptions &opts, ConfigList *config, char *&cwd) {
...
#ifndef MEM_OPT_VERSION
	if (!genCompressedPsf && !opts.defined("coordinates") && !opts.defined("ambercoor") && !opts.defined("grocoorfile")
			&& !usePluginIO)
	...
	//  specified
	if (opts.defined("temperature") && (opts.defined("velocities") || opts.defined("binvelocities"))) {
...
//record the absolute file name for binAtomFile, binCoorFile and binVelFile etc.
	...
	if (!amberOn) { //****** BEGIN CHARMM/XPLOR type changes
					//// set default
	...
	if (!(minimizeOn || minimizeCGOn) && !opts.defined("temperature") && !opts.defined("velocities")
			&& !opts.defined("binvelocities")) {
		NAMD_die("Must have either an initial temperature or a velocity file");
	}

	if (minimizeOn || minimizeCGOn) {
		initialTemp = 0.0;
	}
	...
	if (opts.defined("extendedSystem")) {
		...
		if (!xscFile)
			NAMD_die("Unable to open extended system file.\n");

		char labels[1024];
		do {
			if (!xscFile)
				NAMD_die("Error reading extended system file.\n");
		...
		for (pos = 0; pos < numpos; ++pos) {
			...
			if (!xscFile)
				NAMD_die("Error reading extended system file.\n");
			if (pos == a_x)
				cellBasisVector1.x = tmp;
			if (pos == a_y)
				cellBasisVector1.y = tmp;
			if (pos == a_z)
				cellBasisVector1.z = tmp;
			if (pos == b_x)
				cellBasisVector2.x = tmp;
			if (pos == b_y)
				cellBasisVector2.y = tmp;
			if (pos == b_z)
				cellBasisVector2.z = tmp;
			if (pos == c_x)
				cellBasisVector3.x = tmp;
			if (pos == c_y)
				cellBasisVector3.y = tmp;
			if (pos == c_z)
				cellBasisVector3.z = tmp;
			if (pos == o_x)
				cellOrigin.x = tmp;
			if (pos == o_y)
				cellOrigin.y = tmp;
	...
	if (!opts.defined("splitPatch")) {
		...
		if (!strcasecmp(s, "position"))
			...
			char err_msg[129];
			sprintf(err_msg, "Illegal value '%s' for 'splitPatch' in configuration file", s);
			NAMD_die(err_msg);
		}
	}
	...
	if (!strcasecmp(s, "none")) {
		...
		sprintf(err_msg, "Illegal value '%s' for 'exclude' in configuration file", s);
		NAMD_die(err_msg);
	}
	...
	if (!opts.defined("MTSAlgorithm")) {
		...
		if (!strcasecmp(s, "naive")) {
			...
			sprintf(err_msg, "Illegal value '%s' for 'MTSAlgorithm' in configuration file", s);
			NAMD_die(err_msg);
		}
	}
	...
	if (!opts.defined("longSplitting")) {
		...
		if (!strcasecmp(s, "sharp"))
			...
			sprintf(err_msg, "Illegal value '%s' for 'longSplitting' in configuration file", s);
			NAMD_die(err_msg);
		}
	}
	...
	if (!opts.defined("rigidBonds")) {
		...
		if (!strcasecmp(s, "all")) {
			...
			char err_msg[256];
			sprintf(err_msg, "Illegal value '%s' for 'rigidBonds' in configuration file", s);
			NAMD_die(err_msg);
		}
	}
	...
	if (switchingActive) {
		...
		if ((switchingDist > cutoff) || (switchingDist < 0)) {
			...
			sprintf(err_msg, "switchDist muct be between 0 and cutoff, which is %f", cutoff);
			NAMD_die(err_msg);
		}
	...
	if (adaptTempOn) {
		...
		if (opts.defined("adaptTempInFile")
				&& (opts.defined("adaptTempTmin") || opts.defined("adaptTempTmax") || adaptTempBins != 0))
			NAMD_die(
	...
	if (langevinOn) {
		if (!opts.defined("langevinDamping"))
			langevinDamping = 0.0;
		if (!opts.defined("langevinHydrogen"))
	...
	// BEGIN LA
	if (loweAndersenOn) {
		if (!opts.defined("loweAndersenRate"))
			loweAndersenRate = 100;
		if (!opts.defined("loweAndersenCutoff"))
...
//Modifications for alchemical fep
	...
	if (alchOn) {
		...
		if (rescaleFreq > 0)
			alchTemp = rescaleTemp;
		else if (reassignFreq > 0)
			alchTemp = reassignTemp;
		else if (langevinOn)
			alchTemp = langevinTemp;
		else if (tCoupleOn)
			alchTemp = tCoupleTemp;
		else
			NAMD_die("Alchemical FEP can be performed only in constant temperature simulations\n");
		...
		if (alchFepOn) {
			...
			if ((alchFepWCARepuOn || alchFepWCADispOn || alchFepElecOn) && (!alchFepWhamOn))
				NAMD_die(
						"alchFepWhamOn has to be on if one of alchFepWCARepuOn/alchFepWCADispOn/alchFepElecOn is set.");
			...
			if (alchFepWCARepuOn && alchFepElecOn)
				NAMD_die(
						"With WCA decomposition, repulsion and electrostatic perturbation can NOT be in the same FEP stage");
			if (alchFepWCADispOn && alchFepElecOn)
				NAMD_die(
						"With WCA decomposition, dispersion and electrostatic perturbation can NOT be in the same FEP stage");
			if (alchFepWCARepuOn
					&& (!opts.defined("alchFepWCArcut1") || !opts.defined("alchFepWCArcut2")
							|| !opts.defined("alchFepWCArcut3")))
				NAMD_die(
						"When using WCA repulsion,  alchFepWCArcut1, alchFepWCArcut2, and alchFepWCArcut3 must be defined!");
			if (alchFepWCARepuOn && ((alchFepWCArcut1 > alchFepWCArcut2) || (alchFepWCArcut2 > alchFepWCArcut3)))
				NAMD_die(
						"When using WCA repulsion,  alchFepWCArcut2 must be larger than alchFEPWCArcut1, alchFepWCArcut3 must be larger than alchFEPWCArcut2!");
...
//           NAMD_die("When using WCA decomposition,  repulsion, dispersion and electrostatic must be in 3 different stages!");
			if (alchFepWhamOn && (alchRepLambda < 0.0) && (alchDispLambda < 0.0) && (alchElecLambda < 0.0))
				NAMD_die(
						"One of alchRepLambda, alchDispLambda and alchElecLambda should be set up when alchFepWhamOn is true!");
...
//fepe
	...
	if (hybridGroupSize < CkNumPes()) {
		...
		if (CkNumPes() % nodesize)
			nodesize = CmiNodeSize(CmiNodeOf(0));
		if (CkNumPes() % nodesize)
			nodesize = 1;
		groupsize += nodesize - 1;
		while (2 * groupsize > CkNumPes())
			--groupsize;
		if (groupsize < nodesize)
			groupsize = nodesize;
		while (groupsize % nodesize)
			--groupsize;
		while (groupsize && CkNumPes() % groupsize)
			groupsize -= nodesize;
		if (2 * groupsize < hybridGroupSize) {
			groupsize += nodesize;
			while (CkNumPes() % groupsize)
				groupsize += nodesize;
		}
		if (2 * groupsize <= CkNumPes())
			hybridGroupSize = groupsize;
	}
	...
	if (PMEOn) {  // idiot checking
		...
		if (PMEGridSpacing == 0.) {
			if (PMEGridSizeX * PMEGridSizeY * PMEGridSizeZ == 0)
				...
				PMEGridSpacing = 1.5;  // only exit in very bad cases
		}
		...
		for (int idim = 0; idim < 3; ++idim) {
			...
			case 0:
				direction = "X";
				gridSize = &PMEGridSizeX;
				cellLength = lattice.a().length();
				break;
			case 1:
				direction = "Y";
				gridSize = &PMEGridSizeY;
				cellLength = lattice.b().length();
				break;
			case 2:
				direction = "Z";
				gridSize = &PMEGridSizeZ;
				cellLength = lattice.c().length();
				break;
			}
			...
			int max2, max3, ts;
			for (max2 = 2, ts = 1; ts < minSize; ++max2)
				ts *= 2;
			for (max3 = 2, ts = 1; ts < minSize; ++max3)
				ts *= 3;
			int max5 = 2;
			int max7 = 1;
			int max11 = 1;
			...
			if (*gridSize * PMEGridSpacing < cellLength) {
				...
				sprintf(errmsg, "PMEGridSize%s %d is too small for cell length %f and PMEGridSpacing %f\n", direction,
						*gridSize, cellLength, PMEGridSpacing);
				NAMD_die(errmsg);
			}
#endif
		}
		...
		while (erfc(ewaldcof * cutoff) / cutoff >= tolerance)
			ewaldcof *= 2.0;
	...
	if (opts.defined("rotConstraints") && opts.defined("movingConstraints")) {
		NAMD_die("Rotating and moving constraints are mutually exclusive!");
	}
	//****** END rotating constraints changes

	//****** BEGIN selective restraints (X,Y,Z) changes
	if (opts.defined("selectConstraints") && !opts.defined("selectConstrX") && !opts.defined("selectConstrY")
			&& !opts.defined("selectConstrZ")) {
	...
	if (SMDOn) {
		...
		if (SMDOutputFreq > 0 && SMDOutputFreq < stepsPerCycle || SMDOutputFreq % stepsPerCycle != 0) {
	...
	if (!eFieldOn) {
		...
		if (!opts.defined("eFieldFreq"))
			eFieldFreq = 0.0;
		if (!opts.defined("eFieldPhase"))
			eFieldPhase = 0.0;
	}
...
void SimParameters::print_config(ParseOptions &opts, ConfigList *config, char *&cwd) {
	...
	if (ldbUnloadOne || CkNumPes() > 256)
		noPatchesOnOne = TRUE;
	if (ldbUnloadZero || noPatchesOnOne || CkNumPes() > 64 || (IMDon && CkNumPes() > 8)) {
		noPatchesOnZero = TRUE;
	}
	if (noPatchesOnZero)
		iout<< iINFO << "REMOVING PATCHES FROM PROCESSOR 0\n";
	if (noPatchesOnOne)
		iout<< iINFO << "REMOVING PATCHES FROM PROCESSOR 1\n";
	iout<< endi;
...
//fepe
	...
	globalForcesOn = (tclForcesOn || freeEnergyOn || miscForcesOn || (IMDon && !IMDignore) || SMDOn || TMDOn
			|| colvarsOn || symmetryOn);
	...
	if (berendsenPressureOn) {
		...
		if ((berendsenPressureFreq % nonbondedFrequency)
				|| (fullElectFrequency && (berendsenPressureFreq % fullElectFrequency)))
	...
	if (MSMOn) {
		...
		// MSMApprox
		enum {
			CUBIC = 0, QUINTIC, QUINTIC2, SEPTIC, SEPTIC3, NONIC, NONIC4, C1HERMITE, NUM_APPROX
		};

		// MSMSplit
		enum {
			TAYLOR2 = 0, TAYLOR3, TAYLOR4, TAYLOR5, TAYLOR6, TAYLOR7, TAYLOR8, NUM_SPLIT
		};
		...
		case CUBIC:
			...
			break;
		case QUINTIC:
			approx_str = "C1 QUINTIC";
			break;
		case QUINTIC2:
			approx_str = "C2 QUINTIC";
			break;
		case SEPTIC:
			approx_str = "C1 SEPTIC";
			break;
		case SEPTIC3:
			approx_str = "C3 SEPTIC";
			break;
		case NONIC:
			approx_str = "C1 NONIC";
			break;
...
void SimParameters::parse_mgrid_params(ConfigList *config) {
	...
	while (current != NULL) {
		...
		mgfp->gridforceScale.x = mgfp->gridforceScale.y = mgfp->gridforceScale.z = 1.;
		mgfp->gridforceVOffset.x = mgfp->gridforceVOffset.y = mgfp->gridforceVOffset.z = 0.;
...
void SimParameters::parse_mgrid_string_param(ConfigList *cl, const char *fieldname, char **dest) {
