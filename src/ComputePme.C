...
public:
	PmePencilMap(int i_a, int i_b, int n_b, int n, int *d) :
			ia(i_a), ib(i_b), nb(n_b), size(n), data(newcopyint(n, d)) {
	}
	...
	virtual void populateInitial(int, CkArrayIndexMax &, void *msg, CkArrMgr *mgr) {
		...
		for (int i = 0; i < size; ++i) {
			if (data[i] == mype) {
				...
				ai.data()[ia] = i / nb;
				ai.data()[ib] = i % nb;
				if (procNum(0, ai) != mype)
					NAMD_bug("PmePencilMap is inconsistent");
				if (!msg)
					NAMD_bug("PmePencilMap multiple pencils on a pe?");
...
struct ijpair_sortop_bit_reversed {
	inline bool operator()(const ijpair &a, const ijpair &b) const {
		return (less_than_bit_reversed(a.i, b.i) || ((a.i == b.i) && less_than_bit_reversed(a.j, b.j)));
	}
};
...
ComputePmeMgr::ComputePmeMgr() :
		pmeProxy(thisgroup), pmeProxyDir(thisgroup) {
...
void ComputePmeMgr::recvArrays(CProxy_PmeXPencil x, CProxy_PmeYPencil y, CProxy_PmeZPencil z) {
	xPencil = x;
	yPencil = y;
	zPencil = z;
...
void ComputePmeMgr::initialize(CkQdMsg *msg) {
	...
	if (alchFepOn || alchThermIntOn) {
		...
		if (alchDecouple)
			numGrids += 2;
		if (alchElecLambdaStart || alchThermIntOn)
			numGrids++;
	} else
		numGrids = 1;
	...
	if (pairOn) {
		...
		if (selfOn)
			pairOn = 0;  // make pairOn and selfOn exclusive
		numGrids = selfOn ? 1 : 3;
	}

	if (numGrids != 1 || simParams->PMEPencils == 0)
		usePencils = 0;
	else if (simParams->PMEPencils > 0)
		usePencils = 1;
	else {
		...
		if (maxslabs > nrps)
			maxslabs = nrps;
		int maxpencils = (simParams->PMEGridSizeX * simParams->PMEGridSizeY * simParams->PMEGridSizeZ)
				/ simParams->PMEMinPoints;
		if (maxpencils > nrps)
			maxpencils = nrps;
		if (maxpencils > 3 * maxslabs)
			usePencils = 1;
		else
			usePencils = 0;
	}
	...
	if (usePencils) {
		...
		if (nrps > CkNumPes())
			nrps = CkNumPes();
		if (simParams->PMEPencils > 1 && simParams->PMEPencils * simParams->PMEPencils <= nrps) {
			xBlocks = yBlocks = zBlocks = simParams->PMEPencils;
		} else {
			int nb2 = (simParams->PMEGridSizeX * simParams->PMEGridSizeY * simParams->PMEGridSizeZ)
					/ simParams->PMEMinPoints;
			if (nb2 > nrps)
				nb2 = nrps;
			if (nb2 < 1)
		...
		if (simParams->PMEPencilsX > 0)
			xBlocks = simParams->PMEPencilsX;
		if (simParams->PMEPencilsY > 0)
			yBlocks = simParams->PMEPencilsY;
		if (simParams->PMEPencilsZ > 0)
			zBlocks = simParams->PMEPencilsZ;
		...
		if (!CkMyPe()) {
			iout<< iINFO << "PME using " << xBlocks << " x " << yBlocks << " x " << zBlocks
		...
		if (!CkMyPe()) {
			iout << iINFO << "PME USING " << numGridNodes << " GRID NODES AND " << numTransNodes
			<< " TRANS NODES\n" << endi;
		}
	...
	if (usePencils) {
		...
		SortableResizeArray<int> xprocs(yBlocks * zBlocks);
		{
			int basepe = 0;
			int npe = CkNumPes();
			if (npe > xBlocks * yBlocks && npe > xBlocks * zBlocks && npe > yBlocks * zBlocks) {
				// avoid node 0
				++basepe;
				--npe;
			}
			...
			for (i = nopatches.size() - 1; i >= 0; --i)
				pmeprocs.add(nopatches[i]);
			if (useZero && !pmap->numPatchesOnNode(0))
				pmeprocs.add(0);
			for (i = patches.size() - 1; i >= 0; --i)
				pmeprocs.add(patches[i]);
			if (pmap->numPatchesOnNode(0))
				pmeprocs.add(0);

			int npes = pmeprocs.size();
			for (i = 0; i < xBlocks * yBlocks; ++i, ++pe)
				zprocs[i] = pmeprocs[pe % npes];
			if (i > 1 && zprocs[0] == zprocs[i - 1])
				zprocs[0] = 0;
#if !USE_RANDOM_TOPO
			zprocs.sort();
#endif
			for (i = 0; i < xBlocks * zBlocks; ++i, ++pe)
				yprocs[i] = pmeprocs[pe % npes];
			if (i > 1 && yprocs[0] == yprocs[i - 1])
				yprocs[0] = 0;
#if !USE_RANDOM_TOPO
			yprocs.sort();
#endif
			for (i = 0; i < yBlocks * zBlocks; ++i, ++pe)
				xprocs[i] = pmeprocs[pe % npes];
			if (i > 1 && xprocs[0] == xprocs[i - 1])
				xprocs[0] = 0;
#if !USE_RANDOM_TOPO
			xprocs.sort();
#endif
		...
		if (CkMyPe() == 0) {
			...
			CkArrayOptions zo(xBlocks, yBlocks, 1);
			zo.setMap(zm);
			CkArrayOptions yo(xBlocks, 1, zBlocks);
			yo.setMap(ym);
			CkArrayOptions xo(1, yBlocks, zBlocks);
			xo.setMap(xm);
			zo.setAnytimeMigration(false);
			zo.setStaticInsertion(true);
			yo.setAnytimeMigration(false);
			yo.setStaticInsertion(true);
			xo.setAnytimeMigration(false);
			xo.setStaticInsertion(true);
	...
	for (pe = 0; pe < numTransPes; ++pe) {
		...
		if (ny > myGrid.K2)
			ny = myGrid.K2;
		localInfo[pe].ny_after_transpose = ny - localInfo[pe].y_start_after_transpose;
	}
	...
	{  // decide how many pes this node exchanges charges with
		...
		numDestRecipPes = 0;
		for (node = 0; node < numNodes; ++node) {
			if (source_flags[node])
				++numSources;
			if (recipPeDest[node])
				++numDestRecipPes;
			if (source_flags[node] && CmiPeOnSamePhysicalNode(node, CkMyPe()))
				++numSourcesSamePhysicalNode;
		}
	...
	if (local_size < local_size_2)
		local_size = local_size_2;
...
void ComputePmeMgr::activate_pencils(CkQdMsg *msg) {
	if (!usePencils)
		return;
	if (CkMyPe() == 0)
		zPencil.dummyRecvGrid(CkMyPe(), 1);
}
...
void ComputePmeMgr::sendTransBarrier(void) {
	...
	if (sendTransBarrier_received < numGridPes)
		return;
	sendTransBarrier_received = 0;
	for (int i = 0; i < numGridPes; ++i) {
...
void ComputePmeMgr::sendTrans(void) {
	...
	for (int j = 0; j < numTransNodes; j++) {
		...
		PmeTransMsg *newmsg = new (nx * totlen * numGrids, PRIORITY_SIZE) PmeTransMsg;
		...
		if (npe > 1) {
			if (node == myTransNode)
				fwdSharedTrans(newmsg);
			else
				pmeNodeProxy[transNodeInfo[node].real_node].recvTrans(newmsg);
		} else
			pmeProxy[transPeMap[transNodeInfo[node].pe_start]].recvTrans(newmsg);
	}
...
void ComputePmeMgr::procTrans(PmeTransMsg *msg) {
	...
	if (msg->nx) {
		...
		int y_skip = localInfo[myTransPe].y_start_after_transpose - localInfo[first_pe].y_start_after_transpose;
		int ny_msg = localInfo[last_pe].y_start_after_transpose + localInfo[last_pe].ny_after_transpose
		...
		for (int g = 0; g < numGrids; ++g) {
			CmiMemcpy((void*) (kgrid + qgrid_size * g + x_start * ny * zdim),
					(void*) (msg->qgrid + nx * (ny_msg * g + y_skip) * zdim), nx * ny * zdim * sizeof(float));
		}
	}
...
void ComputePmeMgr::gridCalc2R(void) {
	...
	for (int g = 0; g < numGrids; ++g) {
		...
		recip_evir2[g][0] = myKSpace->compute_energy(kgrid + qgrid_size * g, lattice, ewaldcof, &(recip_evir2[g][1]));
...
void ComputePmeMgr::sendUntrans(void) {
	...
	for (int j = 0; j < numGridNodes; j++) {
		...
		for (int g = 0; g < numGrids; ++g) {
			...
			for (int i = 0; i < npe; ++i, ++pe) {
				...
				if (node == myGridNode) {
					...
					CmiMemcpy((void*) qmsg, (void*) (kgrid + qgrid_size * g + li.x_start * ny * zdim),
		...
		if (npe > 1) {
			if (node == myGridNode)
				fwdSharedUntrans(newmsg);
			else
				pmeNodeProxy[gridNodeInfo[node].real_node].recvUntrans(newmsg);
		} else
			pmeProxy[gridPeMap[gridNodeInfo[node].pe_start]].recvUntrans(newmsg);
	}
...
void ComputePmeMgr::procUntrans(PmeUntransMsg *msg) {
	...
	if (msg->ny) {
		int zdim = myGrid.dim3;
		int last_pe = first_pe + nodeInfo.npe - 1;
		int x_skip = localInfo[myGridPe].x_start - localInfo[first_pe].x_start;
		int nx_msg = localInfo[last_pe].x_start + localInfo[last_pe].nx - localInfo[first_pe].x_start;
		int nx = localInfo[myGridPe].nx;
...
ComputePme::ComputePme(ComputeID c, PatchID pid) :
		Compute(c), patchID(pid) {
	DebugM(4, "ComputePme created.\n");
	basePriority = PME_PRIORITY;
	setNumPatches(1);

	CProxy_ComputePmeMgr::ckLocalBranch(CkpvAccess(BOCclass_group).computePmeMgr)->addCompute(this);
	...
	for (int g = 0; g < numGrids; ++g)
		myRealSpace[g] = new PmeRealSpace(myGrid);
}
...
void ComputePmeMgr::initialize_computes() {
	...
	for (int n = fsize * numGrids, i = 0; i < n; ++i)
		f_arr[i] = 2;
	...
	for (int g = 0; g < numGrids; ++g) {
		...
		if (usePencils) {
			...
			for (int ap = 0; ap < numPencilsActive; ++ap) {
				...
				int iend = ibegin + block1;
				if (iend > K1)
					iend = K1;
				int jbegin = jb * block2;
				int jend = jbegin + block2;
				if (jend > K2)
					jend = K2;
...
int ComputePme::noWork() {
	...
	if (patch->flags.doFullElectrostatics) {
		if (!myMgr->ungridForcesCount && !myMgr->recipEvirCount)
	...
	if (++(myMgr->noWorkCount) == myMgr->pmeComputes.size()) {
		...
		if (myMgr->amd_reduction)
			myMgr->amd_reduction->submit();
	}
...
void ComputePmeMgr::recvRecipEvir(PmeEvirMsg *msg) {
	if (!pmeComputes.size())
		NAMD_bug("ComputePmeMgr::recvRecipEvir() called on pe without patches");
	for (int g = 0; g < numGrids; ++g) {
		evir[g] += msg->evir[g];
	}
	delete msg;
	// CkPrintf("recvRecipEvir pe %d %d %d\n", CkMyPe(), ungridForcesCount, recipEvirCount);
	if (!--recipEvirCount && !ungridForcesCount)
		submitReductions();
}
...
void ComputePme::doWork() {
	...
	const BigReal coulomb_sqrt = sqrt(
	COULOMB * ComputeNonbondedUtil::scaling * ComputeNonbondedUtil::dielectric_1);
	...
	if (((alchFepOn || alchThermIntOn) && (!alchDecouple)) || lesOn) {
		...
		if (numGrids != 1)
			NAMD_bug("ComputePme::doWork assertion 1 failed");
		g = 0;
		PmeParticle *lgd = localGridData[g];
		int nga = 0;
		for (int i = 0; i < numLocalAtoms; ++i) {
		...
		if (numGrids != 3)
			NAMD_bug("ComputePme::doWork assertion 2 failed");
		g = 0;
		PmeParticle *lgd = localGridData[g];
		int nga = 0;
		for (int i = 0; i < numLocalAtoms; ++i) {
		...
		if (numGrids != 1)
			NAMD_bug("ComputePme::doWork assertion 3 failed");
	...
	if (sequence() != myMgr->compute_sequence)
		NAMD_bug("ComputePme sequence mismatch in doWork()");
	...
	for (g = 0; g < numGrids; ++g) {
		...
		myRealSpace[g]->fill_charges(q, myMgr->q_list, myMgr->q_count, strayChargeErrors, f, myMgr->fz_arr,
				localGridData[g]);
	}
...
void ComputePmeMgr::sendPencils(Lattice &lattice, int sequence) {
	...
	for (int ap = 0; ap < numPencilsActive; ++ap) {
		...
		PmeGridMsg *msg = new (hd * zlistlen, hd * flen, hd * fcount * zlistlen, PRIORITY_SIZE) PmeGridMsg;
		msg->sourceNode = CkMyPe();
		msg->hasData = hd;
		msg->lattice = lattice;
		if (hd) {
	...
	if (strayChargeErrors) {
		strayChargeErrors = 0;
		iout<< iERROR << "Stray PME grid charges detected: " << CkMyPe() << " sending to (x,y)";
...
void ComputePmeMgr::sendData(Lattice &lattice, int sequence) {
	...
	for (int j = 0; j < numGridPes; j++) {
		...
		if (!recipPeDest[pe] && !strayChargeErrors)
			continue;
		int start = pe * bsize;
		int len = bsize;
		if (start >= qsize) {
		...
		for (g = 0; g < numGrids; ++g) {
			...
			if (!recipPeDest[pe]) {
				...
				if (errfound) {
					iout<< iERROR << "Stray PME grid charges detected: " << CkMyPe()
					<< " sending to " << gridPeMap[pe] << " for planes";
					int iz = -1;
					for (i = 0; i < flen; ++i) {
						if (f[i] == 3) {
							f[i] = 2;
		...
		PmeGridMsg *msg = new (zlistlen, flen * numGrids, fcount * zlistlen, PRIORITY_SIZE) PmeGridMsg;
...
void ComputePme::ungridForces() {
	...
	if (sequence() != myMgr->compute_sequence)
		NAMD_bug("ComputePme sequence mismatch in ungridForces()");
		...
		for (g = 0; g < numGrids; ++g) {
			...
			if (alchFepOn || alchThermIntOn) {
				...
				if (simParams->alchFepWhamOn) {
					...
					elecLambdaUp =
							(simParams->alchLambda <= alchElecLambdaStart) ?
									0. : (simParams->alchLambda - alchElecLambdaStart) / (1. - alchElecLambdaStart);
					elecLambdaDown =
							((1 - simParams->alchLambda) <= alchElecLambdaStart) ?
									0. :
									((1 - simParams->alchLambda) - alchElecLambdaStart) / (1. - alchElecLambdaStart);
				}

				if (g == 0)
					scale = elecLambdaUp;
				else if (g == 1)
					scale = elecLambdaDown;
				else if (g == 2)
					scale = (elecLambdaUp + elecLambdaDown - 1) * (-1);
				if (alchDecouple) {
					if (g == 2)
						scale = 1 - elecLambdaUp;
					else if (g == 3)
						scale = 1 - elecLambdaDown;
					else if (g == 4)
						scale = (elecLambdaUp + elecLambdaDown - 1) * (-1);
				}
				int nga = 0;
				...
				if (alchFepOn) {
					if (simParams->alchFepWhamOn) {
						if (simParams->alchFepElecOn) {
							if (g == 0)
								scale = simParams->alchElecLambda;
							else if (g == 1)
								scale = 1. - simParams->alchElecLambda;
						} else {
							if (g == 0)
								scale = 0.0;
							else if (g == 1)
						...
						if (g == 0)
							scale = simParams->alchLambda;
						else if (g == 1)
							scale = 1. - simParams->alchLambda;
					}
				} else if (lesOn) {
	...
	// add in forces
	{
		...
		Force *fpme = r->f[Results::pme]; //lyk
		int numAtoms = patch->getNumAtoms();
		...
		if (!myMgr->strayChargeErrors && !simParams->commOnly) {
			for (int i = 0; i < numAtoms; ++i) {
				...
				f[i].y += results_ptr->y;
				f[i].z += results_ptr->z;
				fpme[i].x += results_ptr->x; //lyk
				fpme[i].y += results_ptr->y;
				fpme[i].z += results_ptr->z;
				++results_ptr;
			}
		}
...
void ComputePmeMgr::submitReductions() {
	...
	for (int g = 0; g < numGrids; ++g) {
		...
		reduction->item(REDUCTION_ELECT_ENERGY_SLOW) += evir[g][0] * scale; //lyk
		...
		if (amd_reduction) {
			amd_reduction->item(REDUCTION_ELECT_ENERGY_SLOW) += evir[g][0] * scale; //lyk
		...
		if (alchFepOn) {
			...
			if (simParams->alchFepWhamOn) {
				...
				elecLambda2Up =
						(simParams->alchLambda2 <= alchElecLambdaStart) ?
								0. : (simParams->alchLambda2 - alchElecLambdaStart) / (1. - alchElecLambdaStart);
				elecLambda2Down =
						((1 - simParams->alchLambda2) <= alchElecLambdaStart) ?
								0. : ((1 - simParams->alchLambda2) - alchElecLambdaStart) / (1. - alchElecLambdaStart);
			}
			...
			if (g == 0)
				...
				scale2 = (elecLambda2Up + elecLambda2Down - 1) * (-1);
			if (alchDecouple && g == 2)
				scale2 = 1 - elecLambda2Up;
			else if (alchDecouple && g == 3)
				scale2 = 1 - elecLambda2Down;
			else if (alchDecouple && g == 4)
				scale2 = (elecLambda2Up + elecLambda2Down - 1) * (-1);
		}
		...
		if (amd_reduction)
			amd_reduction->item(REDUCTION_ELECT_ENERGY_SLOW_F) += evir[g][0] * scale2;
		...
		if (alchThermIntOn) {
			...
			if (g == 0)
				reduction->item(REDUCTION_ELECT_ENERGY_PME_TI_1) += evir[g][0];
			if (g == 1)
				reduction->item(REDUCTION_ELECT_ENERGY_PME_TI_2) += evir[g][0];
			if (!alchDecouple) {
				if (g == 2)
					reduction->item(REDUCTION_ELECT_ENERGY_PME_TI_1) -= evir[g][0];
				if (g == 2)
...
void PmeZPencil::fft_init() {
	...
	nx = block1;
	if ((thisIndex.x + 1) * block1 > K1)
		nx = K1 - thisIndex.x * block1;
	ny = block2;
	if ((thisIndex.y + 1) * block2 > K2)
...
void PmeYPencil::fft_init() {
	...
	if ((thisIndex.x + 1) * block1 > K1)
		nx = K1 - thisIndex.x * block1;
	nz = block3;
	if ((thisIndex.z + 1) * block3 > dim3 / 2)
...
void PmeXPencil::fft_init() {
	...
	myKSpace = new PmeKSpace(initdata.grid, thisIndex.y * block2, thisIndex.y * block2 + ny, thisIndex.z * block3,
			thisIndex.z * block3 + nz);
...
void PmeYPencil::send_subset_trans(int fromIdx, int toIdx) {
	...
	for (int isend = fromIdx; isend <= toIdx; ++isend) {
		...
		if (hasData) {
			...
			if (md != msg->qgrid + nx * ny * nz * 2)
				CkPrintf("error in YX at %d %d %d\n", thisIndex.x, jb, thisIndex.z);
		}
		msg->sequence = sequence;
...
void PmeXPencil::pme_kspace() {
	...
	int numGrids = 1;
	for (int g = 0; g < numGrids; ++g) {
		evir[0] = myKSpace->compute_energy(data + 0 * g, lattice, ewaldcof, &(evir[1]));
	}
