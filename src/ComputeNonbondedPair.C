...
ComputeNonbondedPair::ComputeNonbondedPair(ComputeID c, PatchID pid[], int trans[],
		ComputeNonbondedWorkArrays* _workArrays, int minPartition, int maxPartition, int numPartitions) :
		ComputePatchPair(c, pid, trans), workArrays(_workArrays), minPart(minPartition), maxPart(maxPartition), numParts(
				numPartitions) {
	reduction = ReductionMgr::Object()->willSubmit(REDUCTIONS_BASIC);
	if (accelMDOn || itsOn || adaptCgOn) { //lyk
	...
	if (pressureProfileOn) {
		...
		pressureProfileReduction = ReductionMgr::Object()->willSubmit(REDUCTIONS_PPROF_NONBONDED,
				3 * pressureProfileSlabs * ((n * (n + 1)) / 2));
	} else {
	...
	if (patch[0]->flags.doNonbonded && (numAtoms[0] && numAtoms[1])) {
		...
		for (int i = 0; i < 2; i++) {
			...
			if (patch[0]->flags.doMolly)
				avgPositionBox[i]->skip();
			// BEGIN LA
			if (patch[0]->flags.doLoweAndersen)
				velocityBox[i]->skip();
			// END LA
		}
...
void ComputeNonbondedPair::doForce(CompAtom* p[2], CompAtomExt* pExt[2], Results* r[2]) {
	...
	if (!patch[0]->flags.doGBIS || gbisPhase == 1) {
		...
		DebugM(2, "doForce() called.\n");DebugM(2,
				numAtoms[0] << " patch #1 atoms and " << numAtoms[1] << " patch #2 atoms\n");
		...
		if (patch[0]->flags.savePairlists) {
			...
			if (!pairlistsValid
					|| (patch[0]->flags.maxAtomMovement + patch[1]->flags.maxAtomMovement > pairlistTolerance)) {
		...
		params.groupplcutoff = cutoff + patch[0]->flags.maxGroupRadius + patch[1]->flags.maxGroupRadius;
		if (params.savePairlists) {
			pairlistsValid = 1;
			pairlistTolerance = patch[0]->flags.pairlistTolerance + patch[1]->flags.pairlistTolerance;
		...
		//lyk
		params.vdwff_11[0] = r[a]->f[Results::vdw11];
		params.vdwff_11[1] = r[b]->f[Results::vdw11];
		params.vdwff_12[0] = r[a]->f[Results::vdw12];
		params.vdwff_12[1] = r[b]->f[Results::vdw12];
		params.vdwff_22[0] = r[a]->f[Results::vdw22];
		params.vdwff_22[1] = r[b]->f[Results::vdw22];

		params.vdwRepff_11[0] = r[a]->f[Results::vdw11_rep];
		params.vdwRepff_11[1] = r[b]->f[Results::vdw11_rep];
		params.vdwRepff_12[0] = r[a]->f[Results::vdw12_rep];
		params.vdwRepff_12[1] = r[b]->f[Results::vdw12_rep];
		params.vdwRepff_22[0] = r[a]->f[Results::vdw22_rep];
		params.vdwRepff_22[1] = r[b]->f[Results::vdw22_rep];

		params.electff_11[0] = r[a]->f[Results::elect11];
		params.electff_11[1] = r[b]->f[Results::elect11];
		params.electff_12[0] = r[a]->f[Results::elect12];
		params.electff_12[1] = r[b]->f[Results::elect12];
		params.electff_22[0] = r[a]->f[Results::elect22];
		params.electff_22[1] = r[b]->f[Results::elect22];
		...
		if (numAtoms[0] && numAtoms[1]) { //only do if has atoms since gbis noWork doesn't account for no atoms
			...
			if (patch[0]->flags.doFullElectrostatics) {
				...
				if (patch[0]->flags.doMolly) {
					...
					if (doEnergy)
						calcMergePairEnergy(&params);
					else
						calcMergePair(&params);

				} else {
					if (doEnergy)
						calcFullPairEnergy(&params);
					else
						calcFullPair(&params);
	...
	if (!patch[0]->flags.doGBIS || gbisPhase == 3) {
		...
		if (accelMDOn || itsOn || adaptCgOn) //lyk
			submitReductionData(reductionData, amd_reduction);
		if (pressureProfileOn)
