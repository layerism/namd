...
Sequencer::Sequencer(HomePatch *p) :
		simParams(Node::Object()->simParameters), patch(p), collection(CollectionMgr::Object()), ldbSteps(0) {
	...
	if (simParams->pressureProfileOn) {
		...
		pressureProfileReduction = ReductionMgr::Object()->willSubmit(REDUCTIONS_PPROF_INTERNAL, 3 * nslabs * ntypes);
	} else {
...
//    patch->write_tip4_props();
	...
	//lyk
	const Bool itsOn = simParams->itsOn;
	const Bool adaptCgOn = simParams->adaptCgOn;
	//lyk
	const Bool accelMDdihe = simParams->accelMDdihe;
	const Bool accelMDdual = simParams->accelMDdual;
	if (accelMDOn && (accelMDdihe || accelMDdual))
		maxForceUsed = Results::amdf;
	if (itsOn || adaptCgOn)  //lyk
		maxForceUsed = Results::pme;
...
//    printf("Doing initial rattle\n");
	...
	if (simParams->lonepairs) {
		patch->atomMapper->registerIDsFullAtom(patch->atom.begin(), patch->atom.end());
	}
	...
	adaptCgMD(step, doNonbonded, doFullElectrostatics); //lyk
	itsMD(step, doNonbonded, doFullElectrostatics); //lyk
	...
	for (++step; step <= numberOfSteps; ++step) {
		...
		if (!commOnly && movDragOn)
			addMovDragToPosition(timestep);
		if (!commOnly && rotDragOn)
			addRotDragToPosition(timestep);
...
void Sequencer::addMovDragToPosition(BigReal timestep) {
	...
	for (int i = 0; i < numAtoms; ++i) {
		...
		if ((simParams->fixedAtomsOn && atom[i].atomFixed) || !(molecule->is_atom_movdragged(atom[i].id)))
			continue;
...
void Sequencer::addRotDragToPosition(BigReal timestep) {
	...
	for (int i = 0; i < numAtoms; ++i) {
		// skip if fixed atom or zero drag attribute
		if ((simParams->fixedAtomsOn && atom[i].atomFixed) || !(molecule->is_atom_rotdragged(atom[i].id)))
			continue;
...
void Sequencer::newMinimizeDirection(BigReal c) {
	...
	for (int i = 0; i < numAtoms; i += hgs) {
		...
		if (minChildVel < fmax2)
			continue;
		int adjustChildren = 1;
		for (int j = i + 1; j < (i + hgs); ++j) {
			if (a[j].velocity.length2() > minChildVel)
				adjustChildren = 0;
		}
		if (adjustChildren) {
...
// Routine originally written by JPhillips, with fresh errors by CMatthews June2012
	...
	if (simParams->langevinOn) {
		...
		for (int i = 0; i < numAtoms; ++i) {
			...
			BigReal f2 = sqrt((1. - f1 * f1) * kbT * (a[i].partition ? tempFactor : 1.0) / a[i].mass);
	...
	if (simParams->langevinOn && !simParams->langevin_useBAOAB) {
		...
		if (simParams->drudeOn) {
			for (i = 0; i < numAtoms; i++) {
				...
				if (i < numAtoms - 1 && a[i + 1].mass < 1.0 && a[i + 1].mass >= 0.001) {
				...
				if (i < numAtoms - 1 && a[i + 1].mass < 1.0 && a[i + 1].mass >= 0.001) {
					...
					if (dt_gamma != 0.0) {
						...
						v_com += random->gaussian_vector()
								* sqrt(2 * dt_gamma * kbT * (a[i].partition ? tempFactor : 1.0) / mass);
						v_com /= (1. + 0.5 * dt_gamma);
					}
					...
					if (dt_gamma != 0.0) {
						...
						v_bnd += random->gaussian_vector()
								* sqrt(2 * dt_gamma * kbT_bnd * (a[i + 1].partition ? tempFactor : 1.0) / mass);
						v_bnd /= (1. + 0.5 * dt_gamma);
					}
					...
					a[i].velocity += random->gaussian_vector()
							* sqrt(2 * dt_gamma * kbT * (a[i].partition ? tempFactor : 1.0) / a[i].mass);
					a[i].velocity /= (1. + 0.5 * dt_gamma);
				}
			...
			if (simParams->useGroupPressure) {
				...
				for (int i = 0; i < numAtoms; i += hgs) {
					...
					if (simParams->fixedAtomsOn && a[i].groupFixed) {
						for (j = i; j < (i + hgs); ++j) {
							a[j].position = patch->lattice.apply_transform(a[j].fixedPosition, a[j].transform);
						}
						continue;
					}
					BigReal m_cm = 0;
				...
				for (int i = 0; i < numAtoms; ++i) {
					if (simParams->fixedAtomsOn && a[i].atomFixed) {
						a[i].position = patch->lattice.apply_transform(a[i].fixedPosition, a[i].transform);
						continue;
					}
...
void Sequencer::rescaleaccelMD(int step, int doNonbonded, int doFullElectrostatics) {
	...
	if ((step < simParams->accelMDFirstStep) || (simParams->accelMDLastStep > 0 && step > simParams->accelMDLastStep))
		return;
...
//lyk
void Sequencer::itsMD(int step, int doNonbonded, int doFullElectrostatics) {
	if (!simParams->itsOn || step < simParams->itsFirstStep || step >= simParams->itsLastStep)
		return;
	...
	Vector rescalingFactor = broadcast->forceRescaling.get(step);
	const BigReal factor = rescalingFactor[0];

	Force ff_11(0.0, 0.0, 0.0), ff_12(0.0, 0.0, 0.0), ff_22(0.0, 0.0, 0.0);

	for (int i = 0; i < numAtoms; ++i) {
		int aip = int(a[i].partition);

		if (simParams->group1 == -1) {
			patch->f[Results::normal][i] += factor * patch->f[Results::amdf][i];
			if (doNonbonded) {
				patch->f[Results::nbond][i] *= (1.0 + factor);
			}
			if (doFullElectrostatics) {
				patch->f[Results::slow][i] *= (1.0 + factor);
			}
		} else {

			if (its_intType == 0) {
				ff_11 = patch->f[Results::vdw11][i];
				ff_12 = patch->f[Results::vdw12][i];
				ff_22 = patch->f[Results::vdw22][i];
			} else if (its_intType == 1) {
				ff_11 = patch->f[Results::elect11][i];
				ff_12 = patch->f[Results::elect12][i];
				ff_22 = patch->f[Results::elect22][i];
			} else if (its_intType == 2) {
				ff_11 = patch->f[Results::vdw11][i] + patch->f[Results::elect11][i];
				ff_12 = patch->f[Results::vdw12][i] + patch->f[Results::elect12][i];
				ff_22 = patch->f[Results::vdw22][i] + patch->f[Results::elect22][i];
			} else if (its_intType == 3) {
				ff_11 = patch->f[Results::vdw11][i] - patch->f[Results::vdw11_rep][i];
				ff_12 = patch->f[Results::vdw12][i] - patch->f[Results::vdw12_rep][i];
				ff_22 = patch->f[Results::vdw22][i] - patch->f[Results::vdw22_rep][i];
			} else if (its_intType == 4) {
				ff_11 = patch->f[Results::vdw11][i] - patch->f[Results::vdw11_rep][i] + patch->f[Results::elect11][i];
				ff_12 = patch->f[Results::vdw12][i] - patch->f[Results::vdw12_rep][i] + patch->f[Results::elect12][i];
				ff_22 = patch->f[Results::vdw22][i] - patch->f[Results::vdw22_rep][i] + patch->f[Results::elect22][i];
			}
			if (simParams->onlyIntOn) {
				if (aip == simParams->group1) {
					patch->f[Results::normal][i] += factor * ff_12;
				}
				if (aip == simParams->group2) {
					patch->f[Results::normal][i] += factor * ff_12;
				}
			} else {
				if (aip == simParams->group1) {
					patch->f[Results::normal][i] += factor * (ff_11 + ff_12);
				}
				if (aip == simParams->group2) {
					patch->f[Results::normal][i] += factor * (ff_22 + ff_12);
				}
			}
		}
...
void Sequencer::adaptCgMD(int step, int doNonbonded, int doFullElectrostatics) {
	if (!simParams->adaptCgOn || step < simParams->adaptCgFirstStep || step >= simParams->adaptCgLastStep)
		return;
	...
	Vector rescalingFactor = broadcast->forceRescaling.get(step);
	const BigReal factor_aa = rescalingFactor[0];
	const BigReal factor_cg = rescalingFactor[1];

	Force ff_aa(0.0, 0.0, 0.0), ff_cg(0.0, 0.0, 0.0);
	/*
	 Force ff_nb(0.0, 0.0, 0.0), ff_slow(0.0, 0.0, 0.0);
	 for (int i = 0; i < numAtoms; ++i) {
	 if (doNonbonded) {
	 ff_nb += patch->f[Results::nbond][i];
	 }
	 if (doFullElectrostatics) {
	 ff_slow += patch->f[Results::slow][i];
	 }
	 }
	 iout<<"FORCE: "<<ff_nb<<"   "<<ff_slow<<"\n"<<endi;
	 */
	for (int i = 0; i < numAtoms; ++i) {
		int aip = int(a[i].partition);
		...
		if (simParams->aaGroup == -1) {
			if (acg_intType == 0) {
				ff_cg = patch->f[Results::vdw22][i];
			} else if (acg_intType == 1) {
				ff_cg = patch->f[Results::elect22][i];
			} else if (acg_intType == 2) {
				ff_cg = patch->f[Results::vdw22][i] + patch->f[Results::elect22][i];
			} else if (acg_intType == 3) {
				ff_cg = patch->f[Results::vdw22][i] - patch->f[Results::vdw22_rep][i];
			} else if (acg_intType == 4) {
				ff_cg = patch->f[Results::vdw22][i] - patch->f[Results::vdw22_rep][i] + patch->f[Results::elect22][i];
			}
			if (simParams->useDihedOn) {
				patch->f[Results::normal][i] += factor_aa * patch->f[Results::amdf][i];
			}
			if (doNonbonded) {
				patch->f[Results::nbond][i] *= (1.0 + factor_aa);
			}
			if (doFullElectrostatics) {
				patch->f[Results::slow][i] *= (1.0 + factor_aa);
			}
			if (aip == simParams->cgGroup) {
				patch->f[Results::normal][i] += factor_cg * ff_cg;
			}
		} else {
			if (acg_intType == 0) {
				ff_aa = patch->f[Results::vdw11][i];
				ff_cg = patch->f[Results::vdw22][i];
			} else if (acg_intType == 1) {
				ff_aa = patch->f[Results::elect11][i];
				ff_cg = patch->f[Results::elect22][i];
			} else if (acg_intType == 2) {
				ff_aa = patch->f[Results::vdw11][i] + patch->f[Results::elect11][i];
				ff_cg = patch->f[Results::vdw22][i] + patch->f[Results::elect22][i];
			} else if (acg_intType == 3) {
				ff_aa = patch->f[Results::vdw11][i] - patch->f[Results::vdw11_rep][i];
				ff_cg = patch->f[Results::vdw22][i] - patch->f[Results::vdw22_rep][i];
			} else if (acg_intType == 4) {
				ff_aa = patch->f[Results::vdw11][i] - patch->f[Results::vdw11_rep][i] + patch->f[Results::elect11][i];
				ff_cg = patch->f[Results::vdw22][i] - patch->f[Results::vdw22_rep][i] + patch->f[Results::elect22][i];
			}
			if (aip == simParams->aaGroup || aip == simParams->cgGroup) {
				patch->f[Results::normal][i] += factor_aa * ff_aa;
			}
			if (aip == simParams->cgGroup) {
				patch->f[Results::normal][i] += factor_cg * ff_cg;
			}
...
void Sequencer::reassignVelocities(BigReal timestep, int step) {
	...
	if ((reassignFreq > 0) && !(step % reassignFreq)) {
		...
		for (int i = 0; i < numAtoms; ++i) {
			a[i].velocity = (
					(simParams->fixedAtomsOn && a[i].atomFixed && a[i].mass > 0.) ?
							Vector(0, 0, 0) :
							sqrt(kbT * (a[i].partition ? tempFactor : 1.0) / a[i].mass) * random->gaussian_vector());
		}
	} else {
...
//lyk
void Sequencer::loadAdaptCgParam() {
	//iout<<"##################BEGIN:loadAdaptCgParam##################"<<"\n"<<endi;
	//iout<<"##################CALL FROM loadAdaptCgParam FUNCTION IN TCL SCRIPT##################"<<"\n"<<endi;
	FullAtom *a = patch->atom.begin();
	int numAtoms = patch->numAtoms;
	Molecule *molecule = Node::Object()->molecule;
	for (int i = 0; i < numAtoms; ++i) {
		float charge=molecule->atomcgcharge(a[i].id);
		int type=molecule->atomcgtype(a[i].id);
		float mass=molecule->atomcgmass(a[i].id);
		if (type>=0) {
			a[i].cgCharge = charge;
			a[i].cgType = type;
			a[i].cgMass = mass;
...
void Sequencer::addForceToMomentum(BigReal dt, const int ftag, const int useSaved, int pressure) {
#if CMK_BLUEGENEL
...

	{
		...
		if (simParams->pairInteractionOn) {
			...
			if (simParams->drudeOn) {
				...
				drudeComKE *= 0.5;
				drudeBondKE *= 0.5;
				reduction->item(REDUCTION_DRUDECOM_CENTERED_KINETIC_ENERGY) += drudeComKE;
				reduction->item(REDUCTION_DRUDEBOND_CENTERED_KINETIC_ENERGY) += drudeBondKE;
			} // end drudeOn
...

	{
		...
		for (int i = 0; i < numAtoms; i += hgs) {
			...
			if (simParams->pairInteractionOn) {
				...
				for (j = i; j < (i + hgs); ++j) {
					if (a[j].partition != 1 && (pairInteractionSelf || a[j].partition != 2))
						continue;
...
void Sequencer::runComputeObjects(int migration, int pairlists) {
	...
	int basePriority = ((seq & 0xffff) << 15) + PATCH_PRIORITY(patch->getPatchID());
