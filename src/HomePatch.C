...
int average(CompAtom *qtilde , const HGArrayVector &q , BigReal *lambda , const int n , const int m ,
		const HGArrayBigReal &imass , const HGArrayBigReal &length2 , const HGArrayInt &ial ,
		const HGArrayInt &ibl , const HGArrayVector &refab , const BigReal tolf , const int ntrial);

void mollify(CompAtom *qtilde , const HGArrayVector &q0 , const BigReal *lambda , HGArrayVector &force ,
		const int n , const int m , const HGArrayBigReal &imass , const HGArrayInt &ial ,
		const HGArrayInt &ibl , const HGArrayVector &refab);
...
HomePatch::HomePatch(PatchID pd , int atomCnt) :
		Patch(pd)
// DMK - Atom Separation (water vs. non-water)
#if NAMD_SeparateWaters != 0
,tempAtom()
#endif
{
	...
	doPairlistCheck_newTolerance = 0.5 * ( simParams->pairlistDist - simParams->cutoff );
...
HomePatch::HomePatch(PatchID pd , FullAtomList &al) :
		Patch(pd)
// DMK - Atom Separation (water vs. non-water)
#if NAMD_SeparateWaters != 0
,tempAtom()
...
void HomePatch::readPatchMap() {
	...
	patchMigrationCounter = numNeighbors = PatchMap::Object()->oneAwayNeighbors(patchID , nnPatchID);
	...
	for (int i = 0; i < 3; i++)
		for (int j = 0; j < 3; j++)
			for (int k = 0; k < 3; k++) {
				int pid = p->pid(p->index_a(patchID) + i - 1 , p->index_b(patchID) + j - 1 ,
						p->index_c(patchID) + k - 1);
				if (pid < 0) {
					DebugM(5 , "ERROR, for patchID " << patchID <<" I got neigh pid = " << pid << "\n");
				}
				if (pid == patchID
						&& !( ( ( i - 1 ) && p->periodic_a() ) || ( ( j - 1 ) && p->periodic_b() )
								|| ( ( k - 1 ) && p->periodic_c() ) )) {
					mInfo[i][j][k] = NULL;
				} else {
	...
	if (!--boxesOpen) {
		if (replacementForces) {
			...
			replacementForces = 0;
				...
				patchID << ": " << CthSelf() << " awakening sequencer " << sequencer->thread << "(" << patchID << ") @" << CmiTimer() << "\n");
		...
		if (flags.doGBIS) {
			if (flags.doNonbonded) {
				...
				if (numGBISP1Arrived == proxy.size() && numGBISP2Arrived == proxy.size()
...
void HomePatch::buildSpanningTree(void) {
	...
	int patchNodesLast = ( PatchMap::Object()->numNodesWithPatches() < ( 0.7 * CkNumPes() ) );
	int nNonPatch = 0;
#if 1
...
void HomePatch::positionsReady(int doMigration) {
	...
	if (npid) { //have proxies
		...
		ProxyDataMsg *nmsg = new (pdMsgPLLen , pdMsgAvgPLLen , pdMsgVLLen , intRadLen , lcpoTypeLen ,
				pdMsgPLExtLen , cudaAtomLen , PRIORITY_SIZE) ProxyDataMsg; // BEGIN LA, END LA
...
void HomePatch::redistrib_lp_force(Vector& fi , Vector& fj , Vector& fk , Vector& fl , const Vector& ri ,
		const Vector& rj , const Vector& rk , const Vector& rl , Tensor *virial , int midpt) {
 ...
 */
void HomePatch::redistrib_lp_water_force(Vector& f_ox , Vector& f_h1 , Vector& f_h2 , Vector& f_lp ,
		const Vector& p_ox , const Vector& p_h1 , const Vector& p_h2 , const Vector& p_lp , Tensor *virial) {
...
void HomePatch::reposition_lonepair(Vector& ri , const Vector& rj , const Vector& rk , const Vector& rl ,
...
void HomePatch::reposition_all_lonepairs(void) {
	...
	for (int i = 0; i < numAtoms; i++) {
		if (atom[i].mass < 0.01) {
			...
			reposition_lonepair(atom[i].position , atom[j.index].position , atom[k.index].position ,
					atom[l.index].position , lph->distance , lph->angle , lph->dihedral);
		}
	}
}

void HomePatch::swm4_omrepos(Vector *ref , Vector *pos , Vector *vel , BigReal invdt) {
...
void HomePatch::redistrib_lonepair_forces(const int ftag , Tensor *virial) {
	...
	for (int i = 0; i < numAtoms; i++) {
		if (atom[i].mass < 0.01) {
			...
			redistrib_lp_force(f_mod[ftag][i] , f_mod[ftag][j.index] , f_mod[ftag][k.index] ,
					f_mod[ftag][l.index] , atom[i].position , atom[j.index].position ,
...
void HomePatch::redistrib_swm4_forces(const int ftag , Tensor *virial) {
	...
	for (int i = 0; i < numAtoms; i++) {
		if (atom[i].mass < 0.01) {
			// found lonepair
			redistrib_lp_water_force(f_mod[ftag][i - 2] , f_mod[ftag][i + 1] , f_mod[ftag][i + 2] ,
					f_mod[ftag][i] , atom[i - 2].position , atom[i + 1].position , atom[i + 2].position ,
					atom[i].position , virial);
		}
	}
}
...
void HomePatch::redistrib_tip4p_forces(const int ftag , Tensor* virial) {
	...
	for (int i = 0; i < numAtoms; i++) {
		if (atom[i].mass < 0.01) {
			// found lonepair
			redistrib_lp_water_force(f_mod[ftag][i - 3] , f_mod[ftag][i - 2] , f_mod[ftag][i - 1] ,
					f_mod[ftag][i] , atom[i - 3].position , atom[i - 2].position , atom[i - 1].position ,
					atom[i].position , virial);
		}
	}
}

void HomePatch::addForceToMomentum(const BigReal timestep , const int ftag , const int useSaved) {
	SimParameters *simParams = Node::Object()->simParameters;
...
int HomePatch::hardWallDrude(const BigReal timestep , Tensor *virial , SubmitReduction *ppreduction) {
	Molecule *mol = Node::Object()->molecule;
	SimParameters *simParams = Node::Object()->simParameters;
	...
	Vector v_ab , vb_1 , vp_1 , vb_2 , vp_2 , new_vel_a , new_vel_b , new_pos_a , new_pos_b , *new_pos ,
			*new_vel;
				...
				if (rab_SQ > r_wall_SQ) {  // to impose the hard wall constraint
					...
					if (fixedAtomsOn && atom[ia].atomFixed) {  // the heavy atom is fixed
						if (atom[ib].atomFixed) {  // the drude is fixed too
							...
							dot_v_r_2 = atom[ib].velocity.x * v_ab.x + atom[ib].velocity.y * v_ab.y
									+ atom[ib].velocity.z * v_ab.z;
							vb_2 = dot_v_r_2 * v_ab;
							...
							if (dot_v_r_2 == 0.0) {
								...
								if (delta_T > maxtime)
									delta_T = maxtime; // make sure it is not crazy
							}
						...
						dot_v_r_1 = atom[ia].velocity.x * v_ab.x + atom[ia].velocity.y * v_ab.y
								+ atom[ia].velocity.z * v_ab.z;
						vb_1 = dot_v_r_1 * v_ab;
					...
					if (invdt == 0) {
						...
						for (j = 0; j < 2; j++) {
							...
							Force df = ( *new_vel - atom[Idx].velocity ) * ( atom[Idx].mass * invdt );
...
int HomePatch::rattle1(const BigReal timestep , Tensor *virial , SubmitReduction *ppreduction) {
	Molecule *mol = Node::Object()->molecule;
	SimParameters *simParams = Node::Object()->simParameters;
	const int fixedAtomsOn = simParams->fixedAtomsOn;
	...
	if (!settle_initialized) {
		for (int ig = 0; ig < numAtoms; ig += atom[ig].hydrogenGroupSize) {
			...
			if (atom[ig].rigidBondLength > 0) {
				...
				settle1init(atom[ig].mass , atom[oatm].mass , atom[ig].rigidBondLength ,
						atom[oatm].rigidBondLength , settle_mOrmT , settle_mHrmT , settle_ra , settle_rb ,
						settle_rc , settle_rra);
	...
	if (watmodel == WAT_TIP4)
		wathgsize = 4;
	else if (watmodel == WAT_SWM4)
		wathgsize = 5;

	for (int ig = 0; ig < numAtoms; ig += atom[ig].hydrogenGroupSize) {
		int hgs = atom[ig].hydrogenGroupSize;
		if (hgs == 1)
			continue;  // only one atom in group
		...
		if (( tmp = atom[ig].rigidBondLength ) > 0) {  // for water
			...
			if (useSettle && !atom[ig].groupFixed) {
				if (simParams->watmodel == WAT_SWM4) {
					...
					pos[2] = pos[0];
					vel[2] = vel[0];
					settle1(ref + 2 , pos + 2 , vel + 2 , invdt , settle_mOrmT , settle_mHrmT , settle_ra ,
					...
					settle1(ref , pos , vel , invdt , settle_mOrmT , settle_mHrmT , settle_ra , settle_rb ,
							settle_rc , settle_rra);
					if (simParams->watmodel == WAT_TIP4) {
				...
				1
						/ ( 1 / rhoi0
								- 1 / rhoi
										* tanh(
												psiFin[i]
														* ( delta + psiFin[i] * ( -beta + gamma * psiFin[i] ) )) );
...
void HomePatch::receiveResult(ProxyGBISP1ResultMsg *msg) {
	...
	if (flags.doNonbonded) {
		...
		if (phase1BoxClosedCalled == true && numGBISP1Arrived == proxy.size()) {
			sequencer->awaken();
		}
	} else {
		//awaken if all phases done on noWork step
		if (boxesOpen == 0 && numGBISP1Arrived == proxy.size() && numGBISP2Arrived == proxy.size()
				&& numGBISP3Arrived == proxy.size()) {
			sequencer->awaken();
		}
	}
...
void HomePatch::receiveResult(ProxyGBISP2ResultMsg *msg) {
	...
	if (flags.doNonbonded) {
		...
		if (phase2BoxClosedCalled == true && numGBISP2Arrived == proxy.size()) {
			sequencer->awaken();
		}
	} else {
		//awaken if all phases done on noWork step
...
void HomePatch::doPairlistCheck() {
	...
	if (max_tol > ( ( 1. - simParams->pairlistTrigger ) * doPairlistCheck_newTolerance )) {
...
void HomePatch::doGroupSizeCheck() {
	...
	SimParameters *simParams = Node::Object()->simParameters;
	BigReal hgcut = 0.5 * simParams->hgroupCutoff;
	hgcut *= hgcut;
	BigReal maxrad2 = 0.;
	...
	while (p_i != p_e) {
		...
		int ngs = hgs;
		if (ngs > 5)
			ngs = 5;  // limit to at most 5 atoms per group
...
void HomePatch::doMarginCheck() {
	...
	if (( aAwayDist * sysdima < minSize * 0.9999 ) || ( bAwayDist * sysdimb < minSize * 0.9999 )
	...
	while (atom_i != atom_e) {
		if (atom_i->migrationGroupSize) {
			...
			if (atom_i->migrationGroupSize != atom_i->hydrogenGroupSize) {
				...
				int c = 1;
				for (int j = atom_i->hydrogenGroupSize; j < mgs; j += ( atom_i + j )->hydrogenGroupSize) {
		...
		if (mInfo[xdev][ydev][zdev]) { // process atom for migration
			...
			DebugM(3 ,
					"Migrating atom " << atom_i->id << " from patch " << patchID << " with position " << atom_i->position << "\n");
			mCur.add(*atom_i);
...
void HomePatch::depositMigration(MigrateAtomsMsg *msg) {
	...
	// added to this patch's list.
	{
		...
		for (mi = migrationList.begin(); mi != migrationList.end(); mi++) {
			DebugM(1 ,
					"Migrating atom " << mi->id << " to patch " << patchID << " with position " << mi->position << "\n");
			if (mi->migrationGroupSize) {
				if (mi->migrationGroupSize != mi->hydrogenGroupSize) {
					...
					int c = 1;
					for (int j = mi->hydrogenGroupSize; j < mgs; j += ( mi + j )->hydrogenGroupSize) {
						pos += ( mi + j )->position;
						++c;
					}
					pos *= 1. / c;
...
inline void lubksb(HGMatrixBigReal &a , int n , HGArrayInt &indx , HGArrayBigReal &b) {
	int i , ii = -1 , ip , j;
	double sum;
	...
	for (i = 0; i < n; i++) {
		...
		for (j = 0; j < n; j++)
			if (( temp = fabs(a[i][j]) ) > big)
				big = temp;
		if (big == 0.0)
			NAMD_die("Singular matrix in routine ludcmp\n");
		vv[i] = 1.0 / big;
	}
	for (j = 0; j < n; j++) {
		for (i = 0; i < j; i++) {
...
inline void G_q(const HGArrayVector &refab , HGMatrixVector &gqij , const int n , const int m ,
		const HGArrayInt &ial , const HGArrayInt &ibl) {
	int i;
