...
void ExclElem::computeForce(BigReal *reduction , BigReal *pressureProfileData) {
	...
	if (modified && r2 < 1.0)
		r2 = 1.0;  // match CUDA interpolation
	...
	if (modified) {
		...
		const LJTable::TableEntry * lj_pars = ljTable->table_row(p_i.vdwType) + 2 * p_j.vdwType;

		// modified - normal = correction
		const BigReal A = scaling * ( ( lj_pars + 1 )->A - lj_pars->A );
				//* rescaleVdwsForce(localIndex[0] , localIndex[1]);
		...
		if (doEnergy) {
			reduction[vdwEnergyIndex] -= ( ( diffa * ( 1. / 6. ) * vdw_d + 0.25 * vdw_c ) * diffa
					+ 0.5 * vdw_b ) * diffa + vdw_a;
			reduction[electEnergyIndex] -= ( ( diffa * ( 1. / 6. ) * fast_d + 0.25 * fast_c ) * diffa
					+ 0.5 * fast_b ) * diffa + fast_a;
			if (doFull) {
				reduction[fullElectEnergyIndex] -= ( ( diffa * ( 1. / 6. ) * slow_d + 0.25 * slow_c ) * diffa
