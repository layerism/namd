...
void Molecule::print_go_params() {
	...
	for (i = 0; i < NumGoChains; i++) {
		for (j = 0; j < NumGoChains; j++) {
			...
			DebugM(3 ,
					"Go index=(" << i << "," << j << ") epsilon=" << go_array[index].epsilon << " exp_a=" << go_array[index].exp_a << " exp_b=" << go_array[index].exp_b << " exp_rep=" << go_array[index].exp_rep << " sigmaRep=" << go_array[index].sigmaRep << " epsilonRep=" << go_array[index].epsilonRep << " cutoff=" << go_array[index].cutoff << std::endl);
		}
	}
...
void Molecule::build_go_sigmas(StringList *goCoordFile , char *cwd) {
		...
		for (j = i + 1; j < numAtoms; j++) {
			...
			if (goSigmaIndices[i] != -1 && goSigmaIndices[j] != -1 && !atoms_1to4(i , j)) {
				...
				if (atomChainTypes[i] && atomChainTypes[j]
					...
					atomAtomDist = sqrt(
							pow(( goPDB->atom(i) )->xcoor() - ( goPDB->atom(j) )->xcoor() , 2.0)
					...
					if (atomAtomDist <= this->get_go_cutoff(atomChainTypes[i] , atomChainTypes[j])) {
						...
						sigma = pow(static_cast < double >(exp_b / exp_a) , ( 1.0 / ( exp_a - exp_b ) ))
								* atomAtomDist;
...
void Molecule::build_go_sigmas2(StringList *goCoordFile , char *cwd) {
			...
			for (j = i + 1; j < numAtoms; j++) {
				...
				if (goIndx != -1) {
					...
					if (atomChainTypes[goIndex] && atomChainTypes[goIndx]
							&& !( this->go_restricted(atomChainTypes[goIndex] , atomChainTypes[goIndx] ,
									residDiff) ) && !atoms_1to4(i , j)) {
						atomAtomDist = sqrt(
								pow(( goPDB->atom(i) )->xcoor() - ( goPDB->atom(j) )->xcoor() , 2.0)
										+ pow(( goPDB->atom(i) )->ycoor() - ( goPDB->atom(j) )->ycoor() , 2.0)
										+ pow(( goPDB->atom(i) )->zcoor() - ( goPDB->atom(j) )->zcoor() ,
												2.0));
						if (atomAtomDist
								<= this->get_go_cutoff(atomChainTypes[goIndex] , atomChainTypes[goIndx])) {
							nativeContacts++;
						} else {
							nonnativeContacts++;
						}
...
void Molecule::print_go_sigmas() {
	...
	DebugM(3 , "GO SIGMA ARRAY\n" << "***************************" << std::endl);DebugM(3 ,
			"numGoAtoms: " << numGoAtoms << std::endl);
...
// JLai
BigReal Molecule::get_gro_force2(BigReal x , BigReal y , BigReal z , int atom1 , int atom2 ,
		BigReal* pairLJEnergy , BigReal* pairGaussEnergy) const {
	...
	*pairGaussEnergy = 0.0;
	...
	if (LJIndex == -1 && GaussIndex == -1) {
		...
		if (GaussIndex != -1) {
			...
			Gauss = gr * one_gauss1 * one_gauss2
					- 2 * A * tmp_gauss1 * one_gauss2 * r1prime * giSigma1[GaussIndex]
					- 2 * tmp_gauss1 * one_gauss2 * r1prime * giSigma1[GaussIndex] * gRepulsive[GaussIndex]
							* ri12 - 2 * A * tmp_gauss2 * one_gauss1 * r2prime * giSigma2[GaussIndex]
					- 2 * tmp_gauss2 * one_gauss1 * r2prime * giSigma2[GaussIndex] * gRepulsive[GaussIndex]
							* ri12;
			*pairGaussEnergy = A
					* ( -1 + ( one_gauss1 ) * ( one_gauss2 ) * ( 1 + gRepulsive[GaussIndex] * ri12 / A ) );
		}
...
// JE
BigReal Molecule::get_go_force(BigReal r , int atom1 , int atom2 , BigReal* goNative ,
...
/************************************************************************/
// JE
BigReal Molecule::get_go_force_new(BigReal r , int atom1 , int atom2 , BigReal* goNative ,
		BigReal* goNonnative) const {
	int resid1;
	int resid2;
	int residDiff;
	...
	if (goIndex1 != -1 && goIndex2 != -1) {
		...
		if (!( const_cast < Molecule* >(this)->go_restricted(chain1 , chain2 , residDiff) )) {
			...
			if (atomAtomDist <= const_cast < Molecule* >(this)->get_go_cutoff(chain1 , chain2)) {
				...
				sigma_ij = pow(static_cast < double >(exp_b / exp_a) , ( 1.0 / ( exp_a - exp_b ) ))
						* atomAtomDist;
				...
				goForce = ( ( 4 / ( r * r ) ) * epsilon * ( exp_a * pow1 - exp_b * pow2 ) );
				DebugM(3 ,
						"get_go_force - (" << atom1 << "," << atom2 << ") chain1:" << chain1 << ", chain2:" << chain2 << ", exp_a:" << exp_a << ", exp_b:" << exp_b << ", sigma_ij:" << sigma_ij << ", r:" << r << ", goForce:" << goForce << std::endl);
			...
			//  if repulsive then calculate repulsive
				...
				DebugM(3 ,
						"get_go_force - (" << atom1 << "," << atom2 << ") chain1:" << chain1 << ", chain2:" << chain2 << ", epsilonRep:" << epsilonRep << ", sigmaRep:" << sigmaRep << ", r:" << r << ", goForce:" << goForce << std::endl);
...
/************************************************************************/
// JLai
BigReal Molecule::get_go_force2(BigReal x , BigReal y , BigReal z , int atom1 , int atom2 ,
		BigReal *goNative , BigReal *goNonnative) const {
	...
	if (LJIndex == -1) {
		...
		*goNonnative = ( 4 * epsilonRep * sigmaRepPow * pow(r , -exp_rep) );
		...
		BigReal LJ = 4 * epsilon
				* ( exp_a * goSigmaPairA[LJIndex] * powA - exp_b * goSigmaPairB[LJIndex] * powB );
...
/************************************************************************/
// JE
Bool Molecule::atoms_1to4(unsigned int atom1 , unsigned int atom2) {
	...
	while (angleNum != -1) {
		...
		DebugM(2 ,
				"angle  atom1:" << angle->atom1 << ", atom2:" << angle->atom2 << ", atom3:" << angle->atom3 << std::endl);
	...
	while (dihedralNum != -1) {
		...
		DebugM(2 ,
				"dihedral  atom1:" << dihedral->atom1 << ", atom2:" << dihedral->atom2 << ", atom3:" << dihedral->atom3 << ", atom4:" << dihedral->atom4 << std::endl);
		if (atom2 == dihedral->atom1 || atom2 == dihedral->atom2 || atom2 == dihedral->atom3
				|| atom2 == dihedral->atom4) {
			return TRUE;
		}
		dihedralNum = *( ++dihedrals );
	}
	...
	while (dihedralNum != -1) {
		...
		DebugM(2 ,
				"dihedral  atom1:" << dihedral->atom1 << ", atom2:" << dihedral->atom2 << ", atom3:" << dihedral->atom3 << ", atom4:" << dihedral->atom4 << std::endl);
		if (atom1 == dihedral->atom1 || atom1 == dihedral->atom2 || atom1 == dihedral->atom3
				|| atom1 == dihedral->atom4) {
			return TRUE;
		}
		dihedralNum = *( ++dihedrals );
	}
