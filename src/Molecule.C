...
int ResidueLookupElem::lookup(const char *segid, int resid, int *begin, int *end) const {
	const ResidueLookupElem *elem = this;
	int rval = -1;  // error
	while (elem && strcasecmp(elem->mySegid, segid))
		elem = elem->next;
...
ResidueLookupElem* ResidueLookupElem::append(const char *segid, int resid, int aid) {
...
int Molecule::get_atom_from_name(const char *segid, int resid, const char *aname) const {
	...
	int i = 0;
	int end = 0;
	if (resLookup->lookup(segid, resid, &i, &end))
		return -1;
	for (; i < end; ++i) {
#ifdef MEM_OPT_VERSION
...
#else
		if (!strcasecmp(aname, atomNames[i].atomname))
			return i;
#endif
	}
	return -1;
...
/*                                                                      */
/************************************************************************/
Molecule::Molecule(SimParameters *simParams, Parameters *param, molfile_plugin_t *pIOHdl, void *pIOFileHdl,
		int natoms) {
#ifdef MEM_OPT_VERSION
	...
	if (pIOHdl->read_bonds != NULL) {
		if (pIOHdl->read_bonds(pIOFileHdl, &numBonds, &from, &to, &bondorder, &bondtype, &nbondtypes, &bondtypename)) {
	...
	if (pIOHdl->read_angles != NULL) {
		if (pIOHdl->read_angles(pIOFileHdl, &numAngles, &plgAngles, &angletypes, &numangletypes, &angletypenames,
				&numDihedrals, &plgDihedrals, &dihedraltypes, &numdihedraltypes, &dihedraltypenames, &numImpropers,
				&plgImpropers, &impropertypes, &numimpropertypes, &impropertypenames, &numCrossterms, &plgCterms,
				&ctermcols, &ctermrows)) {
	...
	if (numAngles != 0)
		plgLoadAngles(plgAngles);
	if (numDihedrals != 0)
		plgLoadDihedrals(plgDihedrals);
	if (numImpropers != 0)
		plgLoadImpropers(plgImpropers);
	if (numCrossterms != 0)
		plgLoadCrossterms(plgCterms);
...

{
	...
	if (!NAMD_find_word(buffer, "psf")) {
		sprintf(err_msg, "UNABLE TO FIND \"PSF\" STRING IN PSF FILE %s", fname);
		NAMD_die(err_msg);
	}
	...
	if (!NAMD_find_word(buffer, "NTITLE")) {
		sprintf(err_msg, "CAN NOT FIND \"NTITLE\" STRING IN PSF FILE %s", fname);
		NAMD_die(err_msg);
	}
	...
	while (((ret_code = NAMD_read_line(psf_file, buffer)) == 0) && (i < NumTitle)) {
	...
	if (ret_code != 0) {
		sprintf(err_msg, "FOUND EOF INSTEAD OF NATOM IN PSF FILE %s", fname);
		NAMD_die(err_msg);
	}
	...
	if (!NAMD_find_word(buffer, "NATOM")) {
		sprintf(err_msg, "DIDN'T FIND \"NATOM\" IN PSF FILE %s", fname);
		NAMD_die(err_msg);
	}
...

{
	...
	while (atom_number < numAtoms) {
		...
		/*  Parse up the line          */
		read_count = sscanf(buffer, "%d %s %s %s %s %s %f %f", &atom_number, segment_name, residue_number, residue_name,
				atom_name, atom_type, &charge, &mass);
...
//          "%*d %*s %*s %*s %*s %*s %*f %*f %*d %*f %*f %f %f", &alpha, &thole);
		...
		if (atom_number != last_atom_number + 1) {
			...
			sprintf(err_msg, "ATOM NUMBERS OUT OF ORDER AT ATOM #%d OF PSF FILE", last_atom_number + 1);
			NAMD_die(err_msg);
		}
		...
		if (tmpResLookup)
			tmpResLookup = tmpResLookup->append(segment_name, atoi(residue_number), atom_number - 1);
		...
		} else if ((atomNames[atom_number - 1].atomname[0] == 'O') && (atoms[atom_number - 1].mass >= 14.0)
				&& (atoms[atom_number - 1].mass <= 18.0)) {
			atoms[atom_number - 1].status |= OxygenAtom;
		}

		/*  Look up the vdw constants for this atom    */
		params->assign_vdw_index(atomNames[atom_number - 1].atomtype, &(atoms[atom_number - 1]));
	}
	...
	while (num_read < numBonds) {
		...
		for (j = 0; j < 2; j++) {
			...
			if (atom_nums[j] >= numAtoms) {
				...
				sprintf(err_msg, "BOND INDEX %d GREATER THAN NATOM %d IN BOND # %d IN PSF FILE", atom_nums[j] + 1,
						numAtoms, num_read + 1);
				NAMD_die(err_msg);
			}
		}
		...
		if (strcasecmp(atomNames[atom_nums[0]].atomtype, atomNames[atom_nums[1]].atomtype) < 0) {
		...
		if (simParams->lonepairs) {
			...
			if (k == 0. && !is_lp(b->atom1) && !is_lp(b->atom2))
				--numBonds;
			else
				++num_read;
		} else {
	...
	while (num_read < numAngles) {
		...
		for (j = 0; j < 3; j++) {
			...
			if (atom_nums[j] >= numAtoms) {
				...
				sprintf(err_msg, "ANGLES INDEX %d GREATER THAN NATOM %d IN ANGLES # %d IN PSF FILE", atom_nums[j] + 1,
						numAtoms, num_read + 1);
				NAMD_die(err_msg);
			}
		}
		...
		/*  to be predictable.          */
		if (strcasecmp(atomNames[atom_nums[0]].atomtype, atomNames[atom_nums[2]].atomtype) < 0) {
			strcpy(atom1name, atomNames[atom_nums[0]].atomtype);
			strcpy(atom2name, atomNames[atom_nums[1]].atomtype);
		...
		/*  parameter object          */
		params->assign_angle_index(atom1name, atom2name, atom3name, &(angles[num_read]), simParams->alchOn ? -1 : 0);
	...
	while (num_read < numDihedrals) {
		...
		for (j = 0; j < 4; j++) {
			...
			if (atom_nums[j] >= numAtoms) {
				...
				sprintf(err_msg, "DIHEDRALS INDEX %d GREATER THAN NATOM %d IN DIHEDRALS # %d IN PSF FILE",
						atom_nums[j] + 1, numAtoms, num_read + 1);
				NAMD_die(err_msg);
			}
		...
		params->assign_dihedral_index(atom1name, atom2name, atom3name, atom4name, &(dihedrals[num_unique - 1]),
	...
	while (num_read < numImpropers) {
		...
		for (j = 0; j < 4; j++) {
			...
			if (atom_nums[j] >= numAtoms) {
				...
				sprintf(err_msg, "IMPROPERS INDEX %d GREATER THAN NATOM %d IN IMPROPERS # %d IN PSF FILE",
						atom_nums[j] + 1, numAtoms, num_read + 1);
				NAMD_die(err_msg);
			}
		...
		params->assign_improper_index(atom1name, atom2name, atom3name, atom4name, &(impropers[num_unique - 1]),
				multiplicity);
	...
	while (num_read < numCrossterms) {
		...
		for (j = 0; j < 8; j++) {
			...
			if (atom_nums[j] >= numAtoms) {
				...
				sprintf(err_msg, "CROSS-TERM INDEX %d GREATER THAN NATOM %d IN CROSS-TERMS # %d IN PSF FILE",
						atom_nums[j] + 1, numAtoms, num_read + 1);
				NAMD_die(err_msg);
			}
		...
		/*  Look up the constants for this bond      */
		params->assign_crossterm_index(atom1name, atom2name, atom3name, atom4name, atom5name, atom6name, atom7name,
				atom8name, &(crossterms[num_read]));
	...
	while (num_read < numDonors) {
		...
		for (j = 0; j < 2; j++) {
			...
			if (d[j] >= numAtoms) {
				...
				sprintf(err_msg, "DONOR INDEX %d GREATER THAN NATOM %d IN DONOR # %d IN PSF FILE", d[j] + 1, numAtoms,
						num_read + 1);
				NAMD_die(err_msg);
			}
	...
	while (num_read < numAcceptors) {
		...
		for (j = 0; j < 2; j++) {
			...
			if (d[j] >= numAtoms) {
				...
				sprintf(err_msg, "ACCEPTOR INDEX %d GREATER THAN NATOM %d IN DONOR # %d IN PSF FILE", d[j] + 1,
						numAtoms, num_read + 1);
				NAMD_die(err_msg);
			}
...

{
	...
	for (num_read = 0; num_read < numExclusions; num_read++) {
		...
		if (exclusion_atoms[num_read] >= numAtoms) {
			...
			sprintf(err_msg, "EXCLUSION INDEX %d GREATER THAN NATOM %d IN EXCLUSION # %d IN PSF FILE",
					exclusion_atoms[num_read] + 1, numAtoms, num_read + 1);
			NAMD_die(err_msg);
		}
	}
	...
	for (num_read = 0; num_read < numAtoms; num_read++) {
		...
		if (current_index != last_index) {
			...
			/*  atom has          */
			for (insert_index = last_index; insert_index < current_index; insert_index++) {
...
void Molecule::read_lphosts(FILE *fd) {
	...
	for (i = 0; i < numLphosts; i++) {
		...
		if ((NAMD_blank_string(buffer)) || (buffer[0] == '!'))
			continue;
		read_count = sscanf(buffer, "%d %d %6s %f %f %f", &numhosts, &index, lptype, &distance, &angle, &dihedral);
		if (read_count != 6 || numhosts != 3 || index != 4 * i + 1 || strcmp(lptype, "F") != 0) {
			char err_msg[128];
...
inline int getLCPOTypeCharmm(char atomType[11], int numBonds) {
	...
	if (atomType[0] == 'H') {
		...
		if ( //Sp3 Carbon
		atomType[1] == 'T' || strcmp(atomType, "CP1") == 0 || strcmp(atomType, "CP2") == 0
				|| strcmp(atomType, "CP3") == 0 || strcmp(atomType, "CS") == 0) {
			if (numBonds == 1)
				return 1;
			else if (numBonds == 2)
				return 2;
		...
		strcmp(atomType, "C") == 0 || strcmp(atomType, "CA") == 0 || strcmp(atomType, "CC") == 0
				|| strcmp(atomType, "CD") == 0 || strcmp(atomType, "CN") == 0 || strcmp(atomType, "CY") == 0
				|| strcmp(atomType, "C3") == 0 || strcmp(atomType, "CE1") == 0 || strcmp(atomType, "CE2") == 0
				|| strcmp(atomType, "CST") == 0 || strcmp(atomType, "CAP") == 0 || strcmp(atomType, "COA") == 0
				|| strcmp(atomType, "CPT") == 0 || strcmp(atomType, "CPH1") == 0 || strcmp(atomType, "CPH2") == 0) {
			if (numBonds == 2)
				return 5;
			else if (numBonds == 3)
				return 6;
		...
		strcmp(atomType, "NY") == 0
				...
				strcmp(atomType, "NH2") == 0
				|| //
				strcmp(atomType, "NR1") == 0 || strcmp(atomType, "NR2") == 0 || strcmp(atomType, "NR3") == 0
				|| strcmp(atomType, "NPH") == 0 || strcmp(atomType, "NC") == 0) {
			if (numBonds == 1)
				return 14;
			else if (numBonds == 2)
				return 15;
		...
		if ( //Sp3 Oxygen
		strcmp(atomType, "OH1") == 0 || strcmp(atomType, "OS") == 0 || strcmp(atomType, "OC") == 0 || //
		...
		strcmp(atomType, "O") == 0 || strcmp(atomType, "OB") == 0 || strcmp(atomType, "OST") == 0
				|| strcmp(atomType, "OCA") == 0 || strcmp(atomType, "OM") == 0) {
			return 9;
		} else if ( // SP1 Oxygen
...
void Molecule::plgLoadAtomBasics(molfile_atom_t *atomarray) {
	...
	for (int i = 0; i < numAtoms; i++) {
		...
		} else if ((atomNames[i].atomname[0] == 'O') && (atoms[i].mass >= 14.0) && (atoms[i].mass <= 18.0)) {
...
void Molecule::plgLoadBonds(int *from, int *to) {
	...
	for (int i = 0; i < numBonds; i++) {
		 ...
		 * first as atom 1.
		 */
		if (strcasecmp(atomNames[thisBond->atom1].atomtype, atomNames[thisBond->atom2].atomtype) < 0) {
		...
		if (simParams->lonepairs) {
			//need to retain Lonepair bonds for Drude
			if (k != 0. || is_lp(thisBond->atom1) || is_lp(thisBond->atom2)) {
				realNumBonds++;
			}
		} else {
			if (k != 0.)
...
void Molecule::plgLoadAngles(int *plgAngles) {
	...
	for (int i = 0; i < numAngles; i++) {
		...
		if (strcasecmp(atomNames[thisAngle->atom1].atomtype, atomNames[thisAngle->atom2].atomtype) < 0) {
		...
		params->assign_angle_index(atom1name, atom2name, atom3name, thisAngle, simParams->alchOn ? -1 : 0);
...
void Molecule::plgLoadDihedrals(int *plgDihedrals) {
	...
	for (int i = 0; i < numDihedrals; i++, atomid += 4) {
		...
		params->assign_dihedral_index(atom1name, atom2name, atom3name, atom4name, thisDihedral, multiplicity,
...
void Molecule::plgLoadImpropers(int *plgImpropers) {
	...
	for (int i = 0; i < numImpropers; i++, atomid += 4) {
		...
		params->assign_improper_index(atom1name, atom2name, atom3name, atom4name, thisImproper, multiplicity);
	}
...
//fepe
	...
	const int pair_self = simParams->pairInteractionOn ? simParams->pairInteractionSelf : 0;
		...
		for (int32 *b = bondsWithAtom[i]; *b != -1; ++b) {
			...
			if (a > i) {
				...
				while (cluster[ca] != ca)
					ca = cluster[ca];
				if (ca > ci)
					cluster[ca] = cluster[ci];
				else
					cluster[ci] = cluster[ca];
			}
		}
	}
	while (1) {
	...
	for (i = 0; i < numBonds; i++) {
		if (numFixedAtoms && fixedAtomFlags[bonds[i].atom1] && fixedAtomFlags[bonds[i].atom2])
			continue;

		if (pair_self && fepAtomFlags[bonds[i].atom1] != 1)
			continue;
		byAtomSize[bonds[i].atom1]++;
		numCalcBonds++;
	}
	...
	for (i = 0; i < numAngles; i++) {
		if (numFixedAtoms && fixedAtomFlags[angles[i].atom1] && fixedAtomFlags[angles[i].atom2]
				&& fixedAtomFlags[angles[i].atom3])
			continue;
		if (pair_self && fepAtomFlags[angles[i].atom1] != 1)
			continue;
	...
	for (i = 0; i < numImpropers; i++) {
		if (numFixedAtoms && fixedAtomFlags[impropers[i].atom1] && fixedAtomFlags[impropers[i].atom2]
				&& fixedAtomFlags[impropers[i].atom3] && fixedAtomFlags[impropers[i].atom4])
			continue;
		if (pair_self && fepAtomFlags[impropers[i].atom1] != 1)
			continue;
	...
	for (i = 0; i < numDihedrals; i++) {
		if (numFixedAtoms && fixedAtomFlags[dihedrals[i].atom1] && fixedAtomFlags[dihedrals[i].atom2]
				&& fixedAtomFlags[dihedrals[i].atom3] && fixedAtomFlags[dihedrals[i].atom4])
			continue;
		if (pair_self && fepAtomFlags[dihedrals[i].atom1] != 1)
			continue;
	...
	for (i = 0; i < numCrossterms; i++) {
		if (numFixedAtoms && fixedAtomFlags[crossterms[i].atom1] && fixedAtomFlags[crossterms[i].atom2]
				&& fixedAtomFlags[crossterms[i].atom3] && fixedAtomFlags[crossterms[i].atom4]
				&& fixedAtomFlags[crossterms[i].atom5] && fixedAtomFlags[crossterms[i].atom6]
				&& fixedAtomFlags[crossterms[i].atom7] && fixedAtomFlags[crossterms[i].atom8])
			continue;
		if (pair_self && fepAtomFlags[crossterms[i].atom1] != 1)
			continue;
	...
	for (i = 0; i < numTotalExclusions; i++) {
		if (numFixedAtoms && fixedAtomFlags[exclusions[i].atom1] && fixedAtomFlags[exclusions[i].atom2])
			continue;
	...
	for (i = 0; i < numTotalExclusions; i++) {
		...
		if (numFixedAtoms && fixedAtomFlags[a1] && fixedAtomFlags[a2])
			continue;
		int32 *l1, *l2;
		if (exclusions[i].modified) {
	...
	if (is_drude_psf || simParams->drudeOn) {
		...
		for (i = 0; i < numTholes; i++) {
			if (numFixedAtoms && fixedAtomFlags[tholes[i].atom1] && fixedAtomFlags[tholes[i].atom2]
					&& fixedAtomFlags[tholes[i].atom3] && fixedAtomFlags[tholes[i].atom4])
				continue;
			if (pair_self && fepAtomFlags[tholes[i].atom1] != 1)
				continue;
		...
		numCalcAnisos = 0;
		for (i = 0; i < numAnisos; i++) {
			if (numFixedAtoms && fixedAtomFlags[anisos[i].atom1] && fixedAtomFlags[anisos[i].atom2]
					&& fixedAtomFlags[anisos[i].atom3] && fixedAtomFlags[anisos[i].atom4])
				continue;
			if (pair_self && fepAtomFlags[anisos[i].atom1] != 1)
				continue;
	...
	for (i = 0; i < numTotalExclusions; i++) {
		...
		if (numFixedAtoms && fixedAtomFlags[a1] && fixedAtomFlags[a2])
			continue;
		if (all_exclusions[a1].min > a2)
			all_exclusions[a1].min = a2;
		if (all_exclusions[a2].min > a1)
			all_exclusions[a2].min = a1;
		if (a2 > all_exclusions[a1].max)
			all_exclusions[a1].max = a2;
		if (a1 > all_exclusions[a2].max)
			all_exclusions[a2].max = a1;
	}
	...
	for (i = 0; i < maxDenseAllFull; ++i)
		denseFullArray[i] = EXCHCK_FULL;
	...
	for (i = 0; i < numTotalExclusions; i++) {
		...
		if (exclusions[i].modified) {
			if (all_exclusions[a1].flags)
				all_exclusions[a1].flags[a2 - all_exclusions[a1].min] =
				EXCHCK_MOD;
			if (all_exclusions[a2].flags)
				all_exclusions[a2].flags[a1 - all_exclusions[a2].min] =
				EXCHCK_MOD;
		} else {
			if (all_exclusions[a1].flags)
				all_exclusions[a1].flags[a2 - all_exclusions[a1].min] =
...
void Molecule::build_exclusions() {
	...
	if (!simParams->amberOn || !simParams->readExclusions) { //  Now calculate the bonded exlcusions based on the exclusion policy
		switch (exclude_flag) {
		case NONE:
			break;
		case ONETWO:
...
void Molecule::build_inherited_excl(int modified) {
	...
	for (i = 0; i < numAtoms; i++) {
		...
		if (-1 != *(bond1 + 1)) {  // and only one bond
			char err_msg[512];
			const char *idescrip = (is_drude(i) ? "DRUDE" : "LONE PAIR");
			sprintf(err_msg, "FOUND MULTIPLY LINKED %s PARTICLE %d", idescrip, i + 1);
			NAMD_die(err_msg);
		}
		...
		if (exclude_flag == NONE) {
			...
			while (*bond2 != -1) {
				...
				if ((is_drude(j) || is_lp(j)) && j != mid1) {
					if (i < j)
						exclusionSet.add(Exclusion(i, j));
					else if (j < i)
						exclusionSet.add(Exclusion(j, i));
				}
				j = bonds[*bond2].atom2;
				if ((is_drude(j) || is_lp(j)) && j != mid1) {
					if (i < j)
							...
							while (*bond4 != -1) {
								...
								while (*bond5 != -1) {
									...
									if ((is_drude(j) || is_lp(j)) && j != mid4) {
										if (i < j)
											exclusionSet.add(Exclusion(i, j, modi));
										else if (j < i)
											exclusionSet.add(Exclusion(j, i, modi));
									}
									j = bonds[*bond5].atom2;
									if ((is_drude(j) || is_lp(j)) && j != mid4) {
										if (i < j)
...
void Molecule::build14excl(int modified) {
	...
	for (i = 0; i < numAtoms; i++) {
		if (is_drude(i) || is_lp(i))
			continue;  // skip Drude and LP for now
	...
	if (simParams->alchFepOn || simParams->alchThermIntOn || simParams->lesOn) {
		...
		for (exclIter = exclIter.begin(); exclIter != exclIter.end(); exclIter++) {
			...
			if (simParams->pairInteractionSelf) {
				...
				if (!(ifep_type == 1 && jfep_type == 2) && !(ifep_type == 2 && jfep_type == 1)) {
					fepExclusionSet.add(*exclIter);
				}
			}
		}
...
void Molecule::print_atoms(Parameters *params) {
	...
	for (i = 0; i < numAtoms; i++) {
		params->get_vdw_params(&sigma, &epsilon, &sigma14, &epsilon14, atoms[i].vdw_type);

		DebugM(2,
				i+1 << " " << atomNames[i].atomname << " " << atomNames[i].atomtype << " " << atomNames[i].resname << " " << atoms[i].mass << " " << atoms[i].charge << " " << sigma << " " << epsilon << " " << sigma14 << " " << epsilon14 << "\n" << endi);
	}
#endif
}
...
void Molecule::print_bonds(Parameters *params) {
	...
	DebugM(2,
			"BOND LIST\n" << "********************************\n" << "ATOM1 ATOM2 TYPE1 TYPE2      k        x0" << endi);

	for (i = 0; i < numBonds; i++) {
		params->get_bond_params(&k, &x0, bonds[i].bond_type);

		DebugM(2,
				bonds[i].atom1+1 << " " << bonds[i].atom2+1 << " " << atomNames[bonds[i].atom1].atomtype << " " << atomNames[bonds[i].atom2].atomtype << " " << k << " " << x0 << endi);
	}
...
void Molecule::print_exclusions() {
	...
	for (i = 0; i < numExclusions; i++) {
		DebugM(2, exclusions[i].atom1+1 << "  " << exclusions[i].atom2+1 << endi);
	}
#endif
}
...
//to other Nodes. --Chao Mei
	...
	if (simParams->alchFepOn || simParams->alchThermIntOn || simParams->lesOn || simParams->pairInteractionOn
			|| simParams->itsOn || simParams->adaptCgOn) {
...
void Molecule::build_gridforce_params(StringList *gridfrcfile, StringList *gridfrccol, StringList *gridfrcchrgcol,
		StringList *potfile, PDB *initial_pdb, char *cwd) {
	PDB *kPDB;
		...
		char *cwd, const char *simmethod)
{
	...
	for (i = 0; i < numAtoms; i++) {
		...
		if (simParams->lesOn) {
			...
			fepAtomFlags[i] = (int) bval;
		} else if (simParams->itsOn || simParams->adaptCgOn) {	//lyk
			if ( bval == (int) bval && bval > 0 ) {
				fepAtomFlags[i] = (int) bval;
				numFepFinal++;
				numFepInitial++;
...
//lyk
void Molecule::loadAdaptCgParam(int atomIndex[], float charge[],float mass[],int type[],
		float vdwEps[],float vdwSgm[],int aid2MapId[],bool exclude[], int n, int ntype, int maxId) {

	//init
	for( int i=0; i<numAtoms; ++i ) {
		atoms[i].cgCharge = 0.0;
		atoms[i].cgType = -1;
		atoms[i].cgMass=0.0;
	}
	...
	nCg=n;
	cgMaxId=maxId;
	cgid2MapId = new int [maxId];
	for (int cgid=0; cgid<cgMaxId; ++cgid) {
		cgid2MapId[cgid]=aid2MapId[cgid];
	}

	for( int i=0; i<n; ++i ) {
		int id=atomIndex[i];	//0 base
		atoms[id].cgCharge = charge[i];
		atoms[id].cgType = type[i];
		atoms[id].cgMass=mass[i];
	}

	nCgType=ntype;
	cgVdwEps=new float [ntype*ntype];
	cgVdwSgm=new float [ntype*ntype];
	for (int i=0; i<ntype*ntype; ++i) {
		cgVdwEps[i]=vdwEps[i];
		cgVdwSgm[i]=vdwSgm[i];
	}

	unsigned long maxExcludePair = n * n;
	cgExclude=new bool [maxExcludePair];

	for (unsigned long i=0; i<maxExcludePair; ++i) {
		cgExclude[i] = exclude[i];
	}
	//iout<<cgVdwEps[5*nCgType+2]<<" "<<cgVdwEps[2*nCgType+5]<<"\n"<<endi;
}
//lyk
