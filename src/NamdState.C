...
int NamdState::configListInit(ConfigList *cfgList) {
	...
	if (simParameters->amberOn) {
		...
		void *plgFile = pIOHandle->open_file_read(moleculeFilename->data , pIOHandle->name , &numAtoms);
		if (plgFile == NULL) {
		...
		if (!simParameters->genCompressedPsf) {
			...
			pdb = new PDB(pIOHandle , plgFile , molecule->numAtoms , molecule->getOccupancyData() ,
					molecule->getBFactorData());
	...
	if (simParameters->constraintsOn) {
		...
		molecule->build_constraint_params(consRefFile , consKFile , configList->find("conskcol") , pdb ,
		NULL);
	}
	...
	if (simParameters->stirOn) {
		...
		molecule->build_stirred_atoms(configList->find("stirFilename") , configList->find("stirredAtomsCol") ,
				pdb ,
				NULL);
	}

	if (simParameters->fixedAtomsOn) {
		molecule->build_fixed_atoms(configList->find("fixedatomsfile") , configList->find("fixedatomscol") ,
				pdb ,
				NULL);
	}
	...
	if (simParameters->mgridforceOn) {
		molecule->build_gridforce_params(configList->find("gridforcefile") ,
				configList->find("gridforcecol") , configList->find("gridforcechargecol") ,
				configList->find("gridforcepotfile") , pdb ,
				NULL);
	}
	/* END gf */
	...
	if (simParameters->excludeFromPressure) {
		molecule->build_exPressure_atoms(configList->find("excludeFromPressureFile") ,
				configList->find("excludeFromPressureCol") , pdb , NULL);
	}

	// If moving drag is active, build the parameters necessary
	if (simParameters->movDragOn) {
		molecule->build_movdrag_params(configList->find("movDragFile") , configList->find("movDragCol") ,
				configList->find("movDragVelFile") , pdb ,
				NULL);
	}

	// If rotating drag is active, build the parameters necessary
	if (simParameters->rotDragOn) {
		molecule->build_rotdrag_params(configList->find("rotDragFile") , configList->find("rotDragCol") ,
				configList->find("rotDragAxisFile") , configList->find("rotDragPivotFile") ,
				configList->find("rotDragVelFile") , configList->find("rotDragVelCol") , pdb ,
				NULL);
	}
	...
	if (simParameters->consTorqueOn) {
		molecule->build_constorque_params(configList->find("consTorqueFile") ,
				configList->find("consTorqueCol") , configList->find("consTorqueAxisFile") ,
				configList->find("consTorquePivotFile") , configList->find("consTorqueValFile") ,
				configList->find("consTorqueValCol") , pdb ,
				NULL);
	}
	...
	if (simParameters->langevinOn) {
		if (simParameters->langevinDamping == 0.0) {
			molecule->build_langevin_params(configList->find("langevinfile") ,
					configList->find("langevincol") , pdb ,
					NULL);
		} else {
			molecule->build_langevin_params(simParameters->langevinDamping , simParameters->drudeDamping ,
		...
		//  names . . .
		molecule->build_langevin_params(configList->find("tcouplefile") , configList->find("tcouplecol") ,
				pdb ,
				NULL);
	}
...
//Modifications for alchemical fep
	...
	if (simParameters->alchOn) {
		molecule->build_fep_flags(configList->find("alchfile") , configList->find("alchcol") , pdb , NULL ,
				"alch");
...
//fepe
	if (simParameters->lesOn) {
		if (simParameters->alchFepOn || simParameters->alchThermIntOn)
			NAMD_bug("FEP/TI and LES are incompatible!");
		molecule->build_fep_flags(configList->find("lesfile") , configList->find("lescol") , pdb , NULL ,
				"les");
	}
	if (simParameters->pairInteractionOn) {
	...
	//lyk
	if (simParameters->itsOn) {
		molecule->build_fep_flags(configList->find("itsFile") ,
				configList->find("itsCol") , pdb , NULL , "its");
	}
	if (simParameters->adaptCgOn) {
		molecule->build_fep_flags(configList->find("adaptCgFile") ,
				configList->find("adaptCgCol") , pdb , NULL , "adaptCg");
	}
	//lyk
