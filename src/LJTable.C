...
//----------------------------------------------------------------------
void LJTable::compute_vdw_params(int i, int j, LJTable::TableEntry *cur,
...
//    printf("Making this interaction tabulated. %i %i %i\n", i, j, K);
		...
		params->get_vdw_params(&sigma_i, &epsilon_i, &sigma_i14, &epsilon_i14,
				i);
		params->get_vdw_params(&sigma_j, &epsilon_j, &sigma_j14, &epsilon_j14,
				j);
		...
		BigReal sigma_ij14 =
				useGeom ?
						sqrt(sigma_i14 * sigma_j14) :
						0.5 * (sigma_i14 + sigma_j14);
