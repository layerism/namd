...
//lyk
#if ( FAST( 1+ ) 0 )
	Force *vdwf11_j = vdwf11_1 + j;
	Force *vdwf12_j = vdwf12_1 + j;
	Force *vdwf22_j = vdwf22_1 + j;

	Force *vdwRepf11_j = vdwRepf11_1 + j;
	Force *vdwRepf12_j = vdwRepf12_1 + j;
	Force *vdwRepf22_j = vdwRepf22_1 + j;

	Force *electf11_j = electf11_1 + j;
	Force *electf12_j = electf12_1 + j;
	Force *electf22_j = electf22_1 + j;
#endif
//lyk
...
#pragma disjoint (*p_j_d,        *f_j)
	...
	ITS(
			register int aip=int(p_i.partition);
			register int ajp=int(p_j->partition);
			if ( aip == group2 && ajp == group2 ) {
				isSelPairs_22=1;
			} else if ( (aip == group2 && ajp == group1) || (ajp == group2 && aip == group1) ) {
				isSelPairs_12=1;
			} else if ( aip == group1 && ajp == group1 ) {
				isSelPairs_11=1;
			}
	...
	ACG(
			register int aip=int(p_i.partition);
			register int ajp=int(p_j->partition);
			if ( (aip == aaGroup || aip == cgGroup) && (ajp == aaGroup || ajp == cgGroup) && aaGroup!=-1) {
				isSelPairs_11=1;
			}
			if ( aip == cgGroup && ajp == cgGroup ) {
				register int id_i = mol->cgid2MapId[pExt_0[i].id];
				register int id_j = mol->cgid2MapId[pExt_1[j].id];
				register int ncg = mol->nCg;
				register bool isExclude=mol->cgExclude[id_i*ncg+id_j];

				if (!isExclude) isSelPairs_22=1;
				//if (isExclude) iout<<"---- "<<pExt_0[i].id<<" "<<pExt_1[j].id<<" "<<isExclude<<"\n"<<endi;
			}
	)
...
#define ADDTENSOR(VIRIAL,FORCE,POS) \
	VIRIAL ## _xx += (FORCE ## _x) * (POS ## _x); \
	VIRIAL ## _xy += (FORCE ## _x) * (POS ## _y); \
	VIRIAL ## _xz += (FORCE ## _x) * (POS ## _z); \
	VIRIAL ## _yy += (FORCE ## _y) * (POS ## _y); \
	VIRIAL ## _yz += (FORCE ## _y) * (POS ## _z); \
	VIRIAL ## _zz += (FORCE ## _z) * (POS ## _z)

	ACG( BigReal cgkqq = cgkq_i * p_j->cgCharge;)
	//lyk
...
#define p_ij_z     vec_extract(p_i_v, 2)
	...
	//lyk
	ITS(
			register BigReal vdwRij_min=0.0;
			register BigReal vdwEpsij=0.0;
			register BigReal rij=0.0;
			if (isSelPairs_11==1 || isSelPairs_12==1 || isSelPairs_22==1) {
				vdwRij_min=pow(2.0*A/B,1.0/6.0);
				vdwEpsij=B*B/4.0/A;
				rij = sqrt( p_ij_x * p_ij_x + p_ij_y * p_ij_y + p_ij_z * p_ij_z );
			}
	)
	ACG(
			register int type_i=p_i.cgType;
			register int type_j=p_j->cgType;
			register int nCgType=mol->nCgType;
			register BigReal cgA=0.0;
			register BigReal cgB=0.0;
			register BigReal cgEps=0.0;
			register BigReal cgSgm=0.0;
			register BigReal rij=0.0;
			register BigReal vdwRij_min=0.0;
			register BigReal vdwEpsij=0.0;
			register BigReal cgVdwRij_min=0.0;
			register BigReal cgVdwEpsij=0.0;

			//register BigReal dUdrOnSgm=0.0;
			//register BigReal UOnSgm=0.0;
			//register BigReal cgSwitchSgm=0.0;

			if (isSelPairs_11==1 || isSelPairs_22==1) {
				rij = sqrt( p_ij_x * p_ij_x + p_ij_y * p_ij_y + p_ij_z * p_ij_z );
				vdwRij_min=pow(2.0*A/B,1.0/6.0);
				vdwEpsij=B*B/4.0/A;
			}
			if (type_i>=0 && type_j>=0 && isSelPairs_22==1) {
				cgEps = mol->cgVdwEps[nCgType*type_i+type_j] * cgScaling;
				cgSgm = mol->cgVdwSgm[nCgType*type_i+type_j];

				register BigReal cgSgm2 = cgSgm * cgSgm;
				register BigReal cgSgm6 = cgSgm2 * cgSgm2 * cgSgm2;
				register BigReal cgSgm12 = cgSgm6 * cgSgm6;
				cgA = 4.0 * cgEps * cgSgm12;
				cgB = 4.0 * cgEps * cgSgm6;
				cgVdwRij_min=cgSgm*pow(2.0,1.0/6.0);
				cgVdwEpsij=cgEps;

				/*cgSwitchSgm = cgSgm * cgVdwCappedSwitchF;
				 register BigReal cgSwitchSgm2 = cgSwitchSgm * cgSwitchSgm;
				 register BigReal cgSwitchSgm6 = cgSwitchSgm2 * cgSwitchSgm2 * cgSwitchSgm2;
				 register BigReal cgSwitchSgm12 = cgSwitchSgm6 * cgSwitchSgm6;
				 register BigReal term12 = cgA / cgSwitchSgm12;
				 register BigReal term6 = cgB / cgSwitchSgm6;
				 dUdrOnSgm = -12.0 * term12 + 6.0 * term6;
				 dUdrOnSgm /= cgSwitchSgm;
				 UOnSgm = term12 - term6;*/
			}
	...
	//lyk-vdw-and-elect-force
	ITS(
			register BigReal vdw_r = 0.0;
			register BigReal vdwtmp_x = 0.0;
			register BigReal vdwtmp_y = 0.0;
			register BigReal vdwtmp_z = 0.0;

			if (isSelPairs_11==1 || isSelPairs_12==1 || isSelPairs_22==1) {
				vdw_r = (diffa * vdw_d + vdw_c) * diffa + vdw_b;
				vdwtmp_x = vdw_r * p_ij_x;
				vdwtmp_y = vdw_r * p_ij_y;
				vdwtmp_z = vdw_r * p_ij_z;
			}

			if (isSelPairs_11==1) {

				ADDF(vdwf11_i,vdwtmp);
				SUBF(vdwf11_j,vdwtmp);
				PAIR(ADDTENSOR(vdw11_virial,vdwtmp,p_ij);)
				if (rij<=vdwRij_min) {
					ADDF(vdwRepf11_i,vdwtmp);
					SUBF(vdwRepf11_j,vdwtmp);
					PAIR(ADDTENSOR(vdwRep11_virial,vdwtmp,p_ij);)
				}

			} else if (isSelPairs_12==1) {

				ADDF(vdwf12_i,vdwtmp);
				SUBF(vdwf12_j,vdwtmp);
				PAIR(ADDTENSOR(vdw12_virial,vdwtmp,p_ij);)
				if (rij<=vdwRij_min) {
					ADDF(vdwRepf12_i,vdwtmp);
					SUBF(vdwRepf12_j,vdwtmp);
					PAIR(ADDTENSOR(vdwRep12_virial,vdwtmp,p_ij);)
				}

			} else if (isSelPairs_22==1) {

				ADDF(vdwf22_i,vdwtmp);
				SUBF(vdwf22_j,vdwtmp);
				PAIR(ADDTENSOR(vdw22_virial,vdwtmp,p_ij);)
				if (rij<=vdwRij_min) {
					ADDF(vdwRepf22_i,vdwtmp);
					SUBF(vdwRepf22_j,vdwtmp);
					PAIR(ADDTENSOR(vdwRep22_virial,vdwtmp,p_ij);)
				}
	...
	ITS(
			...
			if (isSelPairs_11==1 || isSelPairs_22==1 || isSelPairs_12==1) {
				elect_d = kqq * table_four_i[8];
				elect_c = kqq * table_four_i[9];
				elect_b = kqq * table_four_i[10];
				elect_a = kqq * table_four_i[11];
				elect_r = (diffa * elect_d + elect_c) * diffa + elect_b;
			}

			if (isSelPairs_11==1) {
				register BigReal electtmp_x = elect_r * p_ij_x;
				register BigReal electtmp_y = elect_r * p_ij_y;
				register BigReal electtmp_z = elect_r * p_ij_z;
				ADDF(electf11_i,electtmp);
				SUBF(electf11_j,electtmp);
				PAIR(ADDTENSOR(elect11_virial,electtmp,p_ij);)

			} else if (isSelPairs_12==1) {
				register BigReal electtmp_x = elect_r * p_ij_x;
				register BigReal electtmp_y = elect_r * p_ij_y;
				register BigReal electtmp_z = elect_r * p_ij_z;
				ADDF(electf12_i,electtmp);
				SUBF(electf12_j,electtmp);
				PAIR(ADDTENSOR(elect12_virial,electtmp,p_ij);)

			} else if (isSelPairs_22==1) {
				register BigReal electtmp_x = elect_r * p_ij_x;
				register BigReal electtmp_y = elect_r * p_ij_y;
				register BigReal electtmp_z = elect_r * p_ij_z;
				ADDF(electf22_i,electtmp);
				SUBF(electf22_j,electtmp);
				PAIR(ADDTENSOR(elect22_virial,electtmp,p_ij);)
			}
	)
	...
	ACG(
			...
			if (isSelPairs_11==1) {
				register BigReal vdw_r = (diffa * vdw_d + vdw_c) * diffa + vdw_b;
				register BigReal vdwtmp_x = vdw_r * p_ij_x;
				register BigReal vdwtmp_y = vdw_r * p_ij_y;
				register BigReal vdwtmp_z = vdw_r * p_ij_z;
				ADDF(vdwf11_i,vdwtmp);
				SUBF(vdwf11_j,vdwtmp);
				PAIR(ADDTENSOR(vdw11_virial,vdwtmp,p_ij);)
				if (rij<=vdwRij_min) {
			...
			BigReal cgvdw_b = 0.0;
			BigReal cgvdw_a = 0.0;
			//BigReal cgvdw_smallr_fdir = 0.0;

			if (isSelPairs_22==1) {
				register BigReal cgvdw_r = 0.0;

				//if (rij<=cgSwitchSgm) {
				//	register BigReal reduceDist = rij / cgSwitchSgm;
				//	register BigReal pow_N = pow(reduceDist ,cgVdwCappedN);
				//	cgvdw_a = -dUdrOnSgm * cgSwitchSgm * ( pow_N -1.0 ) / cgVdwCappedN - UOnSgm;  //minus energy small r
				//	cgvdw_smallr_fdir = -dUdrOnSgm * pow_N / reduceDist / rij;//force small r
				//	cgvdw_r = cgvdw_smallr_fdir;
				//} else {
				cgvdw_d = cgA * table_four_i[0] - cgB * table_four_i[4];
				cgvdw_c = cgA * table_four_i[1] - cgB * table_four_i[5];
				cgvdw_b = cgA * table_four_i[2] - cgB * table_four_i[6];
				cgvdw_a = cgA * table_four_i[3] - cgB * table_four_i[7];
				cgvdw_r = (diffa * cgvdw_d + cgvdw_c) * diffa + cgvdw_b;
				//}
				register BigReal cgvdwtmp_x = cgvdw_r * p_ij_x;
				register BigReal cgvdwtmp_y = cgvdw_r * p_ij_y;
				register BigReal cgvdwtmp_z = cgvdw_r * p_ij_z;
				ADDF(vdwf22_i,cgvdwtmp);
				SUBF(vdwf22_j,cgvdwtmp);
				PAIR(ADDTENSOR(vdw22_virial,cgvdwtmp,p_ij);)
				if (rij<=cgVdwRij_min) {
					ADDF(vdwRepf22_i,cgvdwtmp);
					SUBF(vdwRepf22_j,cgvdwtmp);
					PAIR(ADDTENSOR(vdwRep22_virial,cgvdwtmp,p_ij);)
				}
	...
	ACG(
			...
			if (isSelPairs_22==1) {
				if (kqq!=0) elect_r = elect_r * cgkqq / kqq;
				register BigReal cgelecttmp_x = elect_r * p_ij_x;
				register BigReal cgelecttmp_y = elect_r * p_ij_y;
				register BigReal cgelecttmp_z = elect_r * p_ij_z;
				ADDF(electf22_i,cgelecttmp);
				SUBF(electf22_j,cgelecttmp);
				PAIR(ADDTENSOR(elect22_virial,cgelecttmp,p_ij);)
			}
	)
...
#define   vdw_a  vec_extract(vdw_v, 3)
	...
	if (tabtype >= 0) {
		...
		ENERGY(
				...
				ITS(
						if (isSelPairs_11==1) {
							vdwEnergy_11 -= vdw_val;
							if (rij<=vdwRij_min) {
								vdwRepEnergy_11 -= vdw_val;
								vdwRepEnergy_11 += vdwEpsij;
							}
						} else if (isSelPairs_12==1) {
							vdwEnergy_12 -= vdw_val;
							if (rij<=vdwRij_min) {
								vdwRepEnergy_12 -= vdw_val;
								vdwRepEnergy_12 += vdwEpsij;
							}
						} else if (isSelPairs_22==1) {
							vdwEnergy_22 -= vdw_val;
							if (rij<=vdwRij_min) {
								vdwRepEnergy_22 -= vdw_val;
								vdwRepEnergy_22 += vdwEpsij;
							}
						}
				)
				ITS(
						register BigReal elect_val =
						( ( diffa * elect_d *(1/6.)+ elect_c * (1/4.)) * diffa + elect_b *(1/2.)) * diffa + elect_a;

						if (isSelPairs_11==1) {
							electEnergy_11 -= elect_val;

						} else if (isSelPairs_12==1) {
							electEnergy_12 -= elect_val;

						} else if (isSelPairs_22==1) {
							electEnergy_22 -= elect_val;
				...
				ACG(
						if (isSelPairs_11==1) {
							...
							/*const BigReal switchOn = ComputeNonbondedUtil::switchOn;
							 const BigReal cutoff = ComputeNonbondedUtil::cutoff;
							 const BigReal switchOn2 = ComputeNonbondedUtil::switchOn2;
							 const BigReal cutoff2 = ComputeNonbondedUtil::cutoff2;
							 BigReal sw=0.0;
							 BigReal dx=p_i.position.x-p_j->position.x;
							 BigReal dy=p_i.position.y-p_j->position.y;
							 BigReal dz=p_i.position.z-p_j->position.z;
							 BigReal rij2=dx*dx + dy*dy + dz*dz;
							 //BigReal rij2 = r2list[k];
							 if (rij > switchOn && rij < cutoff) {
							 sw = (rij2-cutoff2)*(rij2-cutoff2)*(cutoff2+2*rij2-3*switchOn2);
							 sw /= (cutoff2-switchOn2)*(cutoff2-switchOn2)*(cutoff2-switchOn2);
							 } else if (rij>=cutoff) {
							 sw=0.0;
							 } else if (rij<=switchOn) {
							 sw=1.0;
							 }
							 BigReal lje=(A/pow(rij2,6.0)-B/pow(rij2,3.0))*sw;
							 iout<<"UIJ: "<<pExt_0[i].id+1<<" "<<pExt_1[j].id+1<<": "<<-vdw_val<<"  "<<lje<<" "<<rij
							 <<" ("<<p_i.position<<") || ("<<p_j->position<<")\n"<<endi;*/

						}
						if (isSelPairs_22==1) {
							register BigReal cgvdw_val=
							( ( diffa * cgvdw_d * (1/6.)+ cgvdw_c * (1/4.)) * diffa + cgvdw_b *(1/2.)) * diffa
							+ cgvdw_a;

							vdwEnergy_22 -= cgvdw_val;

							if (rij<=cgVdwRij_min) {
								vdwRepEnergy_22 -= cgvdw_val;
								vdwRepEnergy_22 += cgVdwEpsij;
							}
						}
				)
				ACG(
						...
						if (isSelPairs_22==1) {
							if (kqq!=0) electEnergy_22 -= elect_val * cgkqq / kqq;
						}
				)
				//lyk
