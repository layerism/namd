...
public:
	...
	int num_group_deg_freedom() const {
		...
		if (s_NumFixedGroups)
			s_NumGroupDegFreedom -= 3 * s_NumFixedGroups;
		if (!(s_NumFixedAtoms || numConstraints || simParams->comMove || simParams->langevinOn)) {
	...
	int num_deg_freedom(int isInitialReport = 0) const {
		...
		int s_NumFixedAtoms = num_fixed_atoms();
		if (s_NumFixedAtoms)
			s_NumDegFreedom -= 3 * s_NumFixedAtoms;
		if (numLonepairs)
			s_NumDegFreedom -= 3 * numLonepairs;
		if (!(s_NumFixedAtoms || numConstraints || simParams->comMove || simParams->langevinOn)) {
			s_NumDegFreedom -= 3;
		}
		...
		int s_NumFixedRigidBonds = (simParams->fixedAtomsOn ? numFixedRigidBonds : 0);
...
//fepe
	...
	void build_rotdrag_params(StringList *, StringList *, StringList *, StringList *, StringList *, StringList *, PDB *,
			char *);
	//  Build the set of rotating drag pars

	void build_constorque_params(StringList *, StringList *, StringList *, StringList *, StringList *, StringList *,
			PDB *, char *);
...
// End of port -- JL  
	...
	//lyk
	void loadAdaptCgParam(int atomIndex[], float charge[], float mass[], int type[], float vdwEps[], float vdwSgm[],
	...
	//lyk
	//  Get the charge of an atom
	Real atomcgcharge(int anum) const {
		return (atoms[anum].cgCharge);
	}
	int atomcgtype(int anum) const {
		return (atoms[anum].cgType);
	}
	Real atomcgmass(int anum) const {
		return (atoms[anum].cgMass);
	}
	//lyk
