...
protected:
	...
	virtual void loadTuples(void) {
...
#else
		T::getMoleculePointers(node->molecule, &numTuples, &tuplesByAtom, &tupleStructs);
#endif
		...
		Real invLesFactor = lesOn ? 1.0 / node->simParameters->lesFactor : 1.0;
...
protected:
	...
	ComputeHomeTuples(ComputeID c) :
		...
		amd_reduction = NULL;
		accelMDdoDihe = false;
		if (params->accelMDOn || params->itsOn || params->adaptCgOn) { //lyk
			amd_reduction = ReductionMgr::Object()->willSubmit(REDUCTIONS_AMD);
			if (params->accelMDdihe || params->accelMDdual || params->useDihedOn)//lyk
				accelMDdoDihe = true;
		}
		if (params->pressureProfileOn) {
			pressureProfileSlabs = T::pressureProfileSlabs = params->pressureProfileSlabs;
			int n = T::pressureProfileAtomTypes = params->pressureProfileAtomTypes;
			pressureProfileReduction = ReductionMgr::Object()->willSubmit(REDUCTIONS_PPROF_BONDED,
...
//-------------------------------------------------------------------
	virtual void doWork(void) {
		...
		for (ap = ap.begin(); ap != ap.end(); ap++) {
			...
			if (ap->p->flags.doMolly)
				ap->x_avg = ap->avgPositionBox->open();
			ap->r = ap->forceBox->open();
			ap->f = ap->r->f[Results::normal];
			if (accelMDdoDihe)
				ap->af = ap->r->f[Results::amdf]; // for dihedral-only or dual-boost accelMD
		}
		...
		for (int i = 0; i < T::reductionDataSize; ++i)
			reductionData[i] = 0;
		...
		for (ap = ap.begin(); ap != ap.end(); ap++) {
			...
			if (ap->p->flags.doMolly)
				ap->avgPositionBox->close(&(ap->x_avg));
