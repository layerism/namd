...
inline void Phase2_PairSer(
		//doesn't do self energies

//input values
		BigReal r, BigReal r2, BigReal r_i, BigReal qiqj, BigReal ai, BigReal aj, BigReal epsilon_p_i,
		BigReal epsilon_s_i, BigReal kappa, int exclij, BigReal scale14, int stat,

//return values
		BigReal & coulEij, BigReal & ddrCoulEij, BigReal & gbEij, BigReal & ddrGbEij, BigReal & dEdai, BigReal & dEdaj);

inline void CalcHPairSer(BigReal r, //distance
		BigReal r2, BigReal ri, BigReal rc, //cutoff
		BigReal ri0, BigReal rjs, BigReal rj0, BigReal ris, int & dij, //domain 1
		int & dji, //domain 2
		BigReal & dhij, BigReal & dhji);
inline void CalcDHPairSer(BigReal r, //distance
		BigReal r2, BigReal ri, BigReal rc, //cutoff
		BigReal ri0, BigReal rjs, BigReal rj0, BigReal ris, int & dij, //domain 1
		int & dji, //domain 2
		BigReal & dhij, BigReal & dhji);
...
ComputeGBISserMgr::ComputeGBISserMgr() :
		gbisProxy(thisgroup), gbisCompute(0), numSources(0), numArrived(0), inittime(0), psitime(0), alphatime(0), dEdrtime(
				0), dEdasumtime(0), dEdaprefixtime(0), dEdalooptime(0), slowForce(0), coordMsgs(0), coord(0), force(0), oldmsg(
				0), numAtoms(0), timestep(0) {
...
ComputeGBISser::ComputeGBISser(ComputeID c) :
	...
	CProxy_ComputeGBISserMgr::ckLocalBranch(CkpvAccess(BOCclass_group).computeGBISserMgr)->setCompute(this);

	reduction = ReductionMgr::Object()->willSubmit(REDUCTIONS_BASIC);
	/*//lyk
	 SimParameters *simParams = Node::Object()->simParameters;
	 if (simParams->accelMDOn || simParams->itsOn || simParams->adaptCgOn) {
 ...
 * (4) s'(cutoff) = 0
 ******************************************************************************/
inline void CalcScaleSer(BigReal r, //rij
...
void ComputeGBISserMgr::calcGBISReg(int stat) {
	...
	for (int i = 0; i < numAtoms; i++) {
		...
		for (int j = i + 1; j < numAtoms; j++) {
			...
			CalcHPairSer(r, r2, r_i, a_cut, coord[i].rho0, coord[j].rhos, coord[j].rho0, coord[i].rhos, dij, dji, hij,
					hji);
	...
	for (int i = 0; i < numAtoms; i++) {
		...
		for (int j = i + 1; j < numAtoms; j++) {
			...
			Phase2_PairSer(r, r2, r_i, qiqj, a[i], a[j], epsilon_p_i, epsilon_s_i, kappa, exclij, scale14, stat,
					coulEij, ddrCoulEij, gbEij, ddrGbEij, dEdai, dEdaj);
			int id1 = coord[i].id;
			int id2 = coord[j].id;
			if (id1 > id2) {
				int tmp = id2;
...
//CkPrintf("self_energy[%03i] = %e\n",i,0.5*gbEij);
		...
		if (stat) {
			dEdai = -0.5 * COUL_CONST * coord[i].charge * coord[i].charge * (kappa * epsilon_s_i * expkappa - Dij / fij)
					/ a[i];
	...
	if (stat) {
		...
		for (int i = 0; i < numAtoms; i++) {
			...
			for (int j = i + 1; j < numAtoms; j++) {
				...
				r = 1.0 / r_i;
				CalcDHPairSer(r, r2, r_i, a_cut, coord[i].rho0, coord[j].rhos, coord[j].rho0, coord[i].rhos, dij, dji,
						dhij, dhji);
...
void ComputeGBISser::saveResults(GBISForceMsg *msg) {
	...
	/*//lyk
	 if (amd_reduction) {
	 amd_reduction->item(REDUCTION_ELECT_ENERGY) += msg->gbInterEnergy;
	 amd_reduction->item(REDUCTION_ELECT_ENERGY) += msg->gbSelfEnergy;
...
inline void dh4Ser(BigReal r, BigReal r2, BigReal ri, // 0.4%
	...
	BigReal rilogr = ri * logr;
	dh = 0.25
			* ((-0.5 + (r2 * r02mrs2 - 2 * r * rs * rs2 + rs2 * r02mrs2) * 0.5 * ri2 * rspri * rspri) * r0i * r0i
					- ri * rilogr);
}
...
inline void CalcHSer(BigReal r, BigReal r2, BigReal ri, BigReal rc, BigReal r0, BigReal rs, BigReal & h, int & d) {
	if (r <= rc - rs && r > 4 * rs) { //II
		h2Ser(r, r2, ri, rc, r0, rs, h);
		d = 2;
	} else if (r <= rc + rs && r > rc - rs) { //I
		h1Ser(r, r2, ri, rc, r0, rs, h);
		d = 1;
	} else if (r > rc + rs) { //0
		h0Ser(r, r2, ri, rc, r0, rs, h);
		d = 0;
	} else if (r <= 4 * rs && r > r0 + rs) { //III
		h3Ser(r, r2, ri, rc, r0, rs, h);
		d = 3;
	} else if (r <= r0 + rs && r > (r0 > rs ? r0 - rs : rs - r0)) { //IV
		h4Ser(r, r2, ri, rc, r0, rs, h);
		d = 4;
	} else if (r0 < rs) { //V
		h5Ser(r, r2, ri, rc, r0, rs, h);
		d = 5;
	} else { //VI
		h6Ser(r, r2, ri, rc, r0, rs, h);
		d = 6;
	}
}
inline void CalcDHSer(BigReal r, BigReal r2, BigReal ri, BigReal rc, BigReal r0, BigReal rs, BigReal & dh, int & d) {
	if (r <= rc - rs && r > 4 * rs) { //II
		dh2Ser(r, r2, ri, rc, r0, rs, dh);
		d = 2;
	} else if (r <= rc + rs && r > rc - rs) { //I
		dh1Ser(r, r2, ri, rc, r0, rs, dh);
		d = 1;
	} else if (r > rc + rs) { //0
		dh0Ser(r, r2, ri, rc, r0, rs, dh);
		d = 0;
	} else if (r <= 4 * rs && r > r0 + rs) { //III
		dh3Ser(r, r2, ri, rc, r0, rs, dh);
		d = 3;
	} else if (r <= r0 + rs && r > (r0 > rs ? r0 - rs : rs - r0)) { //IV
		dh4Ser(r, r2, ri, rc, r0, rs, dh);
		d = 4;
	} else if (r0 < rs) { //V
		dh5Ser(r, r2, ri, rc, r0, rs, dh);
		d = 5;
	} else { //VI
		dh6Ser(r, r2, ri, rc, r0, rs, dh);
		d = 6;
	}
}
inline void CalcHPairSer(BigReal r, //distance
 ...
 */
inline void Calc_Coul_PairSer(BigReal r_i, BigReal qiqj, BigReal epsilon_p_i, int exclij, BigReal scale14,
		BigReal & coulE, BigReal & ddrCoulE) {
 ...
 */
inline void Calc_dEdr_PairSer(
		//no longer does i==j
		BigReal r, BigReal r2, BigReal qiqj, BigReal ai, BigReal aj, BigReal kappa, BigReal epsilon_p_i,
		BigReal epsilon_s_i, BigReal & aiaj, BigReal & expr2aiaj4, BigReal & fij, BigReal & f_i, BigReal & expkappa,
		BigReal & Dij, BigReal & gbE,   //return
 ...
 */
inline void Calc_dEda_PairSer(BigReal r2, BigReal ai, BigReal aj, BigReal qiqj, BigReal kappa, BigReal aiaj,
		BigReal expkappa, BigReal expr2aiaj4, BigReal fij, BigReal f_i, BigReal Dij, BigReal epsilon_s_i,
		BigReal & dEdai, //return
		BigReal & dEdaj //return
		) {

	BigReal tmp_dEda = 0.5 * qiqj * f_i * f_i * (kappa * epsilon_s_i * expkappa - Dij * f_i) * (aiaj + 0.25 * r2)
			* expr2aiaj4; //0
	dEdai = tmp_dEda / ai;
	dEdaj = tmp_dEda / aj;
}
...
//input values
		BigReal r, BigReal r2, BigReal r_i, BigReal qiqj, BigReal ai, BigReal aj, BigReal epsilon_p_i,
		BigReal epsilon_s_i, BigReal kappa, int exclij, BigReal scale14, int doSlow,

//return values
		BigReal & coulEij, BigReal & ddrCoulEij, BigReal & gbEij, BigReal & ddrGbEij, BigReal & dEdai,
		BigReal & dEdaj) {
	...
	Calc_dEdr_PairSer(r, r2, qiqj, ai, aj, kappa, epsilon_p_i, epsilon_s_i, aiaj, expr2aiaj4, fij, f_i, expkappa, Dij,
			gbEij, ddrGbEij);

	//calculate dEda
	if (doSlow) {
		Calc_dEda_PairSer(r2, ai, aj, qiqj, kappa, aiaj, expkappa, expr2aiaj4, fij, f_i, Dij, epsilon_s_i, dEdai,
				dEdaj);
	}
}
