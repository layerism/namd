...
public:
	...
	enum {
		dihedralEnergyIndex, TENSOR(virialIndex), reductionDataSize
	};
	enum {
...
public:
	...
	ComputeDihedrals(ComputeID c, PatchIDList &p) :
			ComputeHomeTuples<DihedralElem, Dihedral, DihedralValue>(c, p) {
		;
	}
...
public:
	...
	ComputeSelfDihedrals(ComputeID c, PatchID p) :
			ComputeSelfTuples<DihedralElem, Dihedral, DihedralValue>(c, p) {
		;
	}
