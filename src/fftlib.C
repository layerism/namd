...
void OptPmeXPencil::fft_init() {
	...
	myKSpace = new PmeKSpace(initdata.grid, thisIndex.y * block2, thisIndex.y * block2 + ny,
...
void OptPmeXPencil::pme_kspace() {
	...
	evir[0] = myKSpace->compute_energy(data, lattice, ewaldcof, &(evir[1]));
...
void OptPmeZPencil::initialize_manytomany() {
	...
	cbw_recvgrid.cb = CkCallback(CkIndex_OptPmeZPencil::many_to_manyRecvGrid(NULL), aidx,
			thisProxy.ckGetArrayID());
	cbw_recvgrid.msg = new (PRIORITY_SIZE) OptPmeDummyMsg;
	cbw_recvgrid.array = this;
	PatchMap *patchMap = PatchMap::Object();
	CmiDirect_manytomany_initialize_recvbase(handle, PHASE_GR, call_ck_cb_recv_grid, &cbw_recvgrid,
	...
	for (idx = 0; idx < grid_msgs.size(); idx++) {
		...
		CmiDirect_manytomany_initialize_recv(handle, PHASE_GR, m2m_recv_grid[idx].patchID, offset,
				fcount * sizeof(float), patchMap->node(m2m_recv_grid[idx].patchID));
	...
	CmiDirect_manytomany_initialize_sendbase(handle, PHASE_YF, NULL, NULL, (char *) many_to_many_data,
			zBlocks, thisIndex.y);

	//Initialize recv untrans
	cbw_recvuntrans.cb = CkCallback(CkIndex_OptPmeZPencil::many_to_manyRecvUntrans(NULL), aidx,
			thisProxy.ckGetArrayID());
	cbw_recvuntrans.msg = new (PRIORITY_SIZE) OptPmeDummyMsg;
	cbw_recvuntrans.array = this;
	CmiDirect_manytomany_initialize_recvbase(handle, PHASE_ZB, call_ck_cb_recv_untrans_z, &cbw_recvuntrans,
	...
	for (int kb = 0; kb < zBlocks; ++kb) {
		...
		CmiDirect_manytomany_initialize_send(handle, PHASE_YF, kb, kb * 2 * nx * ny * block3 * sizeof(float),
				2 * nx * ny * nz * sizeof(float), pe);

		//Initialize recv untrans
		CmiDirect_manytomany_initialize_recv(handle, PHASE_ZB, kb, kb * nx * ny * block3 * 2 * sizeof(float),
				nx * ny * nz * 2 * sizeof(float), pe);
...
void OptPmeYPencil::initialize_manytomany() {
	...
	//send trans
	CmiDirect_manytomany_initialize_sendbase(handle, PHASE_XF, NULL, NULL, (char *) many_to_many_data,
			yBlocks, thisIndex.x);
	//send untrans
	CmiDirect_manytomany_initialize_sendbase(handle, PHASE_ZB, NULL, NULL, (char *) many_to_many_data,
			yBlocks, thisIndex.z);

	//recv trans
	CkArrayIndex3D idx(thisIndex.x, thisIndex.y, thisIndex.z);
	cbw_recvtrans.cb = CkCallback(CkIndex_OptPmeYPencil::many_to_manyRecvTrans(NULL), idx,
			thisProxy.ckGetArrayID());
	cbw_recvtrans.msg = new (PRIORITY_SIZE) OptPmeDummyMsg;
	cbw_recvtrans.array = this;
	CmiDirect_manytomany_initialize_recvbase(handle, PHASE_YF, call_ck_cb_recv_trans_y, &cbw_recvtrans,
			(char *) many_to_many_data, initdata.yBlocks, -1);

	//recv untrans
	cbw_recvuntrans.cb = CkCallback(CkIndex_OptPmeYPencil::many_to_manyRecvUntrans(NULL), idx,
			thisProxy.ckGetArrayID());
	cbw_recvuntrans.msg = new (PRIORITY_SIZE) OptPmeDummyMsg;
	cbw_recvuntrans.array = this;
	CmiDirect_manytomany_initialize_recvbase(handle, PHASE_YB, call_ck_cb_recv_untrans_y, &cbw_recvuntrans,
			(char *) many_to_many_data, initdata.yBlocks, -1);
	...
	for (int jb = 0; jb < yBlocks; ++jb) {
		...
		CmiDirect_manytomany_initialize_send(handle, PHASE_ZB, jb, 2 * nx * block2 * nz * jb * sizeof(float),
				2 * nx * ny * nz * sizeof(float), pe);

		//recv trans
		CmiDirect_manytomany_initialize_recv(handle, PHASE_YF, jb, jb * nx * block2 * nz * 2 * sizeof(float),
				nx * ny * nz * 2 * sizeof(float), pe);
		...
		CmiDirect_manytomany_initialize_send(handle, PHASE_XF, jb, 2 * nx * block2 * nz * jb * sizeof(float),
				2 * nx * ny * nz * sizeof(float), pe);

		//Recv untrans
		CmiDirect_manytomany_initialize_recv(handle, PHASE_YB, jb, jb * nx * block2 * nz * 2 * sizeof(float),
				nx * ny * nz * 2 * sizeof(float), pe);
...
void OptPmeXPencil::initialize_manytomany() {
	...
	CmiDirect_manytomany_initialize_sendbase(handle, PHASE_YB, NULL, NULL, (char *) many_to_many_data,
			xBlocks, thisIndex.y);
	CkArrayIndex3D idx(thisIndex.x, thisIndex.y, thisIndex.z);
	cbw_recvtrans.cb = CkCallback(CkIndex_OptPmeXPencil::many_to_manyRecvTrans(NULL), idx,
			thisProxy.ckGetArrayID());
	cbw_recvtrans.msg = new (PRIORITY_SIZE) OptPmeDummyMsg;
	cbw_recvtrans.array = this;
	CmiDirect_manytomany_initialize_recvbase(handle, PHASE_XF, call_ck_cb_recv_trans_x, &cbw_recvtrans,
			(char *) many_to_many_data, initdata.xBlocks, -1);
	...
	for (int ib = 0; ib < xBlocks; ++ib) {
		...
		CProxy_OptPmePencilMapY yproxy(global_map_y);
		int pe = yproxy.ckLocalBranch()->procNum(0, index);
		CmiDirect_manytomany_initialize_send(handle, PHASE_YB, ib, 2 * block1 * ny * nz * ib * sizeof(float),
				2 * nx * ny * nz * sizeof(float), pe);

		CmiDirect_manytomany_initialize_recv(handle, PHASE_XF, ib, ib * block1 * ny * nz * 2 * sizeof(float),
				nx * ny * nz * 2 * sizeof(float), pe);
