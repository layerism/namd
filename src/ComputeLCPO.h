...
class LCPONeighborList {
	...
	LCPONeighborList& operator=(const LCPONeighborList&) {
		return *this;
	}
public:
	LCPONeighborList() :
			maxNeighbors(initsize), maxAtoms(initsize), curNeighbor(0), curAtom(0) {
...
public:
	ComputeLCPO(ComputeID c, PatchID pid[], int t[], ComputeNonbondedWorkArrays* _workArrays, int minPartition,
...
protected:
	...
	SubmitReduction *amd_reduction;  //lyk
