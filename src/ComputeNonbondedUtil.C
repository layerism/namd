...
//lyk
Bool ComputeNonbondedUtil::itsOn;
int ComputeNonbondedUtil::group1;
int ComputeNonbondedUtil::group2;

Bool ComputeNonbondedUtil::adaptCgOn;
int ComputeNonbondedUtil::cgGroup;
int ComputeNonbondedUtil::aaGroup;
BigReal ComputeNonbondedUtil::cg_dielectric_1;
BigReal ComputeNonbondedUtil::cgVdwCappedSwitchF;
BigReal ComputeNonbondedUtil::cgVdwCappedN;
BigReal ComputeNonbondedUtil::cgScaling;
//lyk
...
void ComputeNonbondedUtil::submitReductionData(BigReal *data, SubmitReduction *reduction) {
	...
	//lyk
	reduction->item(REDUCTION_11_LJ_ENERGY) += data[vdwEnergyIndex_11];
	reduction->item(REDUCTION_12_LJ_ENERGY) += data[vdwEnergyIndex_12];
	reduction->item(REDUCTION_22_LJ_ENERGY) += data[vdwEnergyIndex_22];

	reduction->item(REDUCTION_11_REPLJ_ENERGY) += data[vdwRepEnergyIndex_11];
	reduction->item(REDUCTION_12_REPLJ_ENERGY) += data[vdwRepEnergyIndex_12];
	reduction->item(REDUCTION_22_REPLJ_ENERGY) += data[vdwRepEnergyIndex_22];

	reduction->item(REDUCTION_11_ELECT_ENERGY) += data[electEnergyIndex_11];
	reduction->item(REDUCTION_12_ELECT_ENERGY) += data[electEnergyIndex_12];
	reduction->item(REDUCTION_22_ELECT_ENERGY) += data[electEnergyIndex_22];

	ADD_TENSOR(reduction, REDUCTION_VIRIAL_11_VDW, data, vdwVirialIndex_11);
	ADD_TENSOR(reduction, REDUCTION_VIRIAL_12_VDW, data, vdwVirialIndex_12);
	ADD_TENSOR(reduction, REDUCTION_VIRIAL_22_VDW, data, vdwVirialIndex_22);

	ADD_TENSOR(reduction, REDUCTION_VIRIAL_11_REPVDW, data, vdwRepVirialIndex_11);
	ADD_TENSOR(reduction, REDUCTION_VIRIAL_12_REPVDW, data, vdwRepVirialIndex_12);
	ADD_TENSOR(reduction, REDUCTION_VIRIAL_22_REPVDW, data, vdwRepVirialIndex_22);

	ADD_TENSOR(reduction, REDUCTION_VIRIAL_11_ELECT, data, electVirialIndex_11);
	ADD_TENSOR(reduction, REDUCTION_VIRIAL_12_ELECT, data, electVirialIndex_12);
	ADD_TENSOR(reduction, REDUCTION_VIRIAL_22_ELECT, data, electVirialIndex_22);
	//lyk
	// Ported by JLai
...
void ComputeNonbondedUtil::submitPressureProfileData(BigReal *data, SubmitReduction *reduction) {
	if (!reduction)
		return;
...
//fepb
	...
	//lyk
	itsOn = simParams->itsOn;
	adaptCgOn = simParams->adaptCgOn;
	//lyk
...
#ifdef NAMD_CUDA
		NAMD_die("ITS is not supported in CUDA version");
#endif
		ComputeNonbondedUtil::calcPair = calc_pair_its;
		ComputeNonbondedUtil::calcPairEnergy = calc_pair_energy_its;
		ComputeNonbondedUtil::calcSelf = calc_self_its;
		ComputeNonbondedUtil::calcSelfEnergy = calc_self_energy_its;
		ComputeNonbondedUtil::calcFullPair = calc_pair_fullelect_its;
		ComputeNonbondedUtil::calcFullPairEnergy = calc_pair_energy_fullelect_its;
		ComputeNonbondedUtil::calcFullSelf = calc_self_fullelect_its;
		ComputeNonbondedUtil::calcFullSelfEnergy = calc_self_energy_fullelect_its;
		ComputeNonbondedUtil::calcMergePair = calc_pair_merge_fullelect_its;
		ComputeNonbondedUtil::calcMergePairEnergy = calc_pair_energy_merge_fullelect_its;
		ComputeNonbondedUtil::calcMergeSelf = calc_self_merge_fullelect_its;
		ComputeNonbondedUtil::calcMergeSelfEnergy = calc_self_energy_merge_fullelect_its;
		ComputeNonbondedUtil::calcSlowPair = calc_pair_slow_fullelect_its;
		ComputeNonbondedUtil::calcSlowPairEnergy = calc_pair_energy_slow_fullelect_its;
		ComputeNonbondedUtil::calcSlowSelf = calc_self_slow_fullelect_its;
		ComputeNonbondedUtil::calcSlowSelfEnergy = calc_self_energy_slow_fullelect_its;
	} else if (adaptCgOn) {

#ifdef NAMD_CUDA
		NAMD_die("ACG is not supported in CUDA version");
#endif
		ComputeNonbondedUtil::calcPair = calc_pair_acg;
		ComputeNonbondedUtil::calcPairEnergy = calc_pair_energy_acg;
		ComputeNonbondedUtil::calcSelf = calc_self_acg;
		ComputeNonbondedUtil::calcSelfEnergy = calc_self_energy_acg;
		ComputeNonbondedUtil::calcFullPair = calc_pair_fullelect_acg;
		ComputeNonbondedUtil::calcFullPairEnergy = calc_pair_energy_fullelect_acg;
		ComputeNonbondedUtil::calcFullSelf = calc_self_fullelect_acg;
		ComputeNonbondedUtil::calcFullSelfEnergy = calc_self_energy_fullelect_acg;
		ComputeNonbondedUtil::calcMergePair = calc_pair_merge_fullelect_acg;
		ComputeNonbondedUtil::calcMergePairEnergy = calc_pair_energy_merge_fullelect_acg;
		ComputeNonbondedUtil::calcMergeSelf = calc_self_merge_fullelect_acg;
		ComputeNonbondedUtil::calcMergeSelfEnergy = calc_self_energy_merge_fullelect_acg;
		ComputeNonbondedUtil::calcSlowPair = calc_pair_slow_fullelect_acg;
		ComputeNonbondedUtil::calcSlowPairEnergy = calc_pair_energy_slow_fullelect_acg;
		ComputeNonbondedUtil::calcSlowSelf = calc_self_slow_fullelect_acg;
		ComputeNonbondedUtil::calcSlowSelfEnergy = calc_self_energy_slow_fullelect_acg;
	} else {
...
//fepe
	...
	//lyk
	group1 = simParams->group1;
	group2 = simParams->group2;
	cgGroup = simParams->cgGroup;
	aaGroup = simParams->aaGroup;
	cg_dielectric_1 = 1.0 / simParams->cgDielectric;
	cgVdwCappedN = simParams->cgVdwCappedN;
	cgVdwCappedSwitchF = simParams->cgVdwCappedSwitchF;
	cgScaling = simParams->cgScaling;
	//lyk
	if (!ljTable)
		ljTable = new LJTable;
	...
	if (simParams->switchingActive)
		splitType = SPLIT_SHIFT;
	if (simParams->martiniSwitching)
		splitType = SPLIT_MARTINI;
	if (simParams->fullDirectOn || simParams->FMAOn || PMEOn || MSMOn || simParams->FMMOn) {
	...
	if (!CkMyPe()) {
		iout<< iINFO << "NONBONDED TABLE R-SQUARED SPACING: " << r2_delta << "\n" << endi;
	}

	BigReal r2_tmp = 1.0;
	int cutoff2_exp = 0;
	while ((cutoff2 + r2_delta) > r2_tmp) {
		r2_tmp *= 2.0;
		cutoff2_exp += 1;
	}
	...
	if (!CkMyPe()) {
		iout<< iINFO << "NONBONDED TABLE SIZE: " << n << " POINTS\n" << endi;
	}
	...
	*(r2_i++) = r2_delta;
	...
	for (i = 1; i < n; ++i) {
		...
		case SPLIT_MARTINI: {
			...
			const BigReal p1 = 1.;
			BigReal A1 = p1 * ((p1 + 1) * COUL_SWITCH - (p1 + 4) * cutoff)
					/ (pow(cutoff, p1 + 2) * pow(cutoff - COUL_SWITCH, 2));
			BigReal B1 = -p1 * ((p1 + 1) * COUL_SWITCH - (p1 + 3) * cutoff)
					/ (pow(cutoff, p1 + 2) * pow(cutoff - COUL_SWITCH, 3));
			BigReal X1 = 1.0 / pow(cutoff, p1) - A1 / 3.0 * pow(cutoff - COUL_SWITCH, 3)
					- B1 / 4.0 * pow(cutoff - COUL_SWITCH, 4);
		...
		case SPLIT_C2:
			...
			//
			// calculate actual energy and gradient
			slow_energy = r2 / (cutoff * cutoff2) * (6.0 * (r2 / cutoff2) - 15.0 * (r / cutoff) + 10.0);
			slow_gradient = r / (cutoff * cutoff2) * (24.0 * (r2 / cutoff2) - 45.0 * (r / cutoff) + 20.0);
			// calculate scor from slow and corr
			scor_energy = slow_energy + (corr_energy - 1.0) / r;
			scor_gradient = slow_gradient - (corr_gradient - 1.0) / r2;
			// calculate fast from slow
		...
		if (simParams->vdwForceSwitching) {  // switch force
			...
			BigReal r12 = (r - switchOn) * (r - switchOn);
			BigReal r13 = (r - switchOn) * (r - switchOn) * (r - switchOn);

			BigReal p6 = 6;
			BigReal A6 = p6 * ((p6 + 1) * switchOn - (p6 + 4) * cutoff)
					/ (pow(cutoff, p6 + 2) * pow(cutoff - switchOn, 2));
			BigReal B6 = -p6 * ((p6 + 1) * switchOn - (p6 + 3) * cutoff)
					/ (pow(cutoff, p6 + 2) * pow(cutoff - switchOn, 3));
			BigReal C6 = 1.0 / pow(cutoff, p6) - A6 / 3.0 * pow(cutoff - switchOn, 3)
					- B6 / 4.0 * pow(cutoff - switchOn, 4);

			BigReal p12 = 12;
			BigReal A12 = p12 * ((p12 + 1) * switchOn - (p12 + 4) * cutoff)
					/ (pow(cutoff, p12 + 2) * pow(cutoff - switchOn, 2));
			BigReal B12 = -p12 * ((p12 + 1) * switchOn - (p12 + 3) * cutoff)
					/ (pow(cutoff, p12 + 2) * pow(cutoff - switchOn, 3));
			BigReal C12 = 1.0 / pow(cutoff, p12) - A12 / 3.0 * pow(cutoff - switchOn, 3)
					- B12 / 4.0 * pow(cutoff - switchOn, 4);
	...
	for (j = 0; j < 5; ++j) {
		...
		if (!CkMyPe()) {
			...
			if (dvmax != 0.0) {
				iout<< iINFO << "ABSOLUTE IMPRECISION IN " << table_name << " TABLE ENERGY: "
				<< dvmax << " AT " << dvmax_r << "\n" << endi;
			}
			if (fdvmax != 0.0) {
				iout<< iINFO << "RELATIVE IMPRECISION IN " << table_name << " TABLE ENERGY: "
				<< fdvmax << " AT " << fdvmax_r << "\n" << endi;
			}
			if (dgmax != 0.0) {
				iout<< iINFO << "ABSOLUTE IMPRECISION IN " << table_name << " TABLE FORCE: "
				<< dgmax << " AT " << dgmax_r << "\n" << endi;
			}
			if (fdgmax != 0.0) {
				iout<< iINFO << "RELATIVE IMPRECISION IN " << table_name << " TABLE FORCE: "
				<< fdgmax << " AT " << fdgmax_r << "\n" << endi;
			}
			if (fdgcdamax != 0.0) {
				iout<< iINFO << "INCONSISTENCY IN " << table_name << " TABLE ENERGY VS FORCE: "
				<< fdgcdamax << " AT " << fdgcdamax_r << "\n" << endi;
				if (fdgcdamax > 0.1) {
					iout << iERROR << "\n";
			...
			if (0 && fdgcdimax != 0.0) {
				iout<< iINFO << "INCONSISTENCY IN " << table_name << " TABLE ENERGY VS FORCE: "
				<< fdgcdimax << " AT " << fdgcdimax_r << "\n" << endi;
			}
			if (0 && fdgcaimax != 0.0) {
				iout<< iINFO << "INCONSISTENCY IN " << table_name << " TABLE AVG VS INT FORCE: "
				<< fdgcaimax << " AT " << fdgcaimax_r << "\n" << endi;
			}
		}
