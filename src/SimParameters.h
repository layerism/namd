...
//  ADD SPECIAL CODE TO send_SimParameters() and receive_SimParameters()
	...
	Bool selectConstraintsOn;       //  Flag TRUE-> selective restraints
									//  active
	Bool constrXOn, constrYOn, constrZOn;                 //  Flag TRUE-> select which Cartesian
...
//fepe
	...
	//lyk
	Bool itsOn;				   				// 	is itsOn
	int itsFirstStep;       				//  First ITS step
	int itsLastStep;        				//  Last ITS step
	int itsOutFreq;             			//  ITS output frequency
	int itsReplicaNum;
	int group1;								//	1 atom index
	int group2;								//	2 atom index
	int itsUpdateFreq;						//	expansion update
	int itsWhamTimes;
	int itsSamplingFreq;					//	sampling interval update
	char itsUpdateLog[128];
	char itsInFile[128];
	char itsIntType[128];
	Bool itsUpdateOn;
	Bool onlyIntOn;
	BigReal itsLbdMin;
	BigReal itsLbdMax;
	BigReal itsUpdateScaling;
	BigReal itsTemp;

	Bool adaptCgOn;							// 	AdaptCg
	int cgGroup;							//	cg atom index (nbond_dd slow_dd)
	int aaGroup;
	BigReal adaptCgFirstStep;       		//  First acg step
	BigReal adaptCgLastStep;        		//  Last acg step
	BigReal adaptCgTemp;
	int adaptCgWhamTimes;
	int adaptCgUpdateFreq;             		//  acg weight update frequency
	int adaptCgSamplingFreq;             	//  acg energy sampling frequency
	BigReal adaptCgLbdMax;
	BigReal adaptCgLbdMin;
	int adaptCgReplicaNum;
	BigReal adaptCgCvMax;
	BigReal adaptCgCvMin;
	BigReal adaptCgUpdateScaling;
	char adaptCgIntType[128];
	char adaptCgInFile[128];
	char adaptCgUpdateLog[128];
	int adaptCgOutFreq;             		//  acg output frequency
	Bool adaptCgUpdateOn;
	BigReal cgDielectric;
	BigReal cgVdwCappedN;
	BigReal cgVdwCappedSwitchF;
	BigReal cgScaling;
	Bool useDihedOn;
...
public:
	...
	int isRecvSpanningTreeOn() {
		return proxyRecvSpanningTree == 1;
	}
...
private:
	...
	void parse_mgrid_string_param(ConfigList *config, const char *fieldname, char** dest);
