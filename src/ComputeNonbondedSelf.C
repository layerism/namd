...
ComputeNonbondedSelf::ComputeNonbondedSelf(ComputeID c, PatchID pid, ComputeNonbondedWorkArrays* _workArrays,
		int minPartition, int maxPartition, int numPartitions) :
		ComputePatch(c, pid), workArrays(_workArrays), minPart(minPartition), maxPart(maxPartition), numParts(
				numPartitions) {
...
void ComputeNonbondedSelf::doForce(CompAtom* p, CompAtomExt* pExt, Results* r) {
	...
	if (!patch->flags.doGBIS || gbisPhase == 1) {
		...
		DebugM(2, "doForce() called.\n");DebugM(1, numAtoms << " patch 1 atoms\n");DebugM(3,
				"NUMATOMSxNUMATOMS = " << numAtoms * numAtoms << "\n");
		...
		//lyk
		params.vdwff_11[0] = r->f[Results::vdw11];
		params.vdwff_11[1] = r->f[Results::vdw11];
		params.vdwff_12[0] = r->f[Results::vdw12];
		params.vdwff_12[1] = r->f[Results::vdw12];
		params.vdwff_22[0] = r->f[Results::vdw22];
		params.vdwff_22[1] = r->f[Results::vdw22];

		params.vdwRepff_11[0] = r->f[Results::vdw11_rep];
		params.vdwRepff_11[1] = r->f[Results::vdw11_rep];
		params.vdwRepff_12[0] = r->f[Results::vdw12_rep];
		params.vdwRepff_12[1] = r->f[Results::vdw12_rep];
		params.vdwRepff_22[0] = r->f[Results::vdw22_rep];
		params.vdwRepff_22[1] = r->f[Results::vdw22_rep];

		params.electff_11[0] = r->f[Results::elect11];
		params.electff_11[1] = r->f[Results::elect11];
		params.electff_12[0] = r->f[Results::elect12];
		params.electff_12[1] = r->f[Results::elect12];
		params.electff_22[0] = r->f[Results::elect22];
		params.electff_22[1] = r->f[Results::elect22];
		//lyk
		...
		if (patch->flags.savePairlists) {
			...
			if (!pairlistsValid || (2. * patch->flags.maxAtomMovement > pairlistTolerance)) {
