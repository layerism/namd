...
public:
	...
	enum {
		crosstermEnergyIndex, TENSOR(virialIndex), reductionDataSize
	};
	enum {
	...
	CrosstermElem(AtomID atom0, AtomID atom1, AtomID atom2, AtomID atom3, AtomID atom4, AtomID atom5, AtomID atom6,
			AtomID atom7) {
		atomID[0] = atom0;
		atomID[1] = atom1;
		atomID[2] = atom2;
	...
	int operator==(const CrosstermElem &a) const {
		return (a.atomID[0] == atomID[0] && a.atomID[1] == atomID[1] && a.atomID[2] == atomID[2]
				&& a.atomID[3] == atomID[3] && a.atomID[4] == atomID[4] && a.atomID[5] == atomID[5]
...
public:
	...
	ComputeCrossterms(ComputeID c, PatchIDList &p) :
			ComputeHomeTuples<CrosstermElem, Crossterm, CrosstermValue>(c, p) {
		;
	}
...
public:
	...
	ComputeSelfCrossterms(ComputeID c, PatchID p) :
			ComputeSelfTuples<CrosstermElem, Crossterm, CrosstermValue>(c, p) {
		;
	}
