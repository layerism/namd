...
struct pe_sortop_bit_reversed {
	...
	inline bool operator()(int a, int b) const {
		...
		if (c > 0)
			return false;
		c = compare_bit_reversed(rankInPhysOfNode[CmiNodeOf(a)], rankInPhysOfNode[CmiNodeOf(b)]);
		if (c < 0)
			return true;
		if (c > 0)
			return false;
...
void WorkDistrib::buildNodeAwarePeOrdering() {
	...
	if (0 && CmiMyPe() == 0)
		for (int i = 0; i < numPe; ++i) {
			CkPrintf("order %5d %5d %5d %5d %5d\n", i, peDiffuseOrdering[i],
					peDiffuseOrderingIndex[i], peCompactOrdering[i], peCompactOrderingIndex[i]);
		}
...
static void recursive_bisect_coord(int x_begin, int x_end, int y_begin, int y_end, int *pe_begin,
		ScaledPosition *coord, int *result, int ydim) {
	int x_len = x_end - x_begin;
	int y_len = y_end - y_begin;
	if (x_len == 1 && y_len == 1) {
		// done, now put this pe in the right place
		if (0)
			CkPrintf("pme %5d %5d on pe %5d at %f %f\n", x_begin, y_begin, *pe_begin,
...
void WorkDistrib::recvComputeMapChanges(ComputeMapChangeMsg *msg) {
	...
	if (!CkMyPe()) { // send
		...
		msg->get(i);
		if (i != nc)
			NAMD_bug("WorkDistrib::recvComputeMapChanges check 1 failed\n");
		for (i = 0; i < nc; i++) {
			int data;
			msg->get(data);
		...
		msg->get(i);
		if (i != nc)
			NAMD_bug("WorkDistrib::recvComputeMapChanges check 2 failed\n");
		for (i = 0; i < nc; i++) {
			char data;
			msg->get(data);
		...
		msg->get(i);
		if (i != nc)
			NAMD_bug("WorkDistrib::recvComputeMapChanges check 3 failed\n");
		delete msg;
...
FullAtomList *WorkDistrib::createAtomLists(void) {
	...
	if (params->staticAtomAssignment) {
		...
		sortAtomsForPatches(order, breaks, sortAtoms.begin(), sortAtoms.size(), numAtoms,
				patchMap->gridsize_c(), patchMap->gridsize_b(), patchMap->gridsize_a());
		...
		for (int pid = 0; pid < numPatches; ++pid) {
			...
			CkPrintf("patch %d (%d %d %d) has %d atoms\n", pid, patchMap->index_a(pid),
					patchMap->index_b(pid), patchMap->index_c(pid), atoms[pid].size());
		}
		delete[] order;
		delete[] breaks;
	} else {
		...
		for (j = 0; j < n; j++) {
			...
			if (a[j].migrationGroupSize) {
				if (a[j].migrationGroupSize != a[j].hydrogenGroupSize) {
					...
					int mgs = a[j].migrationGroupSize;
					int c = 1;
					for (int k = a[j].hydrogenGroupSize; k < mgs; k += a[j + k].hydrogenGroupSize) {
						pos += a[j + k].position;
						++c;
					}
					pos *= 1. / c;
...
//Modifications for alchemical fep
			...
			//lyk
			Bool itsOn = params->itsOn;
			Bool adaptCgOn = params->adaptCgOn;
			if (adaptCgOn) {
				a[j].cgCharge = molecule->atomcgcharge(aid);
				a[j].cgType = molecule->atomcgtype(aid);
				a[j].cgMass = molecule->atomcgmass(aid);
			} else {
				a[j].cgCharge = 0.0;
				a[j].cgType = 0.0;
				a[j].cgMass = 0.0;
...
void WorkDistrib::distributeHomePatches() {
	...
	for (int i = 0; i < patchMap->numPatches(); i++) {
		if (patchMap->node(i) != node->myid()) {
			DebugM(3, "patchMgr->movePatch(" << i << "," << patchMap->node(i) << ")\n");
...
void WorkDistrib::savePatchMap(PatchMapMsg *msg) {
	...
	int pids[3];
	int npid = 0;
	if (send_far < range_end)
		pids[npid++] = CkNodeFirst(send_far);
	if (send_near < send_far)
		pids[npid++] = CkNodeFirst(send_near);
...
void WorkDistrib::patchMapInit(void) {
	...
	int numPatches = patchMap->sizeGrid(xmin, xmax, lattice, patchSize, 1.e9,
			params->staticAtomAssignment, twoAwayX ? 2 : 1, twoAwayY ? 2 : 1, twoAwayZ ? 2 : 1);
	if ((numPatches > (0.3 * availPes) || numPatches > maxNumPatches) && twoAwayZ < 0) {
		twoAwayZ = 0;
		numPatches = patchMap->sizeGrid(xmin, xmax, lattice, patchSize, 1.e9,
				params->staticAtomAssignment, twoAwayX ? 2 : 1, twoAwayY ? 2 : 1, twoAwayZ ? 2 : 1);
	}
	if ((numPatches > (0.6 * availPes) || numPatches > maxNumPatches) && twoAwayY < 0) {
		twoAwayY = 0;
		numPatches = patchMap->sizeGrid(xmin, xmax, lattice, patchSize, 1.e9,
				params->staticAtomAssignment, twoAwayX ? 2 : 1, twoAwayY ? 2 : 1, twoAwayZ ? 2 : 1);
	}
	if ((numPatches > availPes || numPatches > maxNumPatches) && twoAwayX < 0) {
		twoAwayX = 0;
		numPatches = patchMap->sizeGrid(xmin, xmax, lattice, patchSize, 1.e9,
				params->staticAtomAssignment, twoAwayX ? 2 : 1, twoAwayY ? 2 : 1, twoAwayZ ? 2 : 1);
	}
	if (numPatches > availPes && numPatches <= (1.4 * availPes) && availPes <= maxNumPatches) {
		int newNumPatches = patchMap->sizeGrid(xmin, xmax, lattice, patchSize, availPes,
				params->staticAtomAssignment, twoAwayX ? 2 : 1, twoAwayY ? 2 : 1, twoAwayZ ? 2 : 1);
...
void WorkDistrib::assignPatchesBitReversal() {
	...
	if (ncpus > 2 * npatches)
		sortNodesAndAssign(seq.begin() + npatches, 1);
}
...
struct nodesort {
	...
	int npatches;
	nodesort() :
			node(-1), a_total(0), b_total(0), c_total(0), npatches(0) {
		;
	}
...
void WorkDistrib::sortNodesAndAssign(int *assignedNode, int baseNodes) {
	...
	usednodes.resize(0);
	for (i = 0; i < nnodes; ++i) {
		if (allnodes[i].npatches)
			usednodes.add(allnodes[i]);
	}
	usednodes.sort();
	int i2 = 0;
	for (i = 0; i < nnodes; ++i) {
		int pe = peCompactOrdering[i];
		if (allnodes[pe].npatches)
			allnodes[usednodes[i2++].node].node = pe;
	}
...
#endif
			if (a < 0 || a >= na)
				NAMD_bug("inconsistent torus topology!");
			if (b < 0 || b >= nb)
				NAMD_bug("inconsistent torus topology!");
			if (c < 0 || c >= nc)
				NAMD_bug("inconsistent torus topology!");
...
static void recursive_bisect_with_curve(int *patch_begin, int *patch_end, int *node_begin,
		int *node_end, double *patchLoads, double *sortedLoads, int *assignedNode,
		TopoManagerWrapper &tmgr) {
	...
	for (int i = 0; i < npatches; ++i) {
		int pid = patches[i];
		if (patchLoads[pid] > maxPatchLoad)
			patchLoads[pid] = maxPatchLoad;
	...
	if (node_split == node_begin) {
		if (0) {
			...
			CkPrintf(
					...
					CmiPhysicalNodeID(*node_begin), *node_begin, CkNodeOf(*node_begin), crds[0],
					crds[1], crds[2], a_min, b_min, c_min, npatches, a_len + 1, b_len + 1,
					c_len + 1, totalRawLoad, nnodes);
		}
		...
		for (int i = 0; i < npatches; ++i) {
			...
			double targetLoad = totalLoad * ((double) (node - node_begin + 1) / (double) nnodes);
			if (0)
				CkPrintf("assign %5d node %5d patch %5d %5d %5d load %7f target %7f\n", i, *node,
						patchMap->index_a(pid), patchMap->index_b(pid), patchMap->index_c(pid),
						sumLoad, targetLoad);
			double extra = (i + 1 < npatches ? 0.5 * patchLoads[patches[i + 1]] : 0);
			if (node + 1 < node_end && sumLoad + extra >= targetLoad) {
				++node;
			}
		}
	...
	{ // walk through patches in sorted order
		int *node = node_begin;
		sumLoad = 0;
		for (patch_split = patch_begin; patch_split != patch_end && node != node_split;
				++patch_split) {
			sumLoad += patchLoads[*patch_split];
			double targetLoad = totalLoad * ((double) (node - node_begin + 1) / (double) nnodes);
			if (0)
				CkPrintf("test %5d node %5d patch %5d %5d %5d load %7f target %7f\n",
						patch_split - patch_begin, *node, patchMap->index_a(*patch_split),
						patchMap->index_b(*patch_split), patchMap->index_c(*patch_split), sumLoad,
						targetLoad);
		...
		double targetLoad = totalLoad * ((double) (node_split - node_begin) / (double) nnodes);
		if (0)
			CkPrintf("split node %5d/%5d patch %5d/%5d load %7f target %7f\n",
					node_split - node_begin, nnodes, patch_split - patch_begin, npatches, sumLoad,
					targetLoad);
	}

	// recurse
	recursive_bisect_with_curve(patch_begin, patch_split, node_begin, node_split, patchLoads,
			sortedLoads, assignedNode, tmgr);
	recursive_bisect_with_curve(patch_split, patch_end, node_split, node_end, patchLoads,
			sortedLoads, assignedNode, tmgr);
}
...
void WorkDistrib::assignPatchesSpaceFillingCurve() {
	...
	recursive_bisect_with_curve(patchOrdering.begin(), patchOrdering.end(), node_begin, node_end,
	...
	for (int pid = 0; pid < numPatches; ++pid) {
		...
		int nodeidx = std::lower_bound(node_begin, node_end, node, pe_sortop_compact())
				- node_begin;
		int basenode = basenode_begin[nodeidx];
		patchMap->assignBaseNode(pid, basenode);
		if (CmiPeOnSamePhysicalNode(node, basenode))
			++samenodecount;
	}
...
void WorkDistrib::mapComputes(void) {
	...
	if (!node->simParameters->pairInteractionOn || node->simParameters->pairInteractionSelf) {
...
//    iout <<"Self numPartitions = " <<numPartitions <<" numAtoms " <<numAtoms <<std::endl;
	...
	for (int p1 = 0; p1 < patchMap->numPatches(); p1++) // do the pairs
		...
		numNeighbors = patchMap->oneOrTwoAwayNeighbors(p1, oneAway, oneAwayDownstream,
				oneAwayTrans);
...
#endif
			for (int partition = 0; partition < numPartitions; partition++) {
				cid = computeMap->storeCompute(patchMap->basenode(dsp), 2, computeNonbondedPairType,
						partition, numPartitions);
...
void WorkDistrib::mapComputeLCPO(void) {
	...
	// one octet per patch
	for (i = 0; i < patchMap->numPatches(); i++) {
		numPatchesInOctet = patchMap->getPatchesInOctet(i, patchesInOctet, oneAwayTrans);

		for (int partition = 0; partition < numPartitions; partition++) {
			cid = computeMap->storeCompute(patchMap->node(i), numPatchesInOctet, computeLCPOType,
					partition, numPartitions);
...
void WorkDistrib::velocities_from_PDB(char *filename, Vector *v, int totalAtoms) {
	...
	if (v_pdb->num_atoms() != totalAtoms) {
		...
		sprintf(err_msg, "FOUND %d COORDINATES IN VELOCITY PDB!!", v_pdb->num_atoms());
...
void WorkDistrib::random_velocities(BigReal Temp, Molecule *structure, Vector *v, int totalAtoms) {
	int i, j;		//  Loop counter
	BigReal kbT;		//  Boltzman constant * Temp
	BigReal randnum;	//  Random number from -6.0 to 6.0
	BigReal kbToverM;	//  sqrt(Kb*Temp/Mass)
	...
	for (i = 0; i < totalAtoms; i++) {
		if (structure->atommass(i) <= 0.) {
			...
			kbToverM = sqrt(
					kbT * (lesOn && structure->get_fep_type(i) ? tempFactor : 1.0)
