...
Controller::Controller(NamdState *s) :
		computeChecksum(0), marginViolations(0), pairlistWarnings(0), simParams(Node::Object()->simParameters), state(
				s), collection(CollectionMaster::Object()), startCTime(0), firstCTime(CmiTimer()), startWTime(0), firstWTime(
				CmiWallTimer()), startBenchTime(0), ldbSteps(0), fflush_count(3) {
	...
	// for accelMD
	if (simParams->accelMDOn || simParams->itsOn || simParams->adaptCgOn) { //lyk
		amd_reduction = ReductionMgr::Object()->willRequire(REDUCTIONS_AMD);
	} else {
		amd_reduction = NULL;
	}
	//lyk
	if (simParams->adaptCgOn) {
		DENSITYALAPHA = simParams->DENSITYALAPHA;
		SKIP = simParams->WHAMSKIP;
		LOGZERROR = simParams->WHAMLOGZERROR;
		if (strcmp(simParams->adaptCgIntType, "vdw") == 0) {
			acg_intType = 0;
		} else if (strcmp(simParams->adaptCgIntType, "elect") == 0) {
			acg_intType = 1;
		} else if (strcmp(simParams->adaptCgIntType, "vdw_elect") == 0) {
			acg_intType = 2;
		} else if (strcmp(simParams->adaptCgIntType, "attvdw") == 0) {
			acg_intType = 3;
		} else if (strcmp(simParams->adaptCgIntType, "attvdw_elect") == 0) {
			acg_intType = 4;
		} else {
			acg_intType = -1;
			NAMD_die("NO SUCH INTERACTION TYPE! ALLOW TYPE: vdw/elect/vdw_elect/attvdw/attvdw_elect");
		}
	}
	if (simParams->itsOn) {
		DENSITYALAPHA = simParams->DENSITYALAPHA;
		SKIP = simParams->WHAMSKIP;
		LOGZERROR = simParams->WHAMLOGZERROR;
		if (strcmp(simParams->itsIntType, "vdw") == 0) {
			its_intType = 0;
		} else if (strcmp(simParams->itsIntType, "elect") == 0) {
			its_intType = 1;
		} else if (strcmp(simParams->itsIntType, "vdw_elect") == 0) {
			its_intType = 2;
		} else if (strcmp(simParams->itsIntType, "attvdw") == 0) {
			its_intType = 3;
		} else if (strcmp(simParams->itsIntType, "attvdw_elect") == 0) {
			its_intType = 4;
		} else {
			its_intType = -1;
			NAMD_die("NO SUCH INTERACTION TYPE! ALLOW TYPE: vdw/elect/vdw_elect/attvdw/attvdw_elect");
	...
	if (simParams->pressureProfileOn || simParams->pressureProfileEwaldOn) {
		...
		if (simParams->pressureProfileOn) {
			ppbonded = new PressureProfileReduction(REDUCTIONS_PPROF_BONDED, nslabs, npairs, "BONDED", freq);
			ppnonbonded = new PressureProfileReduction(REDUCTIONS_PPROF_NONBONDED, nslabs, npairs, "NONBONDED", freq);
			// internal partitions by atom type, but not in a pairwise fashion
			ppint = new PressureProfileReduction(REDUCTIONS_PPROF_INTERNAL, nslabs, ntypes, "INTERNAL", freq);
		} else {
	...
	langevinPiston_strainRate = Tensor::symmetric(simParams->strainRate, simParams->strainRate2);
...
Controller::~Controller(void) {
	...
	/*
	 ...
	 delete[] acg_aaEnergySamples;
	 delete[] acg_cgEnergySamples;
	 delete[] acg_logZ;
	 delete[] acg_effCvSamples;
	 #ifdef WHAMFLAG
	 delete[] acg_whamLogZ;
	 DELETE2D(acg_logZHis, simParams->adaptCgWhamTimes, 0);
	 DELETE2D(acg_cvEff, acg_samplesNum_wham / SKIP, 0);
	 delete[] acg_logZEffHis;
	 delete[] acg_aaEnergySamples_wham;
	 delete[] acg_cgEnergySamples_wham;
	 #endif
	 */
...
void Controller::minimize() {
	...
	for (int i_slow = 0; min_huge_count && i_slow < 100; ++i_slow) {
		if (min_huge_count >= old_min_huge_count) {
			if (++steps_at_same_min_huge_count > 10)
				break;
		} else {
...
#define PRINT_BRACKET \
    iout << "LINE MINIMIZER BRACKET: DX " \
		...
		for (itcnt = 10; itcnt > 0; --itcnt) {
			...
			if (fabs(last.dudx) < tol)
				break;
			if (lo.x != mid.x && (lo.u - mid.u) < tol * (mid.x - lo.x))
				break;
			if (mid.x != hi.x && (hi.u - mid.u) < tol * (hi.x - mid.x))
				break;
		}
		// new direction
		broadcast->minimizeCoefficient.publish(minSeq++, 0.);
...
void Controller::berendsenPressure(int step) {
	if (simParams->berendsenPressureOn) {
		...
		if (!(berendsenPressure_count % freq)) {
			...
			factor *= ((simParams->berendsenPressureCompressibility / 3.0) * simParams->dt * freq
					/ simParams->berendsenPressureRelaxationTime);
...
void Controller::langevinPiston1(int step) {
	if (simParams->langevinPistonOn) {
		...
		strainRate += (0.5 * dt * cellDims * state->lattice.volume() / mass) * (controlPressure - ptarget);

		if (simParams->fixCellDims) {
			if (simParams->fixCellDimX)
				strainRate.xx = 0;
			if (simParams->fixCellDimY)
				strainRate.yy = 0;
			if (simParams->fixCellDimZ)
				strainRate.zz = 0;
		}
...
void Controller::rescaleVelocities(int step) {
	...
	if (rescaleFreq > 0) {
		rescaleVelocities_sumTemps += temperature;
...
void Controller::tcoupleVelocities(int step) {
	if (simParams->tCoupleOn) {
		...
		if (temperature > 0.)
			coefficient = tCoupleTemp / temperature - 1.;
...
void Controller::receivePressure(int step, int minimize) {
	...
	BigReal groupKineticEnergyHalfstep = kineticEnergyHalfstep - reduction->item(REDUCTION_INT_HALFSTEP_KINETIC_ENERGY);
	BigReal groupKineticEnergyCentered = kineticEnergyCentered - reduction->item(REDUCTION_INT_CENTERED_KINETIC_ENERGY);

	BigReal atomTempHalfstep = 2.0 * kineticEnergyHalfstep / (numDegFreedom * BOLTZMANN);
	BigReal atomTempCentered = 2.0 * kineticEnergyCentered / (numDegFreedom * BOLTZMANN);
	BigReal groupTempHalfstep = 2.0 * groupKineticEnergyHalfstep / (numGroupDegFreedom * BOLTZMANN);
	BigReal groupTempCentered = 2.0 * groupKineticEnergyCentered / (numGroupDegFreedom * BOLTZMANN);
	...
	if (simParameters->drudeOn) {
		BigReal drudeComKE = reduction->item(REDUCTION_DRUDECOM_CENTERED_KINETIC_ENERGY);
		BigReal drudeBondKE = reduction->item(REDUCTION_DRUDEBOND_CENTERED_KINETIC_ENERGY);
	...
	if ((volume = lattice.volume()) != 0.) {
		...
		//lyk
		if (simParams->adaptCgOn) {
			pressure_acg = virial_acg / volume;
			pressure_normal += pressure_acg;
			groupPressure_normal += pressure_acg;
		}
		if (simParams->itsOn) {
			pressure_its = virial_its / volume;
			pressure_normal += pressure_its;
			groupPressure_normal += pressure_its;
		}
		//lyk
	...
	if (simParameters->useGroupPressure) {
		...
		if (!(numFixedAtoms || molecule->numConstraints || simParameters->comMove || simParameters->langevinOn)) {
	...
	if (simParameters->fixCellDims) {
		if (simParameters->fixCellDimX)
			controlNumDegFreedom -= 1;
		if (simParameters->fixCellDimY)
			controlNumDegFreedom -= 1;
		if (simParameters->fixCellDimZ)
			controlNumDegFreedom -= 1;
	}
...
#undef AVGXY
		...
		controlPressure_normal = Tensor::identity(trace(controlPressure_normal) / 3.);
		controlPressure_nbond = Tensor::identity(trace(controlPressure_nbond) / 3.);
		controlPressure_slow = Tensor::identity(trace(controlPressure_slow) / 3.);
		controlPressure = Tensor::identity(trace(controlPressure) / 3.);
	}
...
#endif
	if ((simParams->accelMDOn || simParams->itsOn || simParams->adaptCgOn) //lyk
	&& simParams->accelMDDebugOn && !(step % simParameters->accelMDOutFreq)) {
...
//    if ( minimize || ((step < simParams->accelMDFirstStep ) || (step > simParams->accelMDLastStep ))) return;
	if (minimize || (step < simParams->accelMDFirstStep)
			|| (simParams->accelMDLastStep > 0 && step > simParams->accelMDLastStep))
		return;
	...
	potentialEnergy = bondEnergy + angleEnergy + dihedralEnergy + improperEnergy + crosstermEnergy + amd_electEnergy
			+ amd_electEnergySlow + amd_ljEnergy;
...
//lyk-----------------ITS && ACG
void Controller::itsMD(int step, int minimize) {
	if (!simParams->itsOn)
		return;

	if (minimize || step < simParams->itsFirstStep || step >= simParams->itsLastStep)
		return;

	amd_reduction->require();
	BigReal kbT = BOLTZMANN * (simParams->itsTemp);
	//------------------------------------------------------
	//					get its energy samples
	//------------------------------------------------------
	if (simParams->onlyIntOn) {
		its_lj = amd_reduction->item(REDUCTION_12_LJ_ENERGY);
		its_replj = amd_reduction->item(REDUCTION_12_REPLJ_ENERGY);
		its_elect = amd_reduction->item(REDUCTION_12_ELECT_ENERGY);
	} else {
		its_lj = amd_reduction->item(REDUCTION_11_LJ_ENERGY) + amd_reduction->item(REDUCTION_12_LJ_ENERGY)
				+ amd_reduction->item(REDUCTION_22_LJ_ENERGY);
		its_replj = amd_reduction->item(REDUCTION_11_REPLJ_ENERGY) + amd_reduction->item(REDUCTION_12_REPLJ_ENERGY)
				+ amd_reduction->item(REDUCTION_22_REPLJ_ENERGY);
		its_elect = amd_reduction->item(REDUCTION_11_ELECT_ENERGY) + amd_reduction->item(REDUCTION_12_REPLJ_ENERGY)
				+ amd_reduction->item(REDUCTION_22_REPLJ_ENERGY);
	}
	...
	if (simParams->group1 == -1) {
		BigReal its_dihedralEnergy = amd_reduction->item(REDUCTION_DIHEDRAL_ENERGY);
		BigReal its_improperEnergy = amd_reduction->item(REDUCTION_IMPROPER_ENERGY);
		BigReal its_crosstermEnergy = amd_reduction->item(REDUCTION_CROSSTERM_ENERGY);
		BigReal its_electEnergy = 0.0;
		BigReal its_ljEnergy = 0.0;
		BigReal its_electEnergySlow = 0.0;
		if (!(step % nbondFreq)) {
			its_electEnergy = amd_reduction->item(REDUCTION_ELECT_ENERGY);
			its_ljEnergy = amd_reduction->item(REDUCTION_LJ_ENERGY);
		} else {
			its_electEnergy = electEnergy;
			its_ljEnergy = ljEnergy;
		}

		if (!(step % slowFreq)) {
			its_electEnergySlow = amd_reduction->item(REDUCTION_ELECT_ENERGY_SLOW);
		} else {
			its_electEnergySlow = electEnergySlow;
		}
		energy = its_dihedralEnergy + its_improperEnergy + its_crosstermEnergy + its_electEnergy + its_ljEnergy
				+ its_electEnergySlow;
	}
	//------------------------------------------------------
	//					init its
	...
	if (step == simParams->itsFirstStep) {
		its_init();
	}
	...
	if (simParams->itsUpdateOn) {
		...
		//					update weight
		//------------------------------------------------------
		if (!(step % simParams->itsUpdateFreq) && step != simParams->itsFirstStep) {
			//
			// get mean and sgm using single simulation data
			// Zi/Zeff=<exp(-beta*(Ui-Ueff))>_eff
			// meani=<exp(-beta*(Ui-Ueff))*mean>_eff/<exp(-beta*(Ui-Ueff))>_eff
			// sgmi= <mean*mean>_i-meani^2
			//
			BigReal logz[its_replicaNum];
			BigReal mean[its_replicaNum];
			BigReal sgm[its_replicaNum];
			its_estimate(mean, sgm, logz);

			//
			//its or lyk update logZ
			//
			BigReal weight[its_replicaNum];
			its_eflatUpdateLogZ(step, logz, weight);

#ifdef WHAMFLAG
			int updateTimes = int(step / simParams->itsUpdateFreq);
			int next_l = updateTimes % simParams->itsWhamTimes;
			its_cvEffHis(step, its_cvEff);
			int loop = 0;
			BigReal error;

			if (updateTimes % 10 == 0) {
				for (loop = 0; loop < MAXLOOP; ++loop) {
					error = its_whamEstimateLogZEff(step, its_cvEff, its_logZEffHis);
					if (error <= LOGZERROR)
						break;
				}
				iout<<"WHAM-FIT: STEP: "<<step<<" STOP-AT: "<<loop+1<<"/"<<MAXLOOP<<"  MAX-ERROR: "<<error<<"\n"<<endi;
				its_whamEstimate(step, its_cvEff, its_whamLogZ, its_meanCV, its_sgmCV);
			}

			for (int i = 0; i < its_replicaNum; ++i) {
				its_logZHis[next_l][i] = its_logZ[i];
			}
#endif

			//output log
			its_updateLog << std::setiosflags(std::ios_base::left) << std::setw(7) << "index" << " " << std::setw(12)
					<< "lbd" << " " << std::setw(15) << "DWS:" << " " << std::setw(10) << "weight" << " "
					<< std::setw(15) << "mean-Cv" << " " << std::setw(12) << "sgm-Cv" << " " << std::setw(12)
					<< "lbd-Hist" << " " << std::setw(12) << "logZ" << " " << std::setw(10) << "logz_wham" << " "
					<< std::setw(5) << int(step / simParams->itsUpdateFreq) << "\n";
			for (int i = 0; i < its_replicaNum; ++i) {
				BigReal lbd_i = its_lbd(i);
				its_updateLog << std::setiosflags(std::ios_base::left) << std::setw(7) << i << " " << std::setw(12)
						<< lbd_i << " " << std::setw(15) << weight[i] << " " << std::setw(10) << its_replicaWeight[i]
						<< " " << std::setw(15) << its_meanCV[i] << " " << std::setw(12) << its_sgmCV[i] << " "
						<< std::setw(12) << its_lbdHistogram[i] << " " << std::setw(12) << its_logZ[i] << " "
						<< std::setw(12) << its_whamLogZ[i] << "\n";
				its_dynamicWeight[i] = 0.0;
			}
			its_updateLog << std::setiosflags(std::ios_base::left) << std::setw(15) << its_lbdMin << std::setw(15)
					<< its_lbdMax << "\n\n";
			its_updateLog.flush();

			for (int l = 0; l < simParams->itsWhamTimes; ++l) {
				its_whamLog << std::setiosflags(std::ios_base::left) << std::setw(12) << its_logZEffHis[l] << " ";
			}
			its_whamLog << "\n";
			its_whamLog.flush();
		}
	...
	//  					update factor
	//
	//				sum{exp(ener_up)} / sum{ exp(ener_down)}
	//------------------------------------------------------
	BigReal eff_cv = its_eff_cv(energy, its_logZ);
	its_rescalingFactor = its_eff_lbd(energy, its_logZ);
	its_updateVirial(its_rescalingFactor);
	//------------------------------------------------------
	//					update lbd Histogram and samples
	//------------------------------------------------------

	if (!(step % simParams->itsSamplingFreq) && simParams->itsUpdateOn) {
		BigReal current_lbd = its_rescalingFactor[0];
		BigReal dlbd = (its_lbdMax - its_lbdMin) / BigReal(its_replicaNum - 1);
		int ilbd = int((current_lbd - its_lbdMin) / dlbd + 0.5);
		its_lbdHistogram[ilbd] += 1.0;

#ifdef WHAMFLAG
		int counter_wham = int(step / simParams->itsSamplingFreq) % its_samplesNum_wham;
		its_energySamples_wham[counter_wham] = energy;
#endif
		int counter = int(step / simParams->itsSamplingFreq) % its_samplesNum;
		its_energySamples[counter] = energy;
		its_effCvSamples[counter] = eff_cv;
	}
	//------------------------------------------------------
	//					send force rescaling factor
	//------------------------------------------------------
	broadcast->forceRescaling.publish(step, its_rescalingFactor);

	if (!(step % simParams->itsOutFreq)) {
		if (simParams->group1 == -1) {
			iout<<"ITS: "<< step
			<<" totE: "<< energy
			<<" effE: "<<eff_cv
			<<" efflbd "<<its_rescalingFactor[0]
			<<"\n"<< endi;
		} else {
			iout<<"ITS: "<< step
			<<" REPLJ: " << its_replj
			<<" ATTLJ: " << its_lj-its_replj
			<<" ELECT: " << its_elect
			<<" effE: "<<eff_cv
			<<" efflbd "<<its_rescalingFactor[0]
			<<"\n"<< endi;
...
void Controller::adaptCgMD(int step, int minimize) {
	if (!simParams->adaptCgOn)
		return;

	if (minimize || step < simParams->adaptCgFirstStep || step >= simParams->adaptCgLastStep)
		return;

	amd_reduction->require();
	BigReal kbT = BOLTZMANN * (simParams->adaptCgTemp);
	//------------------------------------------------------
	//					get acg energy samples
	//------------------------------------------------------
	aa_lj = amd_reduction->item(REDUCTION_11_LJ_ENERGY);
	aa_replj = amd_reduction->item(REDUCTION_11_REPLJ_ENERGY);
	aa_elect = amd_reduction->item(REDUCTION_11_ELECT_ENERGY);
	cg_lj = amd_reduction->item(REDUCTION_22_LJ_ENERGY);
	cg_replj = amd_reduction->item(REDUCTION_22_REPLJ_ENERGY);
	cg_elect = amd_reduction->item(REDUCTION_22_ELECT_ENERGY);

	BigReal aaEnergy = 0.0;
	BigReal cgEnergy = 0.0;

	acg_selectEnergy(aaEnergy, cgEnergy);

	if (simParams->aaGroup == -1) {
		BigReal acg_dihedralEnergy = amd_reduction->item(REDUCTION_DIHEDRAL_ENERGY);
		BigReal acg_improperEnergy = amd_reduction->item(REDUCTION_IMPROPER_ENERGY);
		BigReal acg_crosstermEnergy = amd_reduction->item(REDUCTION_CROSSTERM_ENERGY);

		BigReal acg_electEnergy = 0.0;
		BigReal acg_ljEnergy = 0.0;
		BigReal acg_electEnergySlow = 0.0;
		if (!(step % nbondFreq)) {
			acg_electEnergy = amd_reduction->item(REDUCTION_ELECT_ENERGY);
			acg_ljEnergy = amd_reduction->item(REDUCTION_LJ_ENERGY);
		} else {
			acg_electEnergy = electEnergy;
			acg_ljEnergy = ljEnergy;
		}
		if (!(step % slowFreq)) {
			acg_electEnergySlow = amd_reduction->item(REDUCTION_ELECT_ENERGY_SLOW);
		} else {
			acg_electEnergySlow = electEnergySlow;
		}
		if (simParams->useDihedOn) {
			aaEnergy = acg_dihedralEnergy + acg_improperEnergy + acg_crosstermEnergy + acg_electEnergy + acg_ljEnergy
					+ acg_electEnergySlow;
		} else {
			aaEnergy = acg_electEnergy + acg_ljEnergy + acg_electEnergySlow;
		}

		/*iout<<"dihe: "<<acg_dihedralEnergy+acg_crosstermEnergy<<" "
		 <<"impdihe: "<<acg_improperEnergy<<" "
		 <<"elect: "<<acg_electEnergy+acg_electEnergySlow<<" "
		 <<"lj: "<<acg_ljEnergy<<"\n"<<endi;*/
	...
	if (step == simParams->adaptCgFirstStep) {
		acg_init();
	}
	...
	if (simParams->adaptCgUpdateOn) {
		...
		//					update weight
		//------------------------------------------------------
		if (!(step % simParams->adaptCgUpdateFreq) && step != simParams->adaptCgFirstStep) {
			//
			// get mean and sgm using single simulation data
			// Zi/Zeff=<exp(-beta*(Ui-Ueff))>_eff
			// meani=<exp(-beta*(Ui-Ueff))*mean>_eff/<exp(-beta*(Ui-Ueff))>_eff
			// sgmi= <mean*mean>_i-meani^2
			//
			BigReal logz[acg_replicaNum];
			BigReal mean[acg_replicaNum];
			BigReal sgm[acg_replicaNum];
			acg_estimate(mean, sgm, logz);

			//
			//its or lyk update logZ
			//
			//acg_purifyLogZ(step, logz);
			//for (int i = 0; i < acg_replicaNum; ++i) {
			//	acg_logZ[i] = logz[i] - logz[0];
			//}
			BigReal weight[acg_replicaNum];
			acg_eflatUpdateLogZ(step, logz, weight);

#ifdef WHAMFLAG
			int updateTimes = int(step / simParams->adaptCgUpdateFreq);
			int next_l = updateTimes % simParams->adaptCgWhamTimes;
			acg_cvEffHis(step, acg_cvEff);
			int loop = 0;
			BigReal error;

			if (updateTimes % 10 == 0) {
				for (loop = 0; loop < MAXLOOP; ++loop) {
					error = acg_whamEstimateLogZEff(step, acg_cvEff, acg_logZEffHis);
					if (error <= LOGZERROR)
						break;
				}
				iout<<"WHAM-FIT: STEP: "<<step<<" STOP-AT: "<<loop+1<<"/"<<MAXLOOP<<"  MAX-ERROR: "<<error<<"\n"<<endi;
				acg_whamEstimate(step, acg_cvEff, acg_whamLogZ, acg_meanCV, acg_sgmCV);
			}

			for (int i = 0; i < acg_replicaNum; ++i) {
				acg_logZHis[next_l][i] = acg_logZ[i];
			}
#endif

			//output log
			acg_updateLog << std::setiosflags(std::ios_base::left) << std::setw(7) << "index" << " " << std::setw(12)
					<< "lbd" << " " << std::setw(15) << "DWS:" << " " << std::setw(10) << "weight" << " "
					<< std::setw(15) << "mean-Cv" << " " << std::setw(12) << "sgm-Cv" << " " << std::setw(12)
					<< "lbd-Hist" << " " << std::setw(12) << "logZ" << " " << std::setw(10) << "logz_wham" << " "
					<< std::setw(5) << int(step / simParams->adaptCgUpdateFreq) << "\n";
			for (int i = 0; i < acg_replicaNum; ++i) {
				BigReal lbd_i = acg_lbd(i);
				acg_updateLog << std::setiosflags(std::ios_base::left) << std::setw(7) << i << " " << std::setw(12)
						<< lbd_i << " " << std::setw(15) << weight[i] << " " << std::setw(10) << acg_replicaWeight[i]
						<< " " << std::setw(15) << acg_meanCV[i] << " " << std::setw(12) << acg_sgmCV[i] << " "
						<< std::setw(12) << acg_lbdHistogram[i] << " " << std::setw(12) << acg_logZ[i] << " "
						<< std::setw(12) << acg_whamLogZ[i] << "\n";
				acg_dynamicWeight[i] = 0.0;
			}
			acg_updateLog << std::setiosflags(std::ios_base::left) << std::setw(15) << acg_lbdMin << std::setw(15)
					<< acg_lbdMax << "\n\n";
			acg_updateLog.flush();

			for (int l = 0; l < simParams->adaptCgWhamTimes; ++l) {
				acg_whamLog << std::setiosflags(std::ios_base::left) << std::setw(12) << acg_logZEffHis[l] << " ";
			}
			acg_whamLog << "\n";
			acg_whamLog.flush();
		}
	...
	//				sum{exp(ener_up)} / sum{ exp(ener_down)}
	//------------------------------------------------------
	BigReal eff_cv = acg_eff_cv(aaEnergy, cgEnergy, acg_logZ);
	acg_rescalingFactor = acg_eff_lbd(aaEnergy, cgEnergy, acg_logZ);
	acg_updateVirial(acg_rescalingFactor);
	//------------------------------------------------------
	//					update lbd Histogram and samples
	//------------------------------------------------------

	if (!(step % simParams->adaptCgSamplingFreq) && simParams->adaptCgUpdateOn) {
		BigReal current_lbd = -acg_rescalingFactor[0];
		BigReal dlbd = (acg_lbdMax - acg_lbdMin) / BigReal(acg_replicaNum - 1);
		int ilbd = int((current_lbd - acg_lbdMin) / dlbd + 0.5);
		acg_lbdHistogram[ilbd] += 1.0;

#ifdef WHAMFLAG
		int counter_wham = int(step / simParams->adaptCgSamplingFreq) % acg_samplesNum_wham;
		acg_aaEnergySamples_wham[counter_wham] = aaEnergy;
		acg_cgEnergySamples_wham[counter_wham] = cgEnergy;
#endif
		int counter = int(step / simParams->adaptCgSamplingFreq) % acg_samplesNum;
		acg_aaEnergySamples[counter] = aaEnergy;
		acg_cgEnergySamples[counter] = cgEnergy;
		acg_effCvSamples[counter] = eff_cv;
	}
	//------------------------------------------------------
	//					send force rescaling factor
	//------------------------------------------------------
	broadcast->forceRescaling.publish(step, acg_rescalingFactor);

	if (!(step % simParams->adaptCgOutFreq)) {
		if (simParams->aaGroup == -1) {
			iout<<"ACG: "<< step
			<<" aa: "<< aaEnergy
			<<" cgREPLJ: " <<cg_replj
			<<" cgATTLJ: " <<cg_lj-cg_replj
			<<" cgELECT: " <<cg_elect
			<<" effE: "<<eff_cv
			<<" efflbd "<<acg_rescalingFactor[1]
			<<"\n"<< endi;
		} else {
			iout<<"ACG: "<< step
			<<" aa/cgREPLJ: " << aa_replj <<" " <<cg_replj
			<<" aa/cgATTLJ: " << aa_lj-aa_replj <<" " <<cg_lj-cg_replj
			<<" aa/cgELECT: " << aa_elect <<" "<<cg_elect
			<<" effE: "<<eff_cv
			<<" efflbd "<<acg_rescalingFactor[1]
			<<"\n"<< endi;
		}
...
void Controller::adaptTempUpdate(int step, int minimize) {
	...
	adaptTempPotEnergyVarNum[adaptTempBin] = adaptTempPotEnergyVarNum[adaptTempBin] * gammaAve
	...
	if (!(step % simParams->adaptTempFreq)) {
		...
		if (adaptTempBin > 0 && adaptTempBin < adaptTempBins - 1) {
			...
			int nMinus = 0;
			int nPlus = 0;
			if (adaptTempBeta - adaptTempBetaMin < deltaBeta)
				deltaBeta = adaptTempBeta - adaptTempBetaMin;
			if (adaptTempBetaMax - adaptTempBeta < deltaBeta)
				deltaBeta = adaptTempBetaMax - adaptTempBeta;
...
void Controller::compareChecksums(int step, int forgiving) {
	...
	if (((int) checksum) > molecule->numCalcExclusions && (!simParams->mollyOn || step % slowFreq)) {
		if (forgiving)
...
void Controller::printTiming(int step) {
	...
	if (simParams->outputTiming && !(step % simParams->outputTiming)) {
		...
		if ((((int) endWTime) >> 6) != (((int) startWTime) >> 6))
			fflush_count = 2;

		const double elapsedW = (endWTime - startWTime) / simParams->outputTiming;
		const double elapsedC = (endCTime - startCTime) / simParams->outputTiming;
...
//fepe
	...
	// Ported by JLai
	potentialEnergy = bondEnergy + angleEnergy + dihedralEnergy + improperEnergy + electEnergy + electEnergySlow
			+ ljEnergy + crosstermEnergy + boundaryEnergy + miscEnergy + goTotalEnergy + groLJEnergy + groGaussEnergy;
	// End of port
	totalEnergy = potentialEnergy + kineticEnergy;
	flatEnergy = totalEnergy + (1.0 / 3.0) * (kineticEnergyHalfstep - kineticEnergyCentered);
	if (!(step % slowFreq)) {
		// only adjust based on most accurate energies
		BigReal s = (4.0 / 3.0) * (kineticEnergyHalfstep - kineticEnergyCentered);
		if (smooth2_avg == XXXBIGREAL)
			smooth2_avg = s;
		if (step != simParams->firstTimestep) {
			smooth2_avg *= 0.9375;
	...
	smoothEnergy = flatEnergy + smooth2_avg - (4.0 / 3.0) * (kineticEnergyHalfstep - kineticEnergyCentered);

	if (simParameters->outputMomenta && !minimize && !(step % simParameters->outputMomenta)) {
		iout<< "MOMENTUM: " << step
		<< " P: " << momentum
		<< " L: " << angularMomentum
		<< "\n" << endi;
	...
	if (pressureProfileSlabs) {
		...
		if (ppbonded)
			ppbonded->getData(first, step, lattice, total);
		if (ppnonbonded)
			ppnonbonded->getData(first, step, lattice, total);
		if (ppint)
			ppint->getData(first, step, lattice, total);
		for (int i = 0; i < arraysize; i++)
			pressureProfileAverage[i] += total[i];
...
#define CALLBACKLIST(LABEL,VALUE) \
	...
	if (step == simParams->N && node->getScript() && node->getScript()->doCallback()) {
		...
		labels << '\0';
		values << '\0';  // insane but makes Linux work
...
#undef CALLBACKDATA
	...
	if (simParams->outputPairlists && pairlistWarnings && !(step % simParams->outputPairlists)) {
	...
	if (!minimize && step % simParameters->outputEnergies)
		return;
...
void Controller::writeExtendedSystemData(int step, std::ofstream &file) {
	...
	file << step;
	if (lattice.a_p())
		file << " " << lattice.a().x << " " << lattice.a().y << " " << lattice.a().z;
	if (lattice.b_p())
		file << " " << lattice.b().x << " " << lattice.b().y << " " << lattice.b().z;
	if (lattice.c_p())
		file << " " << lattice.c().x << " " << lattice.c().y << " " << lattice.c().z;
...
void Controller::outputFepEnergy(int step) {
	if (simParams->alchFepOn) {
		...
		BigReal dE = electEnergy_f + electEnergySlow_f + ljEnergy_f - (electEnergy + electEnergySlow + ljEnergy);
		...
		if (stepInRun == 0) {
			...
			if (!step) {
				if (!FepWhamOn) {
					fepFile << "#NEW FEP WINDOW: " << "LAMBDA SET TO " << alchLambda << " LAMBDA2 " << alchLambda2
							<< std::endl;
				}
			}
		}
		if ((alchEquilSteps) && (stepInRun == alchEquilSteps)) {
			fepFile << "#" << alchEquilSteps << " STEPS OF EQUILIBRATION AT " << "LAMBDA " << simParams->alchLambda
		...
		if (alchEnsembleAvg && (step == simParams->N)) {
			fepSum = fepSum + dG;
			fepFile << "#Free energy change for lambda window [ " << alchLambda << " " << alchLambda2 << " ] is " << dG
...
void Controller::outputTiEnergy(int step) {
	if (simParams->alchThermIntOn) {
		...
		if (stepInRun == 0) {
			...
			BigReal elec_lambda_1 =
					(alchLambda <= alchElecLambdaStart) ?
							0. : (alchLambda - alchElecLambdaStart) / (1. - alchElecLambdaStart);
			BigReal elec_lambda_2 =
					((1 - alchLambda) <= alchElecLambdaStart) ?
							0. : ((1 - alchLambda) - alchElecLambdaStart) / (1. - alchElecLambdaStart);
			BigReal vdw_lambda_1 = (alchLambda >= alchVdwLambdaEnd) ? 1. : alchLambda / alchVdwLambdaEnd;
			BigReal vdw_lambda_2 = ((1 - alchLambda) >= alchVdwLambdaEnd) ? 1. : (1 - alchLambda) / alchVdwLambdaEnd;
			...
			tiFile << "#NEW TI WINDOW: " << "LAMBDA " << alchLambda << "\n#PARTITION 1 VDW LAMBDA " << vdw_lambda_1
		...
		if (stepInRun == alchEquilSteps) {
			tiFile << "#" << alchEquilSteps << " STEPS OF EQUILIBRATION AT " << "LAMBDA " << simParams->alchLambda
					<< " COMPLETED\n" << "#STARTING COLLECTION OF ENSEMBLE AVERAGE" << std::endl;
		}
...
void Controller::outputExtendedSystem(int step) {
	...
	if (step >= 0) {
		...
		if (simParams->xstFrequency && ((step % simParams->xstFrequency) == 0)) {
